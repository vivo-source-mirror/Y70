...
char *get_packagename(uid_t appid)
{
	int i;
	struct hashtable_entry *hash_cur;
	char *package_name;
	...
	hash_for_each_rcu(package_to_appid, i, hash_cur, hlist) {
		if (atomic_read(&hash_cur->value) == (appid_t)appid) {
			rcu_read_unlock();
			package_name = kstrdup(hash_cur->key.name, GFP_KERNEL);
			return package_name;
		}
	}
	rcu_read_unlock();
