...
{
#ifdef ACCESS_PARTITION_BY_VFS
	struct file *fp;

	fp = filp_open(BLOG_DEV, O_RDONLY, 0);
	if (IS_ERR(fp)) {
		blog_dbg("[%s][%d] %s:%d Open %s fail, ret %d!\n", __func__, __LINE__, 
				current->comm, current->pid, BLOG_DEV, PTR_ERR(fp));
		return -ENODEV;
	}

	if (kernel_read(fp, buf, count, (loff_t *)&(pos + BLOG_PARTITION_OFFSET)) != count) {
		blog_dbg("[%s][%d] Read %s fail!\n", __func__, __LINE__, BLOG_DEV);
		return -EPERM;
	}
...
static int write_log_partition(unsigned long pos, unsigned char *buf, size_t count)
{
	...
	fp = filp_open(LBOG_DEV, O_WRONLY, 0);
	if (IS_ERR(fp)) {
		blog_dbg("[%s][%d] %s:%d Open %s fail, ret %d!\n", __func__, __LINE__, 
				current->comm, current->pid, LOG_DEV, PTR_ERR(fp));
		return -ENODEV;
	}

	if (kernel_write(fp, buf, count, (loff_t *)&(pos + BLOG_PARTITION_OFFSET)) != count) {
		blog_dbg("[%s][%d] Write %s fail!\n", __func__, __LINE__, BLOG_DEV);
		return -EPERM;
	}
...
void kstep_log(const char *fmt, ...)
{
	...
	if (clk / 1000000000 > BOOT_STEP_TIMEOUT)
		return;
...
/*
 * format:
 * echo "zygote 1" > /proc/steplog
 * echo "zygote 3 pre" > /proc/steplog
 * echo "zygote 3 post" > /proc/steplog
 */
...
static int __init proc_steplog_init(void)
{
	...
	proc_create("step", 0220, blog_entry, &step_proc_fops);
