...
bool page_poisoning_enabled(void)
{
	 ...
	 */
	 //modify page posion by chenguodong start
	return (want_page_poisoning ||
		(!IS_ENABLED(CONFIG_ARCH_SUPPORTS_DEBUG_PAGEALLOC)));
	//modify page posion by chenguodong end
}
			     ...
			     unsigned char *mem, size_t bytes)
{
	...
	if (!__ratelimit(&ratelimit))
		...
		pr_err("pagealloc: single bit error on page with phys start 0x%px\n",
			(unsigned long)page_to_phys(page));
	else
		pr_err("pagealloc: memory corruption on page with phys start 0x%px\n",
			(unsigned long)page_to_phys(page));
