...
/* vivo lyk add for engineermode begin */
#include <linux/debugfs.h>
/* vivo lyk add for engineermode end */
...
// vivo guojunzheng add for FACTORY_TEST begin
 static ssize_t factory_power_off_write(struct file *file, const char __user *buf, size_t len, loff_t *ppos)
{
	int rc;
	char kbuf[9];
	...
	if (kbuf[0] == '1') {
		printk("factory_power_off_write kernel_power_off\n\r");
		kernel_power_off();
	}
	return len;
}

static const struct file_operations factory_power_off_fops = {
	.owner = THIS_MODULE,
	.write = factory_power_off_write,
};

static struct proc_dir_entry *gFactoryPowerOffEntry;
int factory_power_setup(void)
{
	gFactoryPowerOffEntry = proc_create("driver/factory_power_off", 0664, NULL, &factory_power_off_fops);
	if (gFactoryPowerOffEntry == NULL) {
		printk("Unable to create /proc entry\n\r");
	}
	return 0;
}

int factory_power_remove(void)
{
	if (gFactoryPowerOffEntry != NULL) {
		remove_proc_entry("driver/factory_power_off", NULL);
		gFactoryPowerOffEntry = NULL;
	}
	return 0;
}
// vivo guojunzheng add for FACTORY_TEST end
...
static int __init init_sel_fs(void)
{
	...
	/* vivo lyk add for engineermode begin */
	debugfs_create_file("em_enforce", 0644, NULL, NULL, &sel_em_enforce_ops);
	/* vivo lyk add for engineermode end */
	// vivo guojunzheng add for FACTORY_TEST begin
	factory_power_setup();
	// vivo guojunzheng add for FACTORY_TEST end
