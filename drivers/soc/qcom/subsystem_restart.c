...
#include <linux/idr.h>
//vivo chenming add for subsys begin
#include <linux/debugfs.h>
//vivo chenming add for subsys end
...
extern unsigned int force_ssr_related;
...
const char *ssr_related_module_name = "adsp";
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	for (i = 0; i < ARRAY_SIZE(restart_levels); i++)
		if (!strncasecmp(buf, restart_levels[i], count)) {
			...
			if (!strcmp(subsys->desc->name, ssr_related_module_name)) {
				pr_info("%s: subsys: %s force restart level to RELATED\n", __func__, subsys->desc->name);
				subsys->restart_level = RESET_SUBSYS_COUPLED;
			} else if (force_ssr_related) {
			/*add for ssr begin by hkc*/
				pr_info("%s: force restart level to RELATED\n",
						__func__);
				subsys->restart_level = RESET_SUBSYS_COUPLED;
			}
			/*add for ssr end by hkc*/
			return orig_count;
		}
	return -EPERM;
}
...
{
	subsys_base_dir = debugfs_create_dir("vivo_msm_subsys", NULL);
...
struct subsys_device *subsys_register(struct subsys_desc *desc)
{
	...
	if (ofnode) {
		...
		subsys->restart_level = RESET_SUBSYS_COUPLED;
		if (!strcmp(desc->name, ssr_related_module_name)) {
			subsys->restart_level = RESET_SUBSYS_COUPLED;
		/*add by hkc begin*/
		} else if (is_debug_mode && !force_ssr_related) {
			subsys->restart_level = RESET_SOC;
		}
		/*add by hkc end*/
		printk("HKC subsys[%s] restart_level[%s]\n", desc->name,
		restart_levels[subsys->restart_level]);
