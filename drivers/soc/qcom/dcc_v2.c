...
extern unsigned int is_perf_version;
...
static int dcc_probe(struct platform_device *pdev)
{
	...
	/* If perf version, get dcc configure data from dtsi,
	 * Else dcc will be configured from upper layer such as init.qcom.debug.sh*/
	if (is_perf_version) {
		printk("dcc_configure_list from dtsi\n");
...
extern unsigned int is_vivolog_flag;
/*add by hkc end*/

static int __init dcc_init(void)
{
#if 0 /*force dcc enabled*/
	/*add by hkc begin*/
	if ((is_debug_mode == 0) && (is_vivolog_flag) == 0) {
		printk("dcc_v2 driver not init\n");
		return -ENODEV;
	}
	/*add by hkc end*/
#endif

	printk("dcc_v2 driver init\n");
