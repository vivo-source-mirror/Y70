...
msm_disable_outputs(struct drm_device *dev, struct drm_atomic_state *old_state)
{
	...
	for_each_connector_in_state(old_state, connector, old_conn_state, i) {
		...
		if ((connector->state->crtc &&
			connector->state->crtc->state->active_changed) || is_atboot) {
...
static void complete_commit(struct msm_commit *c)
{
	...
	if (state->ud_flag & UD_FLAG_DC_DIMMING_MASK) {
		SDE_ATRACE_BEGIN("dsi_panel_set_dc_dimming_brightness");
		dsi_panel_set_dc_dimming_brightness(state->dc_diming_brightness, state->ud_flag & UD_FLAG_DC_DIMMING_MASK);
		SDE_ATRACE_END("dsi_panel_set_dc_dimming_brightness");
		state->ud_flag = 0;
	}

	if (state->ud_flag == 1) {
		dsi_panel_hbm_ud_ctrl(true);
		state->ud_flag = 0;
	} else if (state->ud_flag == 2) {
		dsi_panel_hbm_ud_ctrl(false);
		state->ud_flag = 0;
	}
	/*zwl add for smart aod start */
	if (vdisp && vdisp->smart_aod.is_support) {
		if (vdisp->doze_mode_state)
			vivo_smart_aod_update(vdisp);
	}
	/*zwl add for smart aod end */
	...
	/* add for smart aod start */
	if (vdisp && vdisp->smart_aod.is_support) {
		if (vdisp->doze_mode_state && vdisp->aod_display_oning) {
			vivo_smart_aod_display_on(dev, vdisp);
			vdisp->aod_display_oning = false;
		}
	}
	/* add for smart aod end */
