					...
					struct drm_device *dev)
{
	...
	if (!strcmp(vdisp->project_name, "pd2007"))
		props.brightness = 1600;// set default brihgtness to 28% ,same to UEFI.
	else
		props.brightness = bl_config->brightness_max_level * 28 / 100;// set default brihgtness to 28% ,same to UEFI.
...
extern bool get_dc_dimming_state(void);/*vivo display add for dc dimming*/
			...
			size_t *len)
{
	...
	size_t dither_sz = 0;
	u32 *p = (u32 *)cfg;
	/*vivo display add for dc dimming start*/
	static u8 reserve_bitdepth[4] = {};
	static bool dc_state;//0:dc off 1:dc on
	/*vivo display add for dc dimming end*/
	...
	*cfg = msm_property_get_blob(&c_conn->property_info,
	...
	/*vivo display add for dc dimming start*/
	if (get_dc_dimming_state()) {//if first in dc,reserve bitdepth table
		if (dc_state == false) {
			dc_state = true;
			reserve_bitdepth[0] = ((struct drm_msm_dither *)(*cfg))->c0_bitdepth;
			reserve_bitdepth[1] = ((struct drm_msm_dither *)(*cfg))->c1_bitdepth;
			reserve_bitdepth[2] = ((struct drm_msm_dither *)(*cfg))->c2_bitdepth;
			reserve_bitdepth[3] = ((struct drm_msm_dither *)(*cfg))->c3_bitdepth;
			pr_info("%s:reserve_dither[%d,%d,%d,%d]", __func__, reserve_bitdepth[0], reserve_bitdepth[1],
				reserve_bitdepth[2], reserve_bitdepth[3]);
		}
		((struct drm_msm_dither *)(*cfg))->c0_bitdepth = 6;
		((struct drm_msm_dither *)(*cfg))->c1_bitdepth = 6;
		((struct drm_msm_dither *)(*cfg))->c2_bitdepth = 6;
		((struct drm_msm_dither *)(*cfg))->c3_bitdepth = 6;
	} else {//if first out dc,resotre bitdepth table
		if (dc_state == true) {
			dc_state = false;
			((struct drm_msm_dither *)(*cfg))->c0_bitdepth = reserve_bitdepth[0];
			((struct drm_msm_dither *)(*cfg))->c1_bitdepth = reserve_bitdepth[1];
			((struct drm_msm_dither *)(*cfg))->c2_bitdepth = reserve_bitdepth[2];
			((struct drm_msm_dither *)(*cfg))->c3_bitdepth = reserve_bitdepth[3];
			pr_info("%s:resotre_dither[%d,%d,%d,%d]", __func__, reserve_bitdepth[0], reserve_bitdepth[1],
				reserve_bitdepth[2], reserve_bitdepth[3]);
		}
	}
	/*vivo display add for dc dimming end*/
...
/* zhuangwenlong add start for change ESD work interval */
static u32 get_status_work_interval(struct sde_connector *c_conn)
{
	struct dsi_display *display = (struct dsi_display *)c_conn->display;
	struct dsi_panel *panel = display->panel;

	if (panel->esd_config.status_mode == ESD_MODE_TP_REPORT) {
		if (panel->esd_config.esd_check_screendown ||
		    panel->esd_config.last_screendown)
			return STATUS_CHECK_BY_TP_AND_DOWN_INTERVAL_MS;
		else
			return STATUS_CHECK_BY_TP_INTERVAL_MS;
	} else {
		return STATUS_CHECK_INTERVAL_MS;
	}
}
/* zhuangwenlong add end for change ESD work interval */
	...
	if (c_conn->ops.check_status &&
		...
		if (en) {
			...
			interval = c_conn->esd_status_interval ?
				...
				get_status_work_interval(c_conn);
...
static int _sde_connector_update_power_locked(struct sde_connector *c_conn)
{
	...
	void *display;
	struct dsi_display *display_temp;
	...
	if (mode != c_conn->last_panel_power_mode && c_conn->ops.set_power) {
		...
		display_temp = (struct dsi_display *)c_conn->display;
		...
		display_temp->panel->panel_power_mode_state = mode;
		pr_info("panel_power_mode_state = %d\n", display_temp->panel->panel_power_mode_state);
	}
