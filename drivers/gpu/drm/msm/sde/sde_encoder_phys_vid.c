...
extern int sde_notify_fingerprint_status(struct drm_crtc *crtc);
...
not_flushed:
	...
	/*  vivo yjq add for fingerprint start */
	sde_notify_fingerprint_status(phys_enc->parent->crtc);
	/*  vivo yjq add for fingerprint end */

	/*  vivo zwl add for video vblank irq start */
		if (vdisp && vdisp->panel_ops.vblank_irq_handler)
			vdisp->panel_ops.vblank_irq_handler(vdisp);
	/*  vivo zwl add for video vblank irq end */
