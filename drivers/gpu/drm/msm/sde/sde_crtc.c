...
/* zwl add start for fingerprint */
#define FINGERSHOOTER_FRAMEDONE_MS	14
#define SDE_MDELAY(_t)	usleep_range(_t * 1000, _t * 1000 + 100)

atomic_t after_shooter_count;
extern atomic_t after_hbm_count;

void sde_fingerprint_notify_work(struct work_struct *work);
/* zwl add end for fingerprint */
		...
		struct drm_crtc_state *old_crtc_state)
{
	...
	if (idle_time && sde_encoder_check_curr_mode(sde_crtc->mixers[0].encoder,
						MSM_DISPLAY_VIDEO_MODE)) {
		...
		struct drm_crtc_state *old_state)
{
	...
	if (old_state) {
		...
		if (cstate->shooter_flag != 0) {
			atomic_set(&after_shooter_count, 1);
			sde_crtc->fingershooter_status = cstate->shooter_flag;
			pr_info(">>>>>>%s notify shooter: 0x%02x\n", __func__, sde_crtc->fingershooter_status);
		}
	} else {
		SDE_ERROR("error old state is NULL\n");
		return;
	}
	/* zwl add end for fingerprint */
...
struct drm_crtc *sde_crtc_init(struct drm_device *dev, struct drm_plane *plane)
{
	...
	sde_crtc->fingerprint_workq = create_singlethread_workqueue("fingerprint_notify_workq");
	INIT_WORK(&sde_crtc->fingerprint_notify_work, sde_fingerprint_notify_work);
...
/* zwl add start for fingerprint */
void sde_crtc_set_fingershooter(struct drm_crtc_state *state, int shooter_flag)
{
	struct sde_crtc_state *cstate = to_sde_crtc_state(state);

	cstate->shooter_flag = shooter_flag;
}
...
void sde_notify_fingershooter_event(struct drm_device *dev, int status)
{
	char *envp[3];
	int i = 0;
	...
	pr_info("notify uevent - fingershooter show: 0x%x, %d\n", status, i);
}

void sde_fingerprint_notify_work(struct work_struct *work)
{
	struct sde_crtc *sde_crtc = container_of(work, struct sde_crtc,
				fingerprint_notify_work);
	struct drm_crtc *crtc = &sde_crtc->base;

	SDE_ATRACE_BEGIN("sde_fingerprint_notify_work");

	/* wait for frame done */
	SDE_MDELAY(FINGERSHOOTER_FRAMEDONE_MS);

	pr_info("start notify fingershooter state: 0x%02x\n", sde_crtc->fingershooter_status);
	sde_notify_fingershooter_event(crtc->dev, sde_crtc->fingershooter_status);

	sde_crtc->fingershooter_status = 0;

	SDE_ATRACE_END("sde_fingerprint_notify_work");
}

int sde_notify_fingerprint_status(struct drm_crtc *crtc)
{
	struct sde_crtc *sde_crtc = container_of(crtc, struct sde_crtc, base);
	bool need_notify = false;

	if (!crtc || !crtc->dev) {
		/*In the process of shutdown, CRTC resources will be released and
		  printed in large quantities*/
		//SDE_ERROR("crtc is null or drm device is null\n");
		return -ENODEV;
	}
	...
	if (atomic_read(&after_shooter_count) > 0)
		atomic_inc(&after_shooter_count);

	if (atomic_read(&after_hbm_count) > 0 && atomic_read(&after_hbm_count) < 3)
		atomic_inc(&after_hbm_count);

	need_notify = (sde_crtc->fingershooter_status & SHOOTER_SHOW_BIT) ?
		(atomic_read(&after_shooter_count) >= 2 && atomic_read(&after_hbm_count) >= 3) :
		atomic_read(&after_shooter_count) >= 2;

	if (need_notify) {
		SDE_ATRACE_BEGIN("sde_fingerprint_notify_status");
		pr_info("prepare notify fingershooter state: 0x%02x\n", sde_crtc->fingershooter_status);
		queue_work(sde_crtc->fingerprint_workq, &sde_crtc->fingerprint_notify_work);
		atomic_set(&after_shooter_count, 0);
		SDE_ATRACE_END("sde_fingerprint_notify_status");
	}
