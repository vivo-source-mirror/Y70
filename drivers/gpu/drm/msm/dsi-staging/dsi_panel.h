...
struct drm_panel_esd_config {
	bool esd_enabled;
	/* zhuangwenlong add start for check screendown */
	int esd_check_screendown;
	int last_screendown;
	/* zhuangwenlong add end for check screendown  */
...
static inline bool dsi_panel_mode_switch_pending(struct dsi_panel *panel)
{
	return (panel->panel_mode_switch_enabled && atomic_read(&panel->mode_switch_pending));
}

static inline bool dsi_panel_unset_mode_state(struct dsi_panel *panel)
{
	return (panel->panel_mode_switch_enabled &&
		(panel->power_mode != SDE_MODE_DPMS_ON ||
		 atomic_read(&panel->mode_switch_pending)));
}
