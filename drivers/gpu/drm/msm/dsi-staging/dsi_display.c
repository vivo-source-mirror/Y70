...
static irqreturn_t dsi_display_panel_te_irq_handler(int irq, void *data)
{
	...
	dsi_panel_for_ud_te_irq_handler(display);
...
static void dsi_display_register_te_irq(struct dsi_display *display)
{
	...
	disable_irq(te_irq);
	display->is_te_irq_enabled = false;
	dsi_panel_ud_register_te_irq(display);
	...
	for (j = 0; j < config->groups; ++j) {
		for (i = 0; i < len; ++i) {
			if (config->return_buf[i] !=
				...
				DRM_ERROR("mismatch: read panel reg value=0x%x,expect value=0x%x\n",
		...
		struct dsi_panel *panel)
{
	...
	LCD_INFO("dsi read panel reg 0x%x\n", config->return_buf[0]);
	return rc;
}
...
extern bool check_lcdb_reg(void);
extern int dump_full_lcdb_registers(void);
...
exit:
	...
	if (rc <= 0) {
		count++;
		/*when esd check fail greater 4 and lcdb_enable_reg equal 0, do reboot*/
		if (count > 4) {
			if (!check_lcdb_reg()) {
				dump_full_lcdb_registers();
...
int lcd_need_recovery = 0;
extern int panel_recovery_no_powerctl;

static int dsi_display_status_tp_report(struct dsi_display *display)
{
	if (lcd_need_reset || lcd_need_recovery) {
		panel_recovery_no_powerctl = lcd_need_recovery ? 0 : 1;
					...
					bool te_check_override)
{
	...
	if (status_mode == ESD_MODE_REG_READ) {
		...
		rc = dsi_display_status_check_te(dsi_display);
	} else if (status_mode == ESD_MODE_TP_REPORT) {
		rc = dsi_display_status_tp_report(dsi_display);
	} else {
		...
		int power_mode, void *disp)
{
	...
	pr_err("Power mode transition from %d to %d %s",
	...
	struct device_attribute *attr, const char *buf, size_t count)
{
	...
	int clk_rate;
	struct dsi_display *display;
#ifdef PANEL_DYNAMIC_DSI_BITRATE_FROM_DTS
	struct dsi_panel *panel;
	...
	if (vdisp) {
		if (!vdisp->dynamic_dsi_timing_enable) {
			pr_err("can not support dynamic clk\n");
			return rc;
		}

		if (clk_rate != vdisp->default_bitrate || clk_rate != vdisp->intent_bitrate) {
			pr_err("%s: bitrate should be match driver init setting\n", __func__);
			return count;
		}
	}
#endif
...
int dsi_display_enable(struct dsi_display *display)
{
	 ...
	 */
	if (display->is_cont_splash_enabled) {
		struct vivo_display_driver_data *vdisp = display->panel->panel_private;
		...
		/*
			B190522-870:at the first boot, the panel has not been initialized,
			application set color mode,may not work.
		*/
		dsi_panel_acquire_panel_lock(display->panel);
		if (vdisp && vdisp->colour_gamut_state)
			dsi_panel_update_seed_crc(display->panel, vdisp->colour_gamut_state);
		dsi_panel_release_panel_lock(display->panel);
