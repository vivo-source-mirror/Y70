...
static void dsi_bridge_enable(struct drm_bridge *bridge)
{
	...
	SDE_ATRACE_BEGIN("dsi_display_post_enable");
	rc = dsi_display_post_enable(display);
	SDE_ATRACE_END("dsi_display_post_enable");
	...
	if (display && display->drm_conn) {
		if (c_bridge->dsi_mode.dsi_mode_flags & DSI_MODE_FLAG_POMS) {
			...
			pr_info("dsi_bridge_enable: set allow_bl_update to true");
		} else {
...
static void dsi_bridge_disable(struct drm_bridge *bridge)
{
	...
	if (display && display->drm_conn) {
		if (bridge->encoder->crtc->state->adjusted_mode.private_flags &
			...
			pr_info("dsi_bridge_disable:set allow_bl_update to false");
