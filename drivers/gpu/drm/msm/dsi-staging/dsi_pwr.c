...
extern void qpnp_lcdb_set_force_pd_enable_wa(int enable);
/*
...
int dsi_pwr_enable_by_name(struct dsi_regulator_info *regs, char *vreg_name, bool enable)
{
	int rc = 0, i = 0;
	struct dsi_vreg *vreg;
	int num_of_v = 0;
	...
	if (enable) {
		...
		for (i = (regs->count - 1); i >= 0; i--) {
			if (!strcmp(regs->vregs[i].vreg_name, vreg_name)) {
				if (!strcmp(regs->vregs[i].vreg_name, "lab") ||
				    !strcmp(regs->vregs[i].vreg_name, "ibb"))
					qpnp_lcdb_set_force_pd_enable_wa(0);
