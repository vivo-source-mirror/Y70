...
extern bool esd_detect_fail_recovery;
atomic_t vddi_forpimc_refcount;
int panel_on_state;
int panel_reset_state;
int panel_recovery_no_powerctl;
extern int lcd_need_reset;
extern int lcd_need_recovery;
static int unset_backlight;
...
const char *cmd_set_prop_map[DSI_CMD_SET_MAX] = {
	...
	"qcom,mdss-dsi-all-pixels-off-command",
	"qcom,mdss-dsi-all-pixels-on-command",
	...
	"qcom,mdss-oled-aod-on-command-first",
	...
	"qcom,mdss-dsi-deep-standby-command",
...
const char *cmd_set_state_map[DSI_CMD_SET_MAX] = {
	...
	"qcom,mdss-dsi-all-pixels-off-command-state",
	"qcom,mdss-dsi-all-pixels-on-command-state",
	...
	"qcom,mdss-alpm-command-state",
	"qcom,mdss-alpm-command-state",
	"qcom,mdss-alpm-command-state",
	"qcom,mdss-alpm-command-state",
	"qcom,mdss-alpm-command-state",
	"qcom,mdss-default-command-state",
	"qcom,mdss-alpm-command-state",
	"qcom,mdss-alpm-command-state",
	"qcom,mdss-alpm-command-state",
	"qcom,mdss-alpm-command-state",
	...
	"qcom,mdss-alpm-command-state",
	"qcom,mdss-alpm-command-state",
	"qcom,mdss-oled-seed-crc-compensate-command-state",
	"qcom,mdss-oled-seed-crc-compensate-command-state",
	"qcom,mdss-oled-seed-crc-compensate-command-state",
	"qcom,mdss-oled-seed-crc-compensate-command-state",
	"qcom,mdss-oled-seed-crc-compensate-command-state",
	...
	"qcom,mdss-oled-seed-crc-compensate-command-state",
	"qcom,mdss-oled-seed-crc-compensate-command-state",
	"qcom,mdss-oled-p3-crc-seed-dc-dimming-level-state",
	"qcom,mdss-oled-p3-crc-seed-dc-dimming-level-state",
	"qcom,mdss-oled-p3-crc-seed-dc-dimming-level-state",
	"qcom,mdss-oled-srgb-seed-dc-dimming-level-state",
	"qcom,mdss-oled-srgb-seed-dc-dimming-level-state",
	"qcom,mdss-oled-srgb-seed-dc-dimming-level-state",
	"qcom,mdss-dsi-off-command-state",
	"qcom,mdss-default-command-state",
...
static int dsi_panel_gpio_request(struct dsi_panel *panel)
{
	...
	if (gpio_is_valid(r_config->disp_enp_gpio)) {
		rc = gpio_request(r_config->disp_enp_gpio, "disp_enp_gpio");
		if (rc) {
			pr_err("request for disp_enp_gpio failed, rc=%d\n", rc);
			goto error_release_disp_enp;
		}
	}

	if (gpio_is_valid(r_config->disp_enn_gpio)) {
		rc = gpio_request(r_config->disp_enn_gpio, "disp_enn_gpio");
		if (rc) {
			pr_err("request for disp_enn_gpio failed, rc=%d\n", rc);
...
error_release_disp_enn:
	if (gpio_is_valid(r_config->disp_enp_gpio))
		gpio_free(r_config->disp_enp_gpio);
...
static int dsi_panel_gpio_release(struct dsi_panel *panel)
{
	...
	if (gpio_is_valid(r_config->disp_enn_gpio))
		gpio_free(r_config->disp_enn_gpio);
...
int incell_tp_reset_output_paneloff(int enable)
{
	...
	pr_info("tp reset %d called by tp\n", enable);

	if (gpio_is_valid(panel->reset_config.tp_reset_gpio))
		gpio_set_value(panel->reset_config.tp_reset_gpio, enable);

	return 0;
}

static int dsi_panel_pre_reset_for_tddi(struct dsi_panel *panel)
{
	struct dsi_panel_reset_config *r_config = &panel->reset_config;
	int rc = 0;
	...
	/* tp reset is always high when power off, so set it low first */
	if (gpio_is_valid(r_config->tp_reset_gpio))
		gpio_set_value(r_config->tp_reset_gpio, 0);

	/* lcd reset is always high when power off, so set it low first */
	if (gpio_is_valid(r_config->reset_gpio))
		gpio_set_value(r_config->reset_gpio, 0);
...
static int dsi_panel_reset_for_tddi(struct dsi_panel *panel, int enable)
{
	int rc = 0;
	struct dsi_panel_reset_config *r_config = &panel->reset_config;
	int i;
	...
	if (enable == 1) {
		/* set tp reset high */
		if (gpio_is_valid(r_config->tp_reset_gpio)) {
			rc = gpio_direction_output(r_config->tp_reset_gpio, 1);
			if (rc) {
				pr_err("unable to set dir and output high for tp rst gpio rc=%d\n", rc);
				goto exit;
			}
			gpio_set_value(r_config->tp_reset_gpio, 1);
...
exit:
	if (rc) {
		...
		(void)dsi_panel_set_pinctrl_state(panel, false);
		(void)dsi_pwr_enable_regulator(&panel->power_info, false);
		atomic_set(&vddi_forpimc_refcount, 0);
	}

	return rc;
}

static int dsi_panel_enp_and_enn_for_panel_suppy(struct dsi_panel *panel, bool enable)
{
	int rc = 0;
	...
	if (enable) {
		if (gpio_is_valid(panel->reset_config.disp_enp_gpio)) {
			rc = gpio_direction_output(panel->reset_config.disp_enp_gpio, 1);
			if (rc) {
				pr_err("unable to set dir for disp_enp_gpio rc=%d\n", rc);
				goto exit;
			}
			gpio_set_value(panel->reset_config.disp_enp_gpio, 1);
		}

		usleep_range(3300, 3800);

		if (gpio_is_valid(panel->reset_config.disp_enn_gpio)) {
			rc = gpio_direction_output(panel->reset_config.disp_enn_gpio, 1);
			if (rc) {
				pr_err("unable to set dir for disp_enn_gpio  rc=%d\n", rc);
				goto exit;
			}
			gpio_set_value(panel->reset_config.disp_enn_gpio, 1);
		}

		usleep_range(800, 1300);
	} else {
		if (gpio_is_valid(panel->reset_config.disp_enn_gpio)) {
			rc = gpio_direction_output(panel->reset_config.disp_enn_gpio, 0);
			if (rc) {
				pr_err("unable to set dir for disp_enn_gpio  rc=%d\n", rc);
				goto exit;
			}
		...
		if (gpio_is_valid(panel->reset_config.disp_enp_gpio)) {
			rc = gpio_direction_output(panel->reset_config.disp_enp_gpio, 0);
			if (rc) {
				pr_err("unable to set dir for disp_enp_gpio  rc=%d\n", rc);
				goto exit;
			}

			/* Make sure that VSP reaches 0V before vddI is powered off */
			usleep_range(4300, 4800);
		}
	}
exit:
	return rc;
...
int incell_avdd_avee_output(int enable)
{
	struct vivo_display_driver_data *vdisp = vivo_get_vdisp();
	struct dsi_panel *panel = (struct dsi_panel *)vdisp->msm_private;
	int ret = 0;
	...
	pr_info("avdd/avee output %d called by tp\n", enable);

	ret = dsi_panel_enp_and_enn_for_panel_suppy(panel, enable);
...
static int dsi_panel_power_on_for_tddi(struct dsi_panel *panel)
{
	int rc = 0;

	if (panel_recovery_no_powerctl) {
		pr_info("no do panel power on for panel recovery\n");
		panel_recovery_no_powerctl = 0;
		return 0;
	}

	pr_info("[%s]\n", panel->name);

	if (!atomic_read(&vddi_forpimc_refcount)) {
		rc = dsi_pwr_enable_regulator(&panel->power_info, true);
		if (!rc) {
			atomic_set(&vddi_forpimc_refcount, 1);
		} else {
			pr_err("[%s] failed to enable vregs, rc=%d\n", panel->name, rc);
			goto exit;
		}
	...
	if (gpio_is_valid(panel->bl_config.en_gpio)) {
		rc = gpio_direction_output(panel->bl_config.en_gpio, 1);
		pr_info("[%s] success set bl_en gpio\n", panel->name);
		if (rc)
			pr_err("unable to set dir for bklt gpio rc=%d\n", rc);
	}
...
exit:
	if (rc == 0)
		panel_reset_state = TDDI_PANEL_POWER_ON;

	return rc;
}

static int dsi_panel_power_off_for_tddi(struct dsi_panel *panel)
{
	int rc = 0;

	if (panel_recovery_no_powerctl) {
		pr_info("no do panel power off for panel recovery\n");
		return 0;
	}
	...
	if (gpio_is_valid(panel->bl_config.en_gpio)) {
		...
		pr_info("[%s] success close bl_en gpio\n", panel->name);
	}
	...
	if (rc == 0)
		panel_reset_state = TDDI_PANEL_POWER_OFF;

	return rc;
}

/*
 * dsi_panel_reset_and_powerctl
 * enable:
 *		0: power off
 *		1: power on, tp reset and lcd reset
 *		2: only power on, but no reset
 *		3: power on and tp reset, but no lcd reset
 */
int dsi_panel_reset_and_powerctl(struct dsi_panel *panel, int enable)
{
	int rc = 0;

	pr_info("for tddi change %d to %d\n", panel_reset_state, enable);

	if (enable == panel_reset_state && enable != 2) {
		pr_err("dsi panel power state have been set %d\n", panel_reset_state);
		return rc;
	}

	if (enable) {
		rc = dsi_panel_power_on_for_tddi(panel);
		if (rc)
			goto exit;

		/* first part reset */
		if (enable != 3) {
			rc = dsi_panel_pre_reset_for_tddi(panel);
			if (rc)
				goto exit;
		}
		...
		if (enable == 1 || enable == 3)
			rc = dsi_panel_reset_for_tddi(panel, enable);
	} else {
		rc = dsi_panel_reset_for_tddi(panel, 0);
		rc = dsi_panel_power_off_for_tddi(panel);
	}

exit:
	if (rc)
		pr_err("[%s] panel power setting failed, rc=%d\n", panel->name, rc);
...
int mdss_dsi_panel_reset_and_powerctl(int enable)
{
	...
	/* if panel on state, Prohibited panel reset */
	if (panel_on_state) {
		pr_err("failed to operate reset and power when panel state %d!\n", panel_on_state);
		goto exit;
	}

	pr_info("dsi panel power and reset called by tp, enable=%d\n", enable);

	rc = dsi_panel_reset_and_powerctl(panel, enable);
				...
				enum dsi_cmd_set_type type)
{
	...
	if (!panel || !panel->cur_mode)
		return -EINVAL;
	if (!strcmp(vdisp->project_name, "no_panel_video"))
		return rc;
	...
	pr_err("%s:cmds[%d]=%s, state %d\n", __func__, type, cmd_set_prop_map[type], state);
	for (i = 0; i < count; i++) {
...
int dsi_panel_write_bl_by_cmd2(struct mipi_dsi_device *dsi, int bl_lvl)
{
	int ret = 0;
	u8 val[2];

	val[0] = (u8)((bl_lvl >> 3) & 0xFF);/* high 8 bit*/
	val[1] = ((u8)(bl_lvl & 0x07)) << 1;	/* low 3 bit*/

	ret = mipi_dsi_dcs_write(dsi, 0x41, (u8[]){0x5A, 0x25}, 2);
	ret |= mipi_dsi_dcs_write(dsi, 0x8F, (u8[]){val[0], val[1]}, 2);
	ret |= mipi_dsi_dcs_write(dsi, 0x41, (u8[]){0X00, 0X2F}, 2);

	return ret;
}

extern void lm3697_brightness_set (int value);/* add for lm3697 backlight */
	...
	u32 bl_lvl)
{
	...
	if ((vdisp->silent_reboot == 1) && (bl_lvl == 0)) {
		vdisp->silent_reboot = 0;
		pr_err("silent_reboot is clear %d\n", vdisp->silent_reboot);
	}

	if (vdisp->silent_reboot == 1)
		bl_lvl = 0;
	/* add for silent reboot end */
...
int dsi_panel_set_backlight(struct dsi_panel *panel, u32 bl_lvl)
{
	...
	if (dsi_panel_mode_switch_pending(panel) ||
	    (panel->panel_mode_switch_enabled && panel->panel_mode != DSI_OP_VIDEO_MODE && bl_lvl > 0)) {
		pr_info("mode switch pending not allow to setbacklight\n");
		unset_backlight = bl_lvl;
		return rc;
	}
	...
	case DSI_BACKLIGHT_I2C:
		rc = vivo_panel_set_backlight(panel, bl_lvl);
		unset_backlight = 0;
		break;
				  ...
				  struct dsi_parser_utils *utils)
{
	...
	if (vdisp && vdisp->lcm_internal_id) {
		pr_err("internal version\n");
		rc = utils->read_u32(utils->data,
					"qcom,mdss-dsi-h-front-porch-internal",
					  &mode->h_front_porch);
		if (rc) {
			pr_err("failed to read qcom,mdss-dsi-h-front-porch-internal, rc=%d\n",
				   rc);
			goto error;
		}

		rc = utils->read_u32(utils->data,
					"qcom,mdss-dsi-h-back-porch-internal",
					  &mode->h_back_porch);
		if (rc) {
			pr_err("failed to read qcom,mdss-dsi-h-back-porch-internal, rc=%d\n",
				   rc);
			goto error;
		}

		rc = utils->read_u32(utils->data,
					"qcom,mdss-dsi-h-pulse-width-internal",
					  &mode->h_sync_width);
		if (rc) {
			pr_err("failed to read qcom,mdss-dsi-h-pulse-width-internal, rc=%d\n",
				   rc);
			goto error;
		}
	} else {
					    ...
					    const char *name)
{
	...
	if (vdisp && vdisp->lcm_internal_id) {
		pr_err("internal version\n");
		rc = utils->read_u32(utils->data, "qcom,mdss-dsi-t-clk-post-internal", &val);
		if (!rc) {
			host->t_clk_post = val;
		...
		val = 0;
		rc = utils->read_u32(utils->data, "qcom,mdss-dsi-t-clk-pre-internal", &val);
		if (!rc) {
			host->t_clk_pre = val;
		...
		struct dsi_parser_utils *utils, enum dsi_op_mode panel_mode)
{
	...
	if (vdisp && vdisp->lcm_internal_id) {
		pr_err("internal version\n");
		data = utils->get_property(utils->data,
				"qcom,mdss-dsi-panel-phy-timings-internal", &len);
	} else {
		data = utils->get_property(utils->data,
...
static int dsi_panel_parse_esd_config(struct dsi_panel *panel)
{
	...
	if (!rc) {
		...
		} else if (!strcmp(string, "esd_tp_report")) {
			esd_config->status_mode = ESD_MODE_TP_REPORT;
		} else {
	...
	if (panel->esd_config.status_mode == ESD_MODE_REG_READ) {
		...
		esd_mode = "te_check";
	} else if (panel->esd_config.status_mode == ESD_MODE_TP_REPORT) {
		esd_mode = "esd_tp_report";
	}
...
static int dsi_panel_write_cmd(struct dsi_panel *panel,
...
static void qcom_vivo_panel_ops_init(struct vivo_display_driver_data *vdisp)
{
	LCD_INFO("\n");
	vdisp->panel_ops.dsi_write_cmd = dsi_panel_write_cmd;
	vdisp->panel_ops.mode_switch_pending = dsi_panel_mode_switch_pending;
	vdisp->panel_ops.unset_mode_state = dsi_panel_unset_mode_state;
	vdisp->panel_ops.update_backlight = dsi_panel_update_backlight;
}
				...
				int topology_override)
{
	...
	vdisp = vivo_get_vdisp();
	if (vdisp) {
		qcom_vivo_panel_ops_init(vdisp);
	}
	drm_panel_init(&panel->drm_panel);
	mutex_init(&panel->panel_lock);
	vivo_panel_init(panel);
	panel->force_dimmng_control = 1;
	panel->panel_power_mode_state = SDE_MODE_DPMS_ON;
	atomic_set(&vddi_forpimc_refcount, 1);
	panel_reset_state = TDDI_PANEL_POWER_ON;
...
int dsi_panel_pre_prepare(struct dsi_panel *panel)
{
	...
	vdisp = (struct vivo_display_driver_data *)panel->panel_private;
	mutex_lock(&panel->panel_lock);
	pr_info("[%s]\n", panel->name);
...
int dsi_panel_set_lp1(struct dsi_panel *panel)
{
	...
	pr_err("[%s]dsi_panel_set_lp1\n", panel->name);
	mutex_lock(&panel->panel_lock);

	if (panel->doze_mode_state == 1) {
		pr_err("[%s] do not need to set aod on again\n", panel->name);
	} else {
		if (panel->oled_hbm_state == PANEL_HBM_AOD_UD_ON && panel->bl_config.bl_level == 0 &&
			panel->ud_hbm_state == UD_HBM_ON) {
			pr_err("[%s] aod off but hbm on cannot enter aod mode\n", panel->name);
		} else if ((panel->oled_hbm_state == PANEL_HBM_AOD_UD_ON) || (panel->oled_hbm_state == PANEL_HBM_AOD_UD_OFF) ||
				   (panel->bl_config.bl_level) ||
				   (panel->panel_power_mode_state != SDE_MODE_DPMS_OFF)) {
			pr_info("set hlpmmode oled_hbm_state = %d; bl_level = %d\n", panel->oled_hbm_state,
					panel->bl_config.bl_level);
			rc = dsi_panel_tx_cmd_set(panel, DSI_CMD_SET_AOD_ON);
			if (rc)
				pr_err("[%s] failed to send SET_AOD_ON cmd, rc=%d\n", panel->name, rc);
			rc = dsi_panel_tx_cmd_set(panel, DSI_CMD_SET_HLPM_HIGHT_BRIGHTNESS);
			if (rc)
				pr_err("[%s] failed to send SET_HLPM_HIGHT cmd, rc=%d\n", panel->name, rc);
			panel->doze_mode_state = 1;
			vdisp->doze_mode_state = 1;
			panel->oled_hlpm_state = 1;
			panel->bl_config.bl_level = 0;
			if (vdisp->smart_aod.is_support && panel->panel_mode == DSI_OP_CMD_MODE)
				vivo_smart_aod_set(vdisp, &vdisp->smart_aod);

			if (vdisp->smart_aod.is_support) {
				if (panel->panel_mode == DSI_OP_VIDEO_MODE)
					vdisp->smart_aod_update_jiffies = get_jiffies_64();
				else
					vdisp->aod_display_oning = true;
			}
...
int dsi_panel_set_lp2(struct dsi_panel *panel)
{
	...
	pr_err("[%s]dsi_panel_set_lp2\n", panel->name);
	mutex_lock(&panel->panel_lock);
	if (panel->doze_mode_state == 1) {
		pr_err("[%s] do not need to set aod on again\n", panel->name);
	} else {
		if (panel->oled_hbm_state == 6 && panel->bl_config.bl_level == 0 &&
			panel->ud_hbm_state == 1) {
			pr_err("[%s] aod off but hbm on cannot enter aod mode\n", panel->name);
		} else if ((panel->oled_hbm_state == 6) || (panel->oled_hbm_state == 7) ||
				   (panel->bl_config.bl_level) ||
				   (panel->panel_power_mode_state != SDE_MODE_DPMS_OFF)) {
			pr_info("lp2 set hlpmmode oled_hbm_state = %d; bl_level = %d\n", panel->oled_hbm_state,
					panel->bl_config.bl_level);
			rc = dsi_panel_tx_cmd_set(panel, DSI_CMD_SET_AOD_ON);
			if (rc)
				pr_err("[%s] failed to send DSI_CMD_SET_LP1 cmd, rc=%d\n",
...
int dsi_panel_set_nolp(struct dsi_panel *panel)
{
	...
	pr_err("[%s]dsi_panel_set_nolp\n", panel->name);
	mutex_lock(&panel->panel_lock);
	if (panel->doze_mode_state == 0) {
		pr_err("[%s] do not need to set aod off again\n", panel->name);
	} else {
		rc = dsi_panel_tx_cmd_set(panel, DSI_CMD_SET_AOD_OFF);/*DSI_CMD_SET_NOLP*/
		if (rc)
			pr_err("[%s] failed to send DSI_CMD_SET_NOLP cmd, rc=%d\n",
			       panel->name, rc);
		panel->doze_mode_state = 0;
		vdisp->doze_mode_state   = 0;
		panel->oled_hlpm_state = 0;
		panel->oled_alpm_state = 0;
		if (vdisp->smart_aod.is_support) {
			vdisp->display_off_jiffies = get_jiffies_64();
			vdisp->smart_aod_update_jiffies = 0;
		}
	}

	if (panel->power_mode == SDE_MODE_DPMS_OFF && panel->panel_mode_switch_enabled
		&& panel->panel_mode == DSI_OP_VIDEO_MODE && vdisp->oled_hbm_state == PANEL_HBM_POWER_OFF) {
		dsi_panel_tx_cmd_set(panel, DSI_CMD_SET_DISPLAY_ON);
		dsi_panel_unset_state_reset(panel);
	}
...
int dsi_panel_prepare(struct dsi_panel *panel)
{
	...
	vdisp = (struct vivo_display_driver_data *)panel->panel_private;
	mutex_lock(&panel->panel_lock);
	if (vdisp && vdisp->smart_aod.is_support)
		vdisp->lp11_jiffies = get_jiffies_64();
...
int dsi_panel_pre_mode_switch_to_video(struct dsi_panel *panel)
{
	...
	mutex_lock(&panel->panel_lock);
	atomic_set(&panel->mode_switch_pending, 1);
...
int dsi_panel_mode_switch_to_cmd(struct dsi_panel *panel)
{
	...
	vdisp = panel->panel_private;
	mutex_lock(&panel->panel_lock);
	/* make sure delay 100ms after sleep out */
	if (vdisp && vdisp->sleep_out_jiffies) {
		LCD_INFO("delay 100ms after sleep out\n");
		do_away_udelay(vdisp->sleep_out_jiffies, 100000);
	}
	...
	atomic_set(&panel->mode_switch_pending, 0);
	mutex_unlock(&panel->panel_lock);
	return rc;
}
...
int dsi_panel_mode_switch_to_vid(struct dsi_panel *panel)
{
	...
	/* make sure delay 34ms before display on for aod -> normal */
	if (vdisp && vdisp->display_off_jiffies) {
		do_away_udelay(vdisp->display_off_jiffies, 34000);
		vdisp->display_off_jiffies = 0;
		vdisp->sleep_out_jiffies = 0;
	}
	...
	if (rc)
		pr_err("[%s] failed to send DSI_CMD_SET_POST_CMD_TO_VID_SWITCH cmds, rc=%d\n",
			panel->name, rc);
	atomic_set(&panel->mode_switch_pending, 0);
	if (panel->power_mode == SDE_MODE_DPMS_ON && panel->panel_mode_switch_enabled)
		dsi_panel_unset_state_reset(panel);

	if (unset_backlight)
		rc = vivo_panel_set_backlight(panel, unset_backlight);
	mutex_unlock(&panel->panel_lock);
	return rc;
}
...
int dsi_panel_enable(struct dsi_panel *panel)
{
	...
	vdisp = panel->panel_private;
	mutex_lock(&panel->panel_lock);
	if (vdisp && vdisp->lp11_jiffies) {
		do_away_udelay(vdisp->lp11_jiffies, 12000);
		vdisp->lp11_jiffies = 0;
	}
	...
	/* for off -> on */
	if (vdisp && vdisp->smart_aod.is_support)
		vdisp->sleep_out_jiffies = get_jiffies_64();

	panel->panel_initialized = true;

	/*below is from PD1932,update acl when resume */
	if (vdisp->oled_acl_state)
		dsi_panel_update_acl(panel, vdisp->oled_acl_state);

	lcd_need_reset = 0;
	lcd_need_recovery = 0;

	if (vdisp->colour_gamut_state && !vdisp->smart_aod.is_support)
...
int dsi_panel_disable(struct dsi_panel *panel)
{
	...
	vdisp->oled_ore_on = 0;
	panel->ud_hbm_state = 0; /* clear hbm status when display is disabled */
	vdisp->tft_cabc_state = -1;
	vdisp->lcm_sre_level = 0;
	/*vdisp->oled_acl_state = 0;*/
	panel->ud_hbm_state = 0; /* clear hbm status when display is disabled */
	panel_on_state = 0;
