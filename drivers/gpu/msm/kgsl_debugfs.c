...
#ifdef CONFIG_RMS_GPU_STAT
static int process_mem_show_quick(struct seq_file *s, void *v)
{
	struct kgsl_process_private *private = s->private;
	struct kgsl_mem_entry *entry;
	uint64_t gl_counted = 0;
	uint64_t gl_uncounted = 0;
	uint64_t egl_counted = 0;
	uint64_t egl_uncounted = 0;
	uint64_t other_counted = 0;
	uint64_t other_uncounted = 0;
	...
	while (1) {
		...
		if (kgsl_mem_entry_get(entry)) {
			...
			if (m->size <= 0 || m->mapsize < 0 || m->mapsize > m->size) {
				kgsl_mem_entry_put(entry);
				continue;
			}

			usermem_type = kgsl_memdesc_usermem_type(m);
			if (usermem_type == KGSL_MEM_ENTRY_KERNEL) {
				if (m->useraddr) {
					gl_counted += m->mapsize;
					gl_uncounted += (m->size - m->mapsize);
				} else {
					gl_uncounted += m->size;
				}
			} else if (usermem_type == KGSL_MEM_ENTRY_ION) {
				kgsl_get_egl_counts(entry, &egl_surface_count,
						&egl_image_count);
				if (egl_surface_count > 0) {
					mem_type = kgsl_memdesc_get_memtype(&entry->memdesc);
					if (mem_type == KGSL_MEMTYPE_EGL_SURFACE)
						egl_uncounted += m->size;
				} else {
					egl_uncounted += (m->size / (egl_image_count ? egl_image_count : 1));
				}
			}
			kgsl_mem_entry_put(entry);
		} else {
	...
	seq_printf(s, "%llu %llu %llu %llu %llu %llu\n",
			gl_counted, gl_uncounted,
			egl_counted, egl_uncounted,
			other_counted, other_uncounted);
	if (private)
		kgsl_process_private_put(private);
	return 0;
}

static int process_mem_open_quick(struct inode *inode, struct file *file)
{
	...
	ret = single_open(file, process_mem_show_quick, private);

	return ret;
}

static const struct file_operations process_mem_fops_quick = {
	.open = process_mem_open_quick,
	.read = seq_read,
	.llseek = seq_lseek,
...
#ifdef CONFIG_RMS_GPU_STAT
	dentry_quick = debugfs_create_file("mem_quick", 0444, private->debug_root,
		(void *) ((unsigned long) private->pid), &process_mem_fops_quick);

	if (IS_ERR_OR_NULL(dentry_quick))
		WARN((dentry_quick == NULL),
			"Unable to create 'mem_quick' file for %s\n", name);
#endif
}
