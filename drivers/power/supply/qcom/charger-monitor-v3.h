#ifndef __CHARGER_MONITOR_V2_H
#define __CHARGER_MONITOR_V2_H

/* voter define */
#define CALLING_EVENT		"CALLING_EVENT"
#define WEIXIN_CALLING_EVENT		"WEIXIN_CALLING_EVENT"
#define CAMERA4K_EVENT		"CAMERA4K_EVENT"
...
#define CHECK_FASTCH_CURRENT_IBAT_THRESHOLD_MA	2100
...
struct cms{
	...
	int bat_id_nums;
	int *bat_id_value;
	bool bat_id_new_param_enable;
	const char * const *bat_id_strings;
	const struct r_item *r_items;
	int r_items_num;
	const struct rc_item *rc_items;
	int rc_items_num;
	int vadc_chan;
	int battery_present;
	int ac_avg_current;
	...
	bool switch_scale_enable;
	bool switch_state;
	int *switch_scale_data;
	...
	bool camera4k_scale_enable;
	bool camera4k_state;
	int *camera4k_scale_data;
	...
	int ex_normal_tc_enable_num;
	int *ex_normal_tc_limit;
	int *ex_normal_tc_rc;
	struct tc_data_item_ex *ex_normal_tc_data;
	int tc_type;
	...
	/*adapter protect function*/
	int *adapter_cooldown_data;
	int adapter_power_derate_enable;
	int adapter_power_derate_param_ready;

	bool ex_fg_fake_batt_id_start;


	/* add for new large current test */
	int battery_temp;
	int main_board_temp;
	int sub_board_temp;
	...
	/*data processing*/
	bool dp_batt_temp_enable;
	int dp_batt_temp_data;

	/* battery id pinctrl */
	struct pinctrl *pinctrl;
	struct pinctrl_state *batid_adc_default;
	struct pinctrl_state *batid_gpio_default;
	struct pinctrl_state *batid_gpio_high;
	struct pinctrl_state *batid_gpio_low;
	bool batid_pinctrl_enable;
	bool batid_extra_handle_enable;
...
static const char * const battery_id_strings[] = {
	...
	"the 1 battery supplier : 33Kohm",
	"the 2 battery supplier : 100Kohm",
	"the 3 battery supplier : 10Kohm",
	"the 4 battery supplier : 100ohm",
	"the 5 battery supplier : 33K,4.7uf",
	"the 6 battery supplier : 100K,2.2uf",
	"the 7 battery supplier : 33K,10uf",
	"out of the battery supplier range",
};

static const char * const new_battery_id_strings[] = {
	"unkown battery supplier",
	"the 1 battery supplier : 3.3Kohm",
	"the 2 battery supplier : 10Kohm",
	"the 3 battery supplier : 18Kohm",
	"the 4 battery supplier : 33kohm",
	"the 5 battery supplier : 56Kohm",
	"the 6 battery supplier : 120Kohm",
	"the 7 battery supplier : 390Kohm",
	"the 8 battery supplier : 33Kom+4.7uf",
	"the 9 battery supplier : 33Kom+10uf",
	"the 10 battery supplier : 120Kom+10uf",
	"the 11 battery supplier : 120Kom+2.2uf",
	"out of the battery supplier range",
};
...
*/
static int default_charge_param_judge_threshold[] = {5, 2, 3500, 4100, 500, 2100, 3000, 8100};
...
static const struct rc_item rc_items_param[] = {
	{
		.v1min	= 746,
		.v1max	= 966,
		.v2min	= 0,
		.v2max	= 129,
		.id		= B_THE_1_SUPPLIER,//33kohm
	},
	{
		.v1min	= 1117,
		.v1max	= 1398,
	...
	},
	{
		...
		.v2min	= 406,
		.v2max	= 719,
		.id		= B_THE_6_SUPPLIER,//100k,2.2uf
	},
	{
		.v1min	= 746,
		.v1max	= 966,
		.v2min	= 337,
		.v2max	= 509,
		.id 	= B_THE_7_SUPPLIER,//33k,10uf
	},
};

static const struct r_item new_r_items_param[] = {
	//3.3Kom
	{
		.v1min	= 78,
		.v1max	= 178,
...
static const struct rc_item new_rc_items_param[] = {
	//33Kom+4.7uf
	{
		.v1min	= 639,
		.v1max	= 848,
	...
	//120Kom+2.2uf
	{
		...
		.v2min	= 560,
		.v2max	= 878,
		.id		= B_THE_11_SUPPLIER,
	},
};

#endif /* __CHARGER_MONITOR_V2_H */
