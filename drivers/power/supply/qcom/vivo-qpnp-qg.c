...
static int qg_debug_mask = QG_DEBUG_PON|QG_DEBUG_PROFILE|QG_DEBUG_SOC|QG_DEBUG_IRQ|QG_DEBUG_STATUS;
module_param_named(
...
/*vivo add*/
#define BATT_ID_GET_TIME_THRE	25
#define HIGH_SOC_THRESHOLD 30
...
static int qg_get_batt_id_from_cms(struct qpnp_qg *chip);
...
static int qg_esr_estimate(struct qpnp_qg *chip)
{
	...
	if (chip->batt_psy)
		power_supply_get_property(chip->batt_psy, POWER_SUPPLY_PROP_DCHG_STATUS, &dchg_status);
	/*vivo end*/

	if (chip->dt.esr_disable || dchg_status.intval == DIRECT_CHARGER_IS_CHARGERING) /*vivo add*/
		return 0;
			       ...
			       union power_supply_propval *pval)
{
	...
	case POWER_SUPPLY_PROP_VOLTAGE_MIN:
		/*vivo modify*/
		if (chip->tbat_now < chip->custom_cutoff_temperature) {
			if (chip->custom_cutoff_volt_mv != chip->last_cutoff_volt_mv) {
				chip->last_cutoff_volt_mv = chip->custom_cutoff_volt_mv;
			}
		} else if (chip->tbat_now > chip->custom_cutoff_temperature) {
			if (chip->dt.vbatt_cutoff_mv != chip->last_cutoff_volt_mv) {
				chip->last_cutoff_volt_mv = chip->dt.vbatt_cutoff_mv;
			}
		}
	...
	case POWER_SUPPLY_PROP_BATT_FULL_CURRENT:
		//vivo add
		if(chip->remove_100_special_map){
			pval->intval = chip->report_qg_iterm * 1000;
		}else{
			/*vivo modify*/
			if (abs(chip->chg_iterm_ma) >= chip->dt.iterm_ma)
				pval->intval = (abs(chip->chg_iterm_ma) + chip->chg_iterm_delta_ma) * 1000;
			else
				pval->intval = chip->dt.iterm_ma * 1000;
			pr_err("chg_iterm_ma=%d,chg_iterm_delta_ma=%d,iterm_ma=%d\n", chip->chg_iterm_ma, chip->chg_iterm_delta_ma, chip->dt.iterm_ma);
			/*vivo end*/
		}
		break;
	...
	case POWER_SUPPLY_PROP_BATTERY_CAPACITY_MAH:
		if (chip->profile_loaded)
			...
			pr_debug("batt profile load not ready, use 4000mah\n");
			pval->intval = 4000;
		}
		break;
	case POWER_SUPPLY_PROP_BATTERY_PARAM_STATUS:
		pval->intval = chip->profile_loaded;
		break;
	case POWER_SUPPLY_PROP_CHG_EST_VBAT_DIFF:
		pval->intval = chip->chg_est_vbat_diff;
		break;
	case POWER_SUPPLY_PROP_BATTERY_ID:
		pval->intval = chip->cms_batt_id;
		break;
	case POWER_SUPPLY_PROP_AGING_SUPPORT:
		if (chip->aging_support)
			pval->intval = 1;
		else
			pval->intval = 0;
		break;
	case POWER_SUPPLY_PROP_VOLTAGE_MAX_DESIGN:
		pval->intval = chip->bp.float_volt_uv;
		break;
	case POWER_SUPPLY_PROP_QG_COUNT:
		pval->intval = chip->dischg_track_work_count;
	/*vivo end*/
	default:
...
static int qg_charge_full_update(struct qpnp_qg *chip)
{
	...
	if (chip->charge_done && !chip->charge_full) {
		if (chip->msoc >= 95 && (health == POWER_SUPPLY_HEALTH_GOOD ||
							health == POWER_SUPPLY_HEALTH_DUAL_ENGINE)) {/*vivo modify*/
...
static void qg_status_change_work(struct work_struct *work)
{
	...
	/*vivo modify*/
	rc = power_supply_get_property(chip->batt_psy,
			POWER_SUPPLY_PROP_STATUS_EX, &prop);/*vivo end*/
	if (rc < 0)
...
static int qg_load_battery_profile(struct qpnp_qg *chip)
{
	...
	/*vivo modify*/
	qg_dbg(chip, QG_DEBUG_PROFILE, "cms batt id %d \n", chip->cms_batt_id * 100);
	if (chip->dt.multi_profile_load) {
	...
	if (rc < 0) {
		pr_err("Failed to read battery capacity mah,set default 3000mah,rc:%d\n", rc);
	...
	//vivo add
	fuelsummary_of_property_put("qcom,max-voltage-uv", PARAMS_TYPE_INT, &(chip->bp.float_volt_uv));
...
static int qg_setup_battery(struct qpnp_qg *chip)
{
	...
	if (!is_battery_present(chip)) {
		...
		/*vivo modify*/
		if (!chip->fixed_batt_id) {
			chip->cms_batt_id = qg_get_batt_id_from_cms(chip);
		} else {
			chip->cms_batt_id = chip->fixed_batt_id;
		}

		if (!chip->cms_batt_id) {
			pr_err("first detect batt_id error,start check_battery_id_work.\n");
			chip->profile_loaded = false;
			cancel_delayed_work(&chip->check_battery_id_work);
			schedule_delayed_work(&chip->check_battery_id_work, 0);
		} else {/*vivo end*/
			rc = qg_load_battery_profile(chip);
			if (rc < 0) {
...
static bool is_usb_psy_available(struct qpnp_qg *chip)
{
	if (chip->usb_psy)
		return true;
...
static int qg_usb_det_pin_mode_check(struct qpnp_qg *chip, int *usb_det_pin_mode)
{
	int rc = 0;
	int usbdet_stat = 0x0;
	u8 sdam_usb_det_pin_mode = 0, temp = 0;
	union power_supply_propval psy_usb_det_pin_mode = {0, };
	static bool sys_bootup_over = false;

	return 0;

	if	(!is_usb_psy_available(chip)) {
		pr_err("usb psy isn't ready!\n");
		return 0;
	}

	if (chip->qg_monitor_work_count < 3 && !sys_bootup_over) {
		pr_err("sys_bootup not enough time!\n");
		return 0;
	}

	rc = qg_sdam_multibyte_read(QG_SDAM_USB_DET_PIN_MODE_OFFSET, &sdam_usb_det_pin_mode, 1);
	if (rc < 0) {
		pr_err("Failed to read sdam_usb_det_pin_mode rc=%d\n", rc);
	} else {
		//pr_err("before sdam_usb_det_pin_mode:%d\n", sdam_usb_det_pin_mode);

		if (!sys_bootup_over) {
			if (sdam_usb_det_pin_mode == USB_DET_PIN_MODE_INVALID) {
				pr_err("last usb_det_pin_mode is invalid,set drp mode.\n");
				psy_usb_det_pin_mode.intval = USB_DET_PIN_MODE_INVALID;
				power_supply_set_property(chip->usb_psy,
								POWER_SUPPLY_PROP_USB_DET_PIN_MODE, &psy_usb_det_pin_mode);
			}
			sys_bootup_over = true;
			*usb_det_pin_mode = sdam_usb_det_pin_mode;
		} else {
			rc = power_supply_get_property(chip->usb_psy,
				POWER_SUPPLY_PROP_USB_DET_PIN_MODE, &psy_usb_det_pin_mode);
			if (rc < 0) {
				pr_err("Failed to get usb_det_pin_mode, rc=%d\n", rc);
			} else {
				//pr_err("NOW usb_det_pin_mode:%d\n", psy_usb_det_pin_mode.intval);
				if (sdam_usb_det_pin_mode != psy_usb_det_pin_mode.intval) {
					pr_err("NOW set sdam usb_det_pin_mode:%d->%d\n", sdam_usb_det_pin_mode, psy_usb_det_pin_mode.intval);
					temp = (u8)psy_usb_det_pin_mode.intval;
					rc = qg_sdam_multibyte_write(QG_SDAM_USB_DET_PIN_MODE_OFFSET, &temp, 1);
					if (rc < 0)
						pr_err("Failed to write sdam_usb_det_pin_mode rc=%d\n", rc);

					/* collect usb_det_pin change */
					usbdet_stat = (((sdam_usb_det_pin_mode & 0xff) << 8) | (psy_usb_det_pin_mode.intval & 0xff));
					//fuelsummary_collect_value(ID_USBDET_PIN_MODE, usbdet_stat);
				}
				*usb_det_pin_mode = psy_usb_det_pin_mode.intval;
			}
		}
	}
...
static int qg_post_init(struct qpnp_qg *chip)
{
	...
	mutex_lock(&chip->check_battid_lock);/*vivo add*/
	if (!chip->profile_loaded) {
	...
	mutex_unlock(&chip->check_battid_lock);/*vivo add*/
...
static int qg_parse_dt(struct qpnp_qg *chip)
{
	...
	//fuelsummary_of_property_put("qcom,s3-entry-ibat-ua", PARAMS_TYPE_INT, &(chip->dt.s3_entry_ibat_ua));
	...
	if (rc < 0)
		...
		chip->dt.s3_exit_ibat_ua = temp;
	//fuelsummary_of_property_put("qcom,s3-exit-ibat-ua", PARAMS_TYPE_INT, &(chip->dt.s3_exit_ibat_ua));
	...
	fuelsummary_of_property_put("qcom,vbatt-empty-mv", PARAMS_TYPE_INT, &(chip->dt.vbatt_empty_mv));
	...
	fuelsummary_of_property_put("qcom,vbatt-empty-cold-mv", PARAMS_TYPE_INT, &(chip->dt.vbatt_empty_cold_mv));
	...
	fuelsummary_of_property_put("qcom,vbatt-low-mv", PARAMS_TYPE_INT, &(chip->dt.vbatt_low_mv));
	...
	chip->aging_support = of_property_read_bool(node, "vivo,battery-aging-support");

	chip->tbat_now = chip->custom_cutoff_temperature = 100;
	chip->custom_cutoff_volt_mv = chip->dt.vbatt_cutoff_mv;
	chip->last_cutoff_volt_mv = chip->dt.vbatt_cutoff_mv;
	fuelsummary_of_property_put("custom_cutoff_temperature", PARAMS_TYPE_INT, &(chip->custom_cutoff_temperature));
	fuelsummary_of_property_put("qcom,vbatt-cutoff-mv", PARAMS_TYPE_INT, &(chip->custom_cutoff_volt_mv));
	fuelsummary_of_property_put("vivo,battery-aging-support", PARAMS_TYPE_BOOL, &(chip->aging_support));
	...
	fuelsummary_of_property_put("qcom,qg-iterm-ma", PARAMS_TYPE_INT, &(chip->dt.iterm_ma));
	...
	//fuelsummary_of_property_put("qcom,hold-soc-while-full", PARAMS_TYPE_BOOL, &(chip->dt.hold_soc_while_full));

	chip->dt.linearize_soc = of_property_read_bool(node,
					"qcom,linearize-soc");
	//fuelsummary_of_property_put("qcom,linearize-soc", PARAMS_TYPE_BOOL, &(chip->dt.linearize_soc));
	...
	//fuelsummary_of_property_put("qcom,shutdown_soc_threshold", PARAMS_TYPE_INT, &(chip->dt.shutdown_soc_threshold));
	...
	chip->dt.qg_hi_current_cfg = of_property_read_bool(node,
	...
	if (!chip->dt.cl_disable) {
		chip->dt.cl_feedback_on = of_property_read_bool(node,
						"qcom,cl-feedback-on");
		//fuelsummary_of_property_put("qcom,cl-feedback-on", PARAMS_TYPE_BOOL, &(chip->dt.cl_feedback_on));
		...
		//fuelsummary_of_property_put("qcom,cl-min-start-soc", PARAMS_TYPE_INT, &(chip->dt.min_start_soc));
		...
		//fuelsummary_of_property_put("qcom,cl-max-start-soc", PARAMS_TYPE_INT, &(chip->dt.max_start_soc));
		...
		//fuelsummary_of_property_put("qcom,cl-max-increment", PARAMS_TYPE_INT, &(chip->dt.max_cap_inc));
		...
		//fuelsummary_of_property_put("qcom,cl-max-decrement", PARAMS_TYPE_INT, &(chip->dt.max_cap_dec));
	...
	//vivo add
	rc = of_property_read_u32(node, "vivo,fixed-batt-id", &chip->fixed_batt_id);
	if (rc < 0) {
		chip->fixed_batt_id = 0;
	}

	chip->remove_100_special_map = of_property_read_bool(node, "vivo,remove-100-special-map");
	pr_err("chip->remove_100_special_map=%d\n", chip->remove_100_special_map);
	if(chip->remove_100_special_map){
		chip->report_qg_iterm = chip->dt.iterm_ma;
	}
...
static int qpnp_qg_suspend(struct device *dev)
{
	...
	/*vivo add for AT-MODE*/
	if (is_atboot) {
		vote(chip->fifo_irq_disable_votable,
				PROFILE_IRQ_DISABLE, true, 0);
	}
	/* add for qg count */
	cancel_delayed_work_sync(&chip->dischg_track_work);
	/*vivo end*/
...
static int qpnp_qg_resume(struct device *dev)
{
	...
	if (is_atboot) {
		vote(chip->fifo_irq_disable_votable,
				PROFILE_IRQ_DISABLE, false, 0);
	}
	/* add for qg count */
	schedule_delayed_work(&chip->dischg_track_work,
...
/*vivo add functiion*/
static bool qg_check_is_chg_est_vbat_diff(struct qpnp_qg *chip, int ocv, int soc)
{
	bool est_vbat_diff = false;
	int	est_soc;
	int ocv_mv;
	int delta_ocv;
	int i;
	...
	if (ocv_mv <= soc_volatge[0][0]) {
		est_soc = 0;
	} else if (ocv_mv > soc_volatge[10][0]) {
		est_soc = 100;
	} else {
		for (i = 0; i < 11; i++) {
			if (ocv_mv <= soc_volatge[i][0]) {
				break;
			}
		}
		delta_ocv = (soc_volatge[i][0] - soc_volatge[i-1][0])/10;
		//pr_warn("i= %d, soc_volatge[i-1][0]= %d, soc start= %d, delta_ocv= %d\n", i, soc_volatge[i-1][0], soc_volatge[i-1][1],delta_ocv);

		for (j = 0; j < 10; j++) {
			if (ocv_mv <= (soc_volatge[i-1][0] + (delta_ocv * j))) {
				break;
			}
		}
		est_soc = soc_volatge[i-1][1] + j;
	}

	if (soc <= 50) {
		if (abs(soc - est_soc) > LOW_SOC_THRESHOLD) {
			est_vbat_diff = true;
		}
	} else {
		if (abs(soc - est_soc) > HIGH_SOC_THRESHOLD) {
			est_vbat_diff = true;
		}
	}

	if (est_vbat_diff)
		pr_warn("ocv_mv= %d, soc= %d, est_soc= %d est_vbat_diff= %d\n", ocv_mv, soc, est_soc, est_vbat_diff);
	return est_vbat_diff;
}

static bool is_cms_available(struct qpnp_qg *chip)
{
	if (!chip->cms_psy)
...
static int qg_get_batt_id_from_cms(struct qpnp_qg *chip)
{
	/*union power_supply_propval batt_id_done = {0,};*/
	union power_supply_propval batt_id = {0,};
	int rc, real_batt_id = 0;

	if (!is_cms_available(chip)) {
		pr_err("cms psy is null,return\n");
		real_batt_id = 0;
	} else {
		/*rc = power_supply_get_property(chip->cms_psy, POWER_SUPPLY_PROP_BATT_ID_DETECT_DONE, &batt_id_done);
		if (rc < 0 || !batt_id_done.intval) {
			pr_err("Failed to get batt id detect done,rc=%d,done=%d\n", rc, batt_id_done.intval);
			real_batt_id = 0;
			goto exit;
		}*/

		rc = power_supply_get_property(chip->cms_psy, POWER_SUPPLY_PROP_BATTERY_ID, &batt_id);
		if (rc < 0) {
		...
		if (batt_id.intval <= B_UNKOWN || batt_id.intval >= B_MAX) {
			real_batt_id = B_MAX;/*use default battery param*/
		} else {
			real_batt_id = batt_id.intval;
		}
	}

exit:
	pr_warn("get bat_id is %d\n", real_batt_id);
	return real_batt_id;
}

#define CHECK_BATTERY_ID_COUNT_MAX	8
#define CHECK_BATTERY_ID_PERIOD_MS	500
static void check_battery_id_work(struct work_struct *work)
{
	struct qpnp_qg *chip = container_of(work, struct qpnp_qg,
					    check_battery_id_work.work);

	int rc, delay_time = CHECK_BATTERY_ID_PERIOD_MS;
	int batt_id, nom_cap_uah;

	if (chip->profile_loaded || chip->check_battid_count >= CHECK_BATTERY_ID_COUNT_MAX) {
		qg_dbg(chip, QG_DEBUG_PROFILE, "check work is done!\n");
		chip->soc_reporting_ready = true;
		chip->check_battid_count = 0;
		return;
	}

	batt_id = qg_get_batt_id_from_cms(chip);
	qg_dbg(chip, QG_DEBUG_PROFILE, "profile_loaded:%d,batt_id:%d,check_battid_count:%d\n",
				chip->profile_loaded, batt_id, chip->check_battid_count);

	if (batt_id) {
		chip->cms_batt_id = batt_id;
		rc = qg_load_battery_profile(chip);
		if (rc < 0) {
			pr_err("Failed to load battery-profile rc=%d\n", rc);
			chip->profile_loaded = false;
		...
		if (chip->profile_loaded) {
			...
			cancel_delayed_work(&chip->ttf->ttf_work);
		...
		mutex_lock(&chip->check_battid_lock);
		if (chip->profile_loaded) {
			vote(chip->vbatt_irq_disable_votable,
					PROFILE_IRQ_DISABLE, false, 0);
			vote(chip->fifo_irq_disable_votable,
		...
		mutex_unlock(&chip->check_battid_lock);

		chip->check_battid_count = 0;

		qg_dbg(chip, QG_DEBUG_PROFILE, "finally,battery_missing=%d cms_batt_id=%d profile_loaded=%d profile=%s soc_reporting_ready=%d\n",
				chip->battery_missing, chip->cms_batt_id,
				chip->profile_loaded, chip->bp.batt_type_str, chip->soc_reporting_ready);
		return;
	}

	chip->check_battid_count++;
	schedule_delayed_work(&chip->check_battery_id_work,
		msecs_to_jiffies(delay_time));
}

static int qg_check_fifo_irq_status(struct qpnp_qg *chip)
{
	static int last_status = 0;
	int rc = 0, now_status = 0;

	/*get irq status*/
	rc = qg_read(chip, chip->qg_base + 0x18,
				(u8 *)&now_status, 1);
	if (rc < 0) {
		pr_err("Failed to read 0x18 reg, rc=%d\n", rc);
		return rc;
	}
	/*get fifo int bit(3) only*/
	now_status &= 0x8;

	pr_err("last_status=%d,now_status=%d\n", last_status, now_status);

	if (now_status == last_status && last_status) {
		mutex_lock(&chip->fifo_irq_lock);
		pr_err("process fifo int status bit...\n");
		rc = qg_masked_write(chip, chip->qg_base + 0x14, 0x8, 0x8);
		if (rc < 0) {
			pr_err("Failed to write 0x14 reg, rc=%d\n", rc);
			mutex_unlock(&chip->fifo_irq_lock);
			return rc;
		}
		mutex_unlock(&chip->fifo_irq_lock);
	}
...
static void qg_monitor_work(struct work_struct *work)
{
	struct qpnp_qg *chip = container_of(work, struct qpnp_qg,
					    qg_monitor_work.work);
	int rc = 0, soc_now, vbatt_now, ibatt_now, batt_temp_now, ocv_now, usb_det_pin_mode = 0;
	union power_supply_propval ui_soc = {-1, };
	union power_supply_propval chg_term_cur = {0, };
	static int pre_soc = -1;
	int cc_uah = 0, soh = 100;
	int64_t learned_cap = 0, nominal_cap = 0;
	union power_supply_propval scene = {SCENE_10STHD,};
	union power_supply_propval vcutoff = {0,};

	rc = qg_get_battery_capacity(chip, &soc_now);
	if (rc < 0) {
		pr_err("get soc error, return default soc 0!\n");
		soc_now = 0;
	}

	rc = qg_get_battery_voltage(chip, &vbatt_now);
	if (rc < 0) {
		pr_err("get vbatt error, return default vbatt 0v!\n");
		vbatt_now = 0;
	}

	rc = qg_sdam_read(SDAM_OCV_UV, &ocv_now);
	if (rc < 0) {
		pr_err("get ocv error, return default ocv -EINVAL!\n");
		ocv_now = -EINVAL;
	}

	rc = qg_get_battery_current(chip, &ibatt_now);
	if (rc < 0) {
		pr_err("get ibatt error, return default ibatt 0!\n");
		ibatt_now = 0;
	}

	rc = qg_get_battery_temp(chip, &batt_temp_now);
	if (rc < 0) {
		pr_err("get batt temp error, return default batt temp 0!\n");
		batt_temp_now = 0;
	} else {
		if (batt_temp_now < chip->custom_cutoff_temperature) {
			if (chip->custom_cutoff_volt_mv != chip->last_cutoff_volt_mv) {
				chip->last_cutoff_volt_mv = chip->custom_cutoff_volt_mv;
			}
		} else if (batt_temp_now > chip->custom_cutoff_temperature) {
			if (chip->dt.vbatt_cutoff_mv != chip->last_cutoff_volt_mv) {
				chip->last_cutoff_volt_mv = chip->dt.vbatt_cutoff_mv;
			}
		}
		if (is_batt_available(chip)) {
			pr_warn("temperature:[%d,%d] cutoff_volt_mv:%d\n",
					batt_temp_now,
					chip->custom_cutoff_temperature,
					chip->last_cutoff_volt_mv);
			vcutoff.intval = chip->last_cutoff_volt_mv * 1000;
			power_supply_set_property(chip->batt_psy,
					POWER_SUPPLY_PROP_VOLTAGE_MIN, &vcutoff);
		}
	}

	if (chip->profile_loaded && batt_temp_now >= 100 && soc_now != -EINVAL && \
			ocv_now != -EINVAL && batt_temp_now != -EINVAL) {
		chip->chg_est_vbat_diff = qg_check_is_chg_est_vbat_diff(chip, ocv_now, soc_now);
	} else {
		pr_warn("chg_est_vbat_diff status %d, profile_loaded %d or temp too low, set to 2\n", \
			chip->chg_est_vbat_diff, chip->profile_loaded);
		chip->chg_est_vbat_diff = 2;
	}

	rc = qg_check_fifo_irq_status(chip);
	if (rc < 0) {
		pr_err("check fifo irq status error!\n");
	}

	/*update charge term current for ffc function*/
	if (is_batt_available(chip)) {
		rc = power_supply_get_property(chip->batt_psy,
				POWER_SUPPLY_PROP_CHARGE_TERM_CURRENT, &chg_term_cur);
		if (rc < 0) {
			pr_err("Failed to get charge term current, rc=%d\n", rc);
		} else {
			chip->chg_iterm_ma = chg_term_cur.intval;
		}
		rc = power_supply_get_property(chip->batt_psy,
				POWER_SUPPLY_PROP_CHARGE_TERM_CURRENT_DELTA, &chg_term_cur);
		if (rc < 0) {
			pr_err("Failed to get charge term current, rc=%d\n", rc);
		} else {
			chip->chg_iterm_delta_ma = chg_term_cur.intval;
		}
	}

	/*usb det pin mode check*/
	qg_usb_det_pin_mode_check(chip, &usb_det_pin_mode);

	if (soc_now == 0 && is_batt_available(chip)) {
		power_supply_get_property(chip->batt_psy,
				POWER_SUPPLY_PROP_CAPACITY, &ui_soc);
	}

	if (((pre_soc != soc_now && chip->charge_status == POWER_SUPPLY_STATUS_DISCHARGING) ||
					ui_soc.intval == 0) && is_batt_available(chip)) {
		pre_soc = soc_now;
	...
	/*qg learned capacity*/
	if (!chip->dt.cl_disable && chip->dt.cl_feedback_on)
		qg_get_learned_capacity(chip, &learned_cap);
	qg_get_nominal_capacity((int *)&nominal_cap, 250, true);
	pr_err("battery_learning_cloud_config%d\n", chip->battery_learning_cloud_config);
	/*reset after 3600*10/60 mins=10h*/
	if (chip->qg_monitor_work_count++ >= 3600)
		chip->qg_monitor_work_count = 0;

	pr_err("charge_status=%d,soc=%d,vbatt=%d,ibatt=%d,batt_temp=%d,ocv=%d,battid=%d,ui_soc=%d,learned_cap=%d,nominal_cap=%d,usb_det_pin_mode=%d\n",
					chip->charge_status, soc_now, vbatt_now, ibatt_now, batt_temp_now, ocv_now, chip->cms_batt_id, ui_soc.intval,
					learned_cap/1000, nominal_cap/1000, usb_det_pin_mode);

	chip->tbat_now = batt_temp_now;
	soh = (learned_cap / 10 / chip->batt_capacity_mah);
	if (soh > 100)
		soh = 100;

	qg_get_charge_counter(chip, &cc_uah);
	fuelsummary_collect_value(ID_COUL, (cc_uah / 1000));
	fuelsummary_collect_value(ID_FCC, (learned_cap / 1000));
	fuelsummary_collect_value(ID_SOH, soh);
	fuelsummary_collect_value(ID_AGINE_COUNT, chip->cl->cap_learning_done_count);
	...
	schedule_delayed_work(&chip->qg_monitor_work,
			msecs_to_jiffies(10000));
}

static void qg_dischg_track_work(struct work_struct *work)
{
	struct qpnp_qg *chip = container_of(work, struct qpnp_qg,
						dischg_track_work.work);

	/*reset after 3600*10/60 mins=10h*/
	if (chip->dischg_track_work_count++ >= 3600)
		chip->dischg_track_work_count = 0;

	qg_dbg(chip, QG_DEBUG_SOC, "discharging tracking count = %d\n",
			chip->dischg_track_work_count);
...
static int qpnp_qg_probe(struct platform_device *pdev)
{
	...
	/*vivo add*/
	INIT_DELAYED_WORK(&chip->qg_monitor_work, qg_monitor_work);
	INIT_DELAYED_WORK(&chip->dischg_track_work, qg_dischg_track_work);
	INIT_DELAYED_WORK(&chip->check_battery_id_work, check_battery_id_work);
	mutex_init(&chip->check_battid_lock);
	mutex_init(&chip->fifo_irq_lock);
	mutex_init(&chip->dp_batt_temp_lock);
	chip->chg_est_vbat_diff = 2;
	chip->chg_iterm_delta_ma = 0;
	chip->dischg_track_work_count = 0;
	chip->battery_learning_cloud_config = true;
	fuelsummary_of_property_put("battery_learning_config", PARAMS_TYPE_BOOL, &(chip->battery_learning_cloud_config));
	/*vivo end*/
	...
	/*vivo add*/
	schedule_delayed_work(&chip->qg_monitor_work, 0);
	schedule_delayed_work(&chip->dischg_track_work, 0);
	/*vivo end*/
...
static int qpnp_qg_remove(struct platform_device *pdev)
{
	...
	/*vivo add*/
	mutex_destroy(&chip->check_battid_lock);
	mutex_destroy(&chip->fifo_irq_lock);
	mutex_destroy(&chip->dp_batt_temp_lock);
	/*vivo end*/
...
static void qpnp_qg_shutdown(struct platform_device *pdev)
{
	struct qpnp_qg *chip = platform_get_drvdata(pdev);
	bool input_present = is_input_present(chip);
	int usb_det_pin_mode;/*vivo add*/

	/*vivo add: usb det pin mode check*/
	pr_info("\n");
	qg_usb_det_pin_mode_check(chip, &usb_det_pin_mode);
	/*vivo end*/
