				...
				int *slave_icl_ua, int *total_settled_icl_ua)
{
	...
	union power_supply_propval pval = {0, };
	/*vivo add*/
	union power_supply_propval primary_fastchg_ua = {0, };
	union power_supply_propval parallel_fastchg_ua = {0, };
	int rc, total_settled_ua = 0;
	int total_temp_ua = 0, slave_temp_ua = 0;/*vivo add*/
	...
	/* vivo add */
	if (!get_effective_result_locked(chip->pl_disable_votable)) {
		if (chip->pl_settled_ua) {
			total_temp_ua = total_settled_ua;
		} else {
			total_temp_ua = total_current_ua;
		}
		slave_temp_ua = total_temp_ua * chip->slave_current_scale / 100;
		pl_dbg(chip, PR_PARALLEL, "slave_temp_ua=%d,slave_current_scale=%d\n",
									slave_temp_ua, chip->slave_current_scale);
		if (chip->batt_psy) {
			rc = power_supply_get_property(chip->batt_psy,
						POWER_SUPPLY_PROP_PRIMARY_FASTCHG_ALLOC,
						&primary_fastchg_ua);
			if (rc < 0)
				primary_fastchg_ua.intval = -1;
			rc = power_supply_get_property(chip->batt_psy,
						POWER_SUPPLY_PROP_PARALLEL_FASTCHG_ALLOC,
						&parallel_fastchg_ua);
			if (rc < 0)
					parallel_fastchg_ua.intval = -1;
		}
		if(parallel_charger_type == 0x02){
			//bq25601d
			if ((slave_temp_ua) >= 1000000 && parallel_fastchg_ua.intval > 1900000) {
				slave_ua = 1100000;
			} else if((slave_temp_ua) >= 1000000 && parallel_fastchg_ua.intval > 1750000) {
				slave_ua = 1000000;
			} else if ((slave_temp_ua) >= 700000) {
				slave_ua = 900000;
			} else if (slave_temp_ua >= 325000) {
				slave_ua = 500000;
			} else {
				slave_ua = 0;
			}
		}else{
			if ((slave_temp_ua) >= 700000) {
					slave_ua = 900000;
			} else if (slave_temp_ua >= 325000) {
					slave_ua = 500000;
			} else {
		...
		//total_current_ua = total_temp_ua;
	}
	/* vivo end */
	...
	*slave_ua = min(*slave_ua, chip->pl_fcc_max);
	...
	if (chip->pl_mode != POWER_SUPPLY_PL_NONE) {
		if (chip->batt_psy) {
			...
			if (primary_fastchg_ua.intval >= 0 && parallel_fastchg_ua.intval >= 0) {
				if (total_ua >= (primary_fastchg_ua.intval + parallel_fastchg_ua.intval)) {
					*master_ua = primary_fastchg_ua.intval;
					*slave_ua = parallel_fastchg_ua.intval;
					pl_dbg(chip, PR_PARALLEL, "get fcc spilt current change(%d, %d)\n", *master_ua, *slave_ua);
				}
			}
		}
	}

	if (*slave_ua > 0 && *slave_ua < 512000) {
		*master_ua = *master_ua + *slave_ua - 512000;
		*slave_ua = 512000;
	}

	chip->slave_current_scale = (*slave_ua * 100) / (*master_ua + *slave_ua);
	/*vivo end*/
}
...
static void pl_taper_work(struct work_struct *work)
{
	...
	while (true) {
		if (chip->taper_count > 3) {
			pl_dbg(chip, PR_PARALLEL, "taper: parallel disabled\n");
		...
		if (rc < 0) {
			pr_err("taper: Couldn't get batt charge type rc=%d\n", rc);
			goto done;
		}
		...
		if (pval.intval == POWER_SUPPLY_CHARGE_TYPE_TAPER) {
			chip->taper_count++;
			pl_dbg(chip, PR_PARALLEL, "taper: count %d taper.\n", chip->taper_count);
		} else {
			pl_dbg(chip, PR_PARALLEL, "taper: fast charging; waiting for next taper\n");
		}

		/* wait for the charger state */
		...
		if (chip->usb_psy) {
			...
			if (rc < 0) {
				pr_err("taper: Couldn't get present from USB rc=%d\n", rc);
				goto done;
			}
		}
		if (!usb_present.intval) {
			pl_dbg(chip, PR_PARALLEL, "taper: usb plugout,exit pl_taper_work.\n");
			goto done;
		}
	}
	/*vivo end*/
...
#define USB_WEAK_INPUT_UA	1400000	//vivo add
			...
			int icl_ua, const char *client)
{
	 ...
	 */
	if (icl_ua <= USB_WEAK_INPUT_UA)	//vivo modify
	...
	if (chip->batt_psy) {
		rc = power_supply_get_property(chip->batt_psy,
			       ...
			       &pval);
		if (rc < 0) {
			pr_err("Couldn't get detect icl ibat current rc=%d\n", rc);
		} else {
			pr_err("### get detect icl ibat current=%d\n", pval.intval);
			//rc = power_supply_set_property(chip->main_psy,
					//POWER_SUPPLY_PROP_CONSTANT_CHARGE_CURRENT_MAX,
					//&pval);
			//if (rc < 0) {
		...
		void *data, int pl_disable, const char *client)
{
	...
	if (chip->pl_mode != POWER_SUPPLY_PL_NONE && !pl_disable) {
		...
		if (!chip->fcc_stepper_enable) {
			...
			/*vivo modify*/
			rc = power_supply_get_property(chip->batt_psy,
						   POWER_SUPPLY_PROP_CHARGE_MODE, &pval);
			if ((rc < 0) || !(pval.intval)) {
				vote(chip->fcc_main_votable, MAIN_FCC_VOTER, true,
									(total_fcc_ua > 2000000 ? 2000000 : total_fcc_ua));
				pr_err("main fcc limit = 2000000\n");
			} else if (pval.intval) {
				vote(chip->fcc_main_votable, MAIN_FCC_VOTER, true,
									(total_fcc_ua > 2300000 ? 2300000 : total_fcc_ua));
				pr_err("main fcc limit = 2300000\n");
			}
			/*vivo end*/
...
static void handle_main_charge_type(struct pl_data *chip)
{
	...
	if (pval.intval == POWER_SUPPLY_CHARGE_TYPE_TAPER
		...
		if (pval.intval == POWER_SUPPLY_CHARGE_TYPE_FAST &&
			/*(chip->taper_entry_fv <
			get_effective_result(chip->fv_votable))*/!chip->taper_work_running) {
			if (chip->taper_count <= 0) {
				pl_dbg(chip, PR_PARALLEL, "TAPER_END_VOTER enabling parallel\n");
				vote(chip->pl_disable_votable, TAPER_END_VOTER,
					false, 0);
			} else {
				pl_dbg(chip, PR_PARALLEL, "before TAPER_END_VOTER enabling parallel,taper_count %d.\n", chip->taper_count);
				chip->taper_count -= 2;
			}
		/*vivo end*/
		}
...
static void handle_settled_icl_change(struct pl_data *chip)
{
	...
	if ((main_limited && (main_settled_ua + chip->pl_settled_ua) < USB_WEAK_INPUT_UA)
			|| (main_settled_ua == 0)
			|| ((total_current_ua >= 0) &&
				(total_current_ua <= USB_WEAK_INPUT_UA)))
		vote(chip->pl_enable_votable_indirect, USBIN_I_VOTER, false, 0);
	else
