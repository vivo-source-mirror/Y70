...
int force_5v_hvdcp(struct smb_charger *chg);
int force_9v_hvdcp(struct smb_charger *chg);
					...
					struct iio_channel **chan)
{
	...
	if (rc < 0) {/*vivo add*/
		smblib_err(chg, "%s channel not match string, %d\n",
							propname, rc);
		return 0;
	}
					...
					int sec_chg)
{
	...
	case POWER_SUPPLY_CHARGER_SEC_PL:
		...
		rc = smblib_masked_write(chg, MISC_SMB_CFG_REG,
						/*vivo modify*/
						/*SMB_EN_SEL_BIT, 0);*/
						SMB_EN_SEL_BIT | STAT_POLARITY_CFG_BIT, STAT_POLARITY_CFG_BIT);
		if (rc < 0) {
			dev_err(chg->dev, "Couldn't select SMB charger rc=%d\n",
				rc);
			return rc;
	...
	if (enable) {
		...
		if (chg->connector_type == POWER_SUPPLY_CONNECTOR_TYPEC) {
			rc = smblib_masked_write(chg, USB_CMD_PULLDOWN_REG,
					EN_PULLDOWN_USB_IN_BIT, 0);
			smblib_err(chg, "set pulldown 0\n");
			if (rc < 0) {
				smblib_err(chg, "Couldn't write %s to EN_PULLDOWN_USB_IN_BIT rc=%d\n", "disable", rc);
			}
		}

		schedule_delayed_work(&chg->otg_monitor_work, msecs_to_jiffies(1000));
		/*vivo end*/
	} else {
		...
		cancel_delayed_work(&chg->otg_monitor_work);
		chg->drp_timer_count = 15;
		chg->drp_ongoing = false;

		if (chg->connector_type == POWER_SUPPLY_CONNECTOR_TYPEC) {
			rc = smblib_masked_write(chg, USB_CMD_PULLDOWN_REG,
					EN_PULLDOWN_USB_IN_BIT, EN_PULLDOWN_USB_IN_BIT);
			smblib_err(chg, "set pulldown %d\n", EN_PULLDOWN_USB_IN_BIT);
			if (rc < 0) {
				smblib_err(chg, "Couldn't write %s to EN_PULLDOWN_USB_IN_BIT rc=%d\n", "enable", rc);
			}
		}
...
static const struct apsd_result *smblib_get_apsd_result(struct smb_charger *chg)
{
	...
	if (result->pst == POWER_SUPPLY_TYPE_USB) {
		...
		chg->bigdata_chg_type = POWER_SUPPLY_TYPE_USB_FLOAT;
...
int smblib_set_usb_suspend(struct smb_charger *chg, bool suspend)
{
	...
	/* vivo add by xusifan, for solving unable to turn on flashlight 
	* when usb is suspend(caused by keep_chg_soc)
	*/
	if (chg->chg_param.smb_version == PMI632_SUBTYPE) {
		schgm_flash_torch_priority(chg, suspend ? TORCH_BOOST_MODE : TORCH_BUCK_MODE);
		smblib_err(chg, "schgm_flash_torch_priority suspend = %d\n", suspend);
	}
...
static int smblib_set_adapter_allowance(struct smb_charger *chg, u8 allowed_voltage)
{
	...
	smblib_dbg(chg, PR_MISC, "set USBIN_ADAPTER_ALLOW_CFG_REG: %d\n",
			allowed_voltage);
	return rc;
}
				...
				union power_supply_propval *val)
{
	...
	/*vivo add*/
	if (psp == POWER_SUPPLY_PROP_TEMP && chg->batt_temp_comp_enable && chg->usb_present) {
		smblib_get_prop_batt_status(chg, &chg_status);
		/*smblib_dbg(chg, PR_MISC, "chg_status=%d,hvdcp_type=%d,temp=%d\n",
							chg_status.intval, hvdcp_type, val->intval);*/
		if (chg_status.intval == POWER_SUPPLY_STATUS_CHARGING) {
			if (chg->chg_voltage_v >= HVDCP_TYPE_9V) {
				if (is_panel_backlight_on() && val->intval >= 50)
					val->intval -= (2 * 10);
				else if (!is_panel_backlight_on() && val->intval >= 150 && val->intval <= 450)
					val->intval -= (3 * 10);
			} else if (val->intval >= 50) {
				val->intval -= (2 * 10);
			}
		}
	}
...
static const struct apsd_result *smblib_update_usb_type(struct smb_charger *chg)
{
	...
	/* vivo add */
	strcpy(chg->usb_type_name,apsd_result->name);
...
static void smblib_uusb_removal(struct smb_charger *chg)
{
	...
	/*vote(chg->pl_disable_votable, PL_DELAY_VOTER, true, 0);*//*vivo delete*/
	...
	rc = smblib_set_adapter_allowance(chg, USBIN_ADAPTER_ALLOW_5V_TO_9V);//vivo modify
	if (rc < 0){
	...
	if (chg->qc2_unsupported_voltage) {
		...
		if (!chg->disable_suspend_on_collapse) {/*vivo add condition*/
...
int smblib_rerun_apsd_if_required(struct smb_charger *chg)
{
	...
	if (!val.intval)
		return 0;
	/* vivo add disable and re-enable DP/DM */
...
int smblib_set_icl_current(struct smb_charger *chg, int icl_ua)
{
	...
	if (!(chg->drp_ongoing) && chg->connector_type == POWER_SUPPLY_CONNECTOR_TYPEC) {
		rc = smblib_masked_write(chg, USB_CMD_PULLDOWN_REG,
				EN_PULLDOWN_USB_IN_BIT,
				  ...
				  union power_supply_propval *val)
{
	...
	if (!rc) {
		if (val->intval == 0 && !report_flag && batt_volt.intval >= chg->custom_cutoff_volt_mv) {
			val->intval = 1;
			smblib_err_ratelimited(chg, "soc=0,batt volt=%dmV(>3.4v),report soc=1!\n", batt_volt.intval/1000);
		} else if (val->intval == 0 && batt_volt.intval < chg->custom_cutoff_volt_mv) {
			report_flag = true;
		...
		if(enable_slave_charger && !is_atboot && !power_off_charging_mode && !vivo_ex_fg_i2c_is_exception(chg) &&
			((ex_fg_support && ex_fg_soc == val->intval) || !ex_fg_support) && fg_soc_really_temp) {
				...
				union power_supply_propval *val)
{
	...
	case INHIBIT_CHARGE:
		...
		chg->vbat_in_iterm = vbat.intval / 1000;	//mV
		break;
	case DISABLE_CHARGE:
	case PAUSE_CHARGE:
	...
	/*vivo modify*/
	if (!stat) {
		if (charge_stat == PAUSE_CHARGE) {
			val->intval = POWER_SUPPLY_STATUS_CHARGING;
		} else if (charge_stat == DISABLE_CHARGE && chg->usb_temporary_present) {
			val->intval = POWER_SUPPLY_STATUS_CHARGING;
		} else {
			val->intval = POWER_SUPPLY_STATUS_NOT_CHARGING;
		}
...
static void smblib_hvdcp_set_fsw(struct smb_charger *chg, int bit)
{
	...
	/*vivo add*/
	smblib_dbg(chg, PR_INTERRUPT, "hvdcp adaptive voltage change bit = %d \n", bit);
}
					...
					int usb_current)
{
	...
	smblib_dbg(chg, PR_MISC, "usb_current=%d\n", usb_current);
				     ...
				     const union power_supply_propval *val)
{
	...
	/*vivo add*/
	smblib_dbg(chg, PR_MISC, "power role set val=%d,drp_backup_state=%d\n", val->intval, chg->drp_backup_state);
				...
				const union power_supply_propval *val)
{
	...
	if (chg->pd_active) {
		 ...
		 * pd_current_max
		 */
		/*vote(chg->usb_icl_votable, PD_VOTER, true, USBIN_100MA);*//*vivo delete*/
...
//vivo add 
//smblib_batch_write sometimes canot write success
/*
...
*/
		temp = thresholds[1] & 0xFFFF;
		//temp = ((temp & 0xFF00) >> 8) | ((temp & 0xFF) << 8);
		rc = smblib_masked_write(chg, base,
				0xff, (u8)((temp & 0xFF00) >> 8));
		if (rc < 0)
			pr_err("Couldn't configure jeita %d\n", base);

		rc = smblib_masked_write(chg, base + 1,
				0xff, (u8)(temp & 0xFF));
		if (rc < 0)
			pr_err("Couldn't configure jeita %d\n", base + 1);
		...
		temp = thresholds[0] & 0xFFFF;
		//temp = ((temp & 0xFF00) >> 8) | ((temp & 0xFF) << 8);
		rc = smblib_masked_write(chg, base + 2,
				0xff, (u8)((temp & 0xFF00) >> 8));
		if (rc < 0)
			pr_err("Couldn't configure jeita %d\n", base + 2);

		rc = smblib_masked_write(chg, base + 3,
				0xff, (u8)(temp & 0xFF));
		if (rc < 0)
			pr_err("Couldn't configure jeita %d\n", base + 3);
		//smblib_dbg(chg, PR_MISC,"write jeita\n");
		smblib_dbg(chg, PR_MISC, "%s Jeita threshold configured\n",
				(type == JEITA_SOFT) ? "Soft" : "Hard");
//vivo add end
	return 0;
...
void smblib_usb_plugin_locked(struct smb_charger *chg)
{
	...
	if (vbus_rising) {
		...
		if(ex_fg_ffc_support) {
			vivo_set_chg_term_voltage(chg, chg->batt_profile_fv_uv);
			vivo_set_chg_term_current(chg, chg->term_current_thresh_hi_ma);
		...
		chg->factory_10W_charge_test_enabe = false;
		...
		if(is_parallel_charger_available(chg)){
			...
			if (chg->pl_enable_gpio > 0)
				gpio_direction_output(chg->pl_enable_gpio, 0);
		}
		...
		schedule_delayed_work(&chg->charge_monitor_work, msecs_to_jiffies(6000));

		if (chg->dchg_disable_votable)
			vote(chg->dchg_disable_votable, KEEP_CHG_SOC, false, 0);

		/* add for solving when fast-plugin source_change_irq delays 2min */
		smblib_dbg(chg, PR_MISC, "force rerun_apsd when vbus rising\n");
		smblib_rerun_apsd(chg);
		/*vivo end*/
	} else {
		...
		vote(chg->usb_icl_votable, USER_DEFAULT_INPUT_VOTER, true, SDP_100_MA);
		...
		if(is_parallel_charger_available(chg)){
			...
			power_supply_set_property(chg->pl.psy,
				POWER_SUPPLY_PROP_PRESENT, &pval);
			if (chg->pl_enable_gpio > 0)
				gpio_direction_output(chg->pl_enable_gpio, 1);
		}
		...
		chg->cc_force_5v_flag = CCF5_FLAG_UNKNOWN;
		chg->cc_force_5v_retries = 0;
		chg->cc_force_5v_usb_conn_protect = 0;
		...
		chg->cable_with_cc_float_otg = false;
		...
		/* when cable disconnect in LCO off state, PM suspend quickly and
		    cause the LCD can not power on normal,add 2s delay wake lock
		    for Android LCD control*/
	...
	/* vivo add */
	schedule_delayed_work(&chg->typec_state_verification_work,
...
static void update_sw_icl_max(struct smb_charger *chg, int pst)
{
	...
	/*vivo add*/
	/*micro usb return, we set icl at usb source change hander*/
	if (chg->connector_type == POWER_SUPPLY_CONNECTOR_MICRO_USB) {
		return;
	}
	/*vivo end*/
...
irqreturn_t usb_source_change_irq_handler(int irq, void *data)
{
	...
	if (chg->pd_active) {/*vivo modify*/
		smblib_dbg(chg, PR_REGISTER, "PD charger set input current max: maybe 2a.\n");
		vote(chg->usb_icl_votable, USER_DEFAULT_INPUT_VOTER, true,
	...
	if (is_present) {
		...
		if (chg->real_charger_type == POWER_SUPPLY_TYPE_USB) {
			...
			if (chg->real_charger_type == POWER_SUPPLY_TYPE_USB_HVDCP ||
				...
				if (chg->detect_icl_ibat_ua == SMBCHG_DETECT_ICL_IBAT_UA) {
					...
					vote(chg->usb_icl_votable, USER_CTRL_PL_VOTER, true, 1450000);
					smblib_dbg(chg, PR_MISC, "first plugin to hvdcp,set input:1.45A.\n");
				}
				force_9v_hvdcp(chg);
		...
		smblib_dbg(chg, PR_MISC, "Type %d: setting uA = %d\n",
			chg->real_charger_type, current_limit_ua);
...
static void typec_src_insertion(struct smb_charger *chg)
{
	...
	smblib_dbg(chg, PR_REGISTER, "LEGACY_CABLE_STATUS_REG = 0x%02x\n", stat);
	...
	smblib_dbg(chg, PR_REGISTER, "typec_legacy=%d,ok_to_pd=%d\n", chg->typec_legacy, chg->ok_to_pd);
...
static void typec_src_removal(struct smb_charger *chg)
{
	...
	typec_src_fault_condition_cfg(chg, false);
	if (!chg->direct_charger_enable)
		smblib_hvdcp_detect_enable(chg, true);/*vivo modify*/
	smblib_update_usb_type(chg);
...
irqreturn_t typec_attach_detach_irq_handler(int irq, void *data)
{
	...
	if (stat & TYPEC_ATTACH_DETACH_STATE_BIT) {
		...
		chg->typec_state_normal = WORK_STATUS_ONGOING;
		smblib_dbg(chg, PR_MISC, "typec state=%d\n", chg->typec_state_normal);
		/*vivo end */
	} else {
		...
		if (!chg->pr_swap_in_progress) {
			 ...
			 */
			if (0 && chg->dual_role)/*vivo modify*/
		...
		chg->typec_state_normal = WORK_STATUS_END;
		smblib_dbg(chg, PR_MISC, "typec state=%d\n", chg->typec_state_normal);
		/*vivo end*/
	}
...
irqreturn_t switcher_power_ok_irq_handler(int irq, void *data)
{
	...
	if (is_storming(&irq_data->storm_data)) {
		...
		if (!is_client_vote_enabled(chg->usb_icl_votable,
			...
			if (chg->weak_charger_count < 3)/* vivo modify */
				vote(chg->usb_icl_votable, WEAK_CHARGER_VOTER,
			...
			cancel_delayed_work(&chg->weak_charger_voter_clear_work);
			if (chg->weak_charger_count < 3) {
				schedule_delayed_work(&chg->weak_charger_voter_clear_work,
						msecs_to_jiffies(5000));
			} else if (chg->weak_charger_count == 4) {
				...
				vote(chg->usb_icl_votable, WEAK_CHARGER_VOTER, true, WEAK_CHG_CUR_100_UA);
				chg->chg_abnormal_flag |= CHG_WEAK_CHARGER_100_ERROR;
			}
			/* vivo end*/
		} else {
			smblib_err(chg,
...
static void otg_oc_work(struct work_struct *work)
{
	struct smb_charger *chg = container_of(work, struct smb_charger, otg_oc_work.work);
	u8 cmd;
	int rc = 0;
	...
	if (typec_mode == POWER_SUPPLY_TYPEC_SINK || typec_mode == POWER_SUPPLY_TYPEC_SINK_POWERED_CABLE) {
		rc = smblib_read(chg, DCDC_CMD_OTG_REG, &cmd);
		if (cmd & OTG_EN_BIT) {
			smblib_dbg(chg, PR_OTG, "re-enable OTG\n");
			rc = smblib_masked_write(chg, DCDC_CMD_OTG_REG, OTG_EN_BIT, 0);
			msleep(10);
			rc = smblib_masked_write(chg, DCDC_CMD_OTG_REG, OTG_EN_BIT, OTG_EN_BIT);
		}
...
irqreturn_t otg_oc_disable_sw_irq_handler(int irq, void *data)
{
	struct smb_irq_data *irq_data = data;
	struct smb_charger *chg = irq_data->parent_data;

	cancel_delayed_work_sync(&chg->otg_oc_work);
	schedule_delayed_work(&chg->otg_oc_work,
	msecs_to_jiffies(2000)); //re-enable after 2sec
				...
				const union power_supply_propval *val)
{
	...
	/*vivo add*/
	if (!chg->drp_enable) {
		pr_info("drp_enable=0, skip...[value=%d]\n", val->intval);
		return 0;
	}
	/*vivo end*/
...
static void smblib_force_9v_hvdcp_work(struct work_struct *work)
{
	int rc;

	struct smb_charger *chg = container_of(work, struct smb_charger, force_9v_hvdcp_work.work);

	rc = force_9v_hvdcp(chg);
	if (rc < 0) {
...
static void smblib_typec_state_verification_work(struct work_struct *work)
{
	int rc;
	struct smb_charger *chg = container_of(work, struct smb_charger,
						typec_state_verification_work.work);

	if (!chg->drp_enable && ((!chg->usb_present && chg->typec_state_normal == WORK_STATUS_ONGOING) ||
		(chg->usb_present && chg->typec_state_normal != WORK_STATUS_ONGOING))) {
		smblib_dbg(chg, PR_MISC, "typec state verification abnormal!\n");
...
int smblib_force_dr_mode(struct smb_charger *chg, int mode)
{
	...
	smblib_dbg(chg, PR_REGISTER, "dual_role mode=%d\n", mode);
...
static void smblib_dual_role_check_work(struct work_struct *work)
{
	...
	pr_err("dual_role check ,dr_mode=%d\n",chg->dr_mode);
 ...
 * vivo function list *
 *******************/
/*add by wanglanbin for enable otg vbus to mirco usb*/
int vivo_smblib_notify_usb_host(struct smb_charger *chg, bool enable)
{
	int rc = 0;
...
static int vivo_set_chg_term_current(struct smb_charger *chg, int term_current)
{
	...
	if (term_current < -10000 ||
			term_current > 10000) {
		dev_err(chg->dev, "ITERM threshold out of range rc=%d\n", rc);
		return -EINVAL;
	}
	pr_info("term_current_thresh_hi_ma=%d,term_current=%d\n", chg->term_current_thresh_hi_ma, term_current);
	/*
	 * Conversion:
	 *	raw (A) = (scaled_mA * ADC_CHG_ITERM_MASK) / (10 * 1000)
	 * Note: raw needs to be converted to big-endian format.
	 */
	if (chg->term_current_thresh_hi_ma != term_current) {
		raw_hi_thresh = ((term_current *
						ADC_CHG_ITERM_MASK) / 10000);
					...
					union power_supply_propval *val)
{
	...
	if (!chg->iio.pri_temp_chan) {
		smblib_err(chg, "pri_temp_chan is null!\n");
		return 0;
	}

	rc = iio_read_channel_processed(chg->iio.pri_temp_chan, &adc_result);
	if (rc < 0) {
		smblib_err(chg, "adc reading error pri_temp_chan, rc = %d\n", rc);
		return rc;
	}
	val->intval = adc_result/100;

	smblib_dbg(chg, PR_MISC, "primary_board_temp = %d\n", adc_result/100);
					...
					union power_supply_propval *val)
{
	...
	if (chg->pl_use_batt_temp_enable) {
		rc = smblib_get_prop_from_bms(chg, POWER_SUPPLY_PROP_TEMP, val);
		return rc;
	}

	if (!chg->iio.pl_temp_chan) {
		smblib_err(chg, "pl_temp_chan is null!\n");
		return 0;
	}

	rc = iio_read_channel_processed(chg->iio.pl_temp_chan, &adc_result);
	if (rc < 0) {
		smblib_err(chg, "adc reading error pl_temp_chan, rc = %d\n", rc);
		return rc;
	}
	val->intval = adc_result/100;
	if (chg->direct_charger_enable) {
	...
	smblib_dbg(chg, PR_MISC, "parallel_board_temp = %d\n", adc_result/100);
		...
		union power_supply_propval *val)
{
	...
	pr_err("bat_board_temp=%d,enable_slave_charger=%d\n", bat_board_temp, enable_slave_charger);
	return 0;
}
				  ...
				  const union power_supply_propval *val)
{
	...
	if (current_ua == 0) {
		...
		if (rc < 0)
			smblib_err(chg, "Couldn't disable battchg, rc %d\n", rc);
		return rc;
	}
	...
	if (!chg->user_batt_current_ua) {
		...
		if (rc < 0){
			smblib_err(chg, "Couldn't enable battchg, rc %d\n", rc);
			return rc;
		}
	}
				  ...
				  const union power_supply_propval *val)
{
	...
	if (rc < 0){
		smblib_err(chg, "Input couldn't vote, rc %d\n", rc);
		return rc;
	}
				  ...
				  const union power_supply_propval *val)
{
	...
	smblib_dbg(chg, PR_MISC, "bsp cut-off:%d,is_atboot:%d\n", enable, is_atboot);

	if (!chg->usb_present && !is_atboot) {
		smblib_dbg(chg, PR_MISC, "usb isn't present and is not at mode,bsp cut-off skip!\n");
		return 0;
	}

	if (chg->usb_present && enable && is_parallel_charger_available(chg)) {
		pval.intval = 1;
		power_supply_set_property(chg->pl.psy,
						POWER_SUPPLY_PROP_PRESENT, &pval);
		msleep(300);
...
#define DEFAULT_CHG_EST_VBAT_DIFF	2
					...
					union power_supply_propval *val)
{
	...
	if (!chg->bms_psy) {
		smblib_dbg(chg, PR_MISC, "bms psy use default value\n");
		val->intval = DEFAULT_CHG_EST_VBAT_DIFF;
		return 0;
	}
	rc = power_supply_get_property(chg->bms_psy,
...
int smblib_set_chg_vbus_voltage(struct smb_charger *chg, int val)
{
	if (!chg->usb_present) {
		smblib_dbg(chg, PR_MISC, "usb can't plugin,not set chg vbus voltage!\n");
		return 0;
	}

	smblib_dbg(chg, PR_MISC, "try to chg_vbus_voltage:%dv.\n", val);

	switch (val) {
	case HVDCP_TYPE_5V:
		chg->cc_force_5v_flag |= CCF5_FLAG_FLASH_LED;
		force_5v_hvdcp(chg);
		break;
	case HVDCP_TYPE_9V:
		chg->cc_force_5v_flag &= (~CCF5_FLAG_FLASH_LED);
		...
		break;
	default:
		smblib_dbg(chg, PR_MISC, "force %d,no support!\n", val);
	}
...
int smblib_set_came_notify(struct smb_charger *chg, bool came_sta)
{
	union power_supply_propval camstat = {0,};
	struct power_supply *mcu_psy = get_mcu_psy(chg);

	camstat.intval = came_sta;
	if (!mcu_psy) {
		smblib_err(chg, "mcu_psy not found\n");
		return 0;
	}
	power_supply_set_property(chg->mcu_psy,
	POWER_SUPPLY_PROP_CAMERA_CHARGE_CLOSE, &camstat);
...
int smblib_get_prop_batt_fcc(struct smb_charger *chg, union power_supply_propval *val)
{
	...
	if (rc < 0) {
		...
		smblib_err(chg, "Couldn't get capacity_mah rc=%d\n", rc);
	}
...
int smblib_get_prop_batt_soc_decimal(struct smb_charger *chg, union power_supply_propval *val)
{
	...
	if(chg->battery_capacity_is_exception || batt_soc.intval  >=98 ||batt_soc.intval  <= 3) {
...
int smblib_handle_soc(struct smb_charger *chg)
{
	...
	smblib_get_prop_batt_capacity(chg,&soc);

	rc = smblib_get_prop_from_bms(chg, POWER_SUPPLY_PROP_BATTERY_PARAM_STATUS, &param_status);
	if (rc) {
		smblib_err(chg, "Couldn't get param_status, rc = %d\n", rc);
		return soc.intval;
	}
	if(!param_status.intval){
		smblib_err(chg, "batt param is not exist!\n");
		return soc.intval;
	}

	if(param_status.intval && chg->ui_soc < 0){
		chg->ui_soc = soc.intval;
	...
	rc = power_supply_get_property(chg->batt_psy,
			POWER_SUPPLY_PROP_STATUS_EX, &chg_status);
	if (rc) {
		smblib_err(chg, "couldn't get chg_status,rc=%d\n", rc);
		chg->ui_soc = soc.intval;
		mutex_unlock(&chg->ui_soc_lock);
		return chg->ui_soc;
	}
	...
	if (chg_status.intval == POWER_SUPPLY_STATUS_CHARGING ||
		...
		if(is_charging_change && chg->ui_soc < soc.intval){
	...
	//smblib_dbg(chg, PR_MISC, "ui_soc=%d,soc=%d,is_charging_change=%d,chg_status=%d \n",
							//chg->ui_soc,soc.intval,is_charging_change,chg_status.intval);
...
static int smblib_get_hvdcp_type(struct smb_charger *chg)
{
	...
	if (rc < 0) {
		smblib_err(chg, "read usbin err rc=%d\n", rc);
		return 0;
	}
...
int force_5v_hvdcp(struct smb_charger *chg)
{
	...
	if (rc) {
		smblib_err(chg, "failed to read CMD_HVDCP_2_REG ->FORCE_5V_BIT, rc=%d\n", rc);
		return rc;
	}
	msleep(300);
...
int force_9v_hvdcp(struct smb_charger *chg)
{
	int rc;
	u8 stat;
	union power_supply_propval flash_state = {0,};

	smblib_dbg(chg, PR_MISC, "\n");

	/* check FLASH_ACTIVE, if active, do not force 9V */
	rc = power_supply_get_property(chg->usb_main_psy, 
		POWER_SUPPLY_PROP_FLASH_ACTIVE, &flash_state);
	if (rc < 0)
		smblib_err(chg, "Couldn't get BATT_PSY FLASH_ACTIVE state, rc = %d\n", rc);

	if (flash_state.intval) {
		smblib_dbg(chg, PR_MISC, "FLASH_ACTIVE is active, do not force 9V\n");
		return 0;
	}
	...
	if (rc) {
		smblib_err(chg, "failed to read CMD_HVDCP_2_REG ->FORCE_9V_BIT, rc=%d\n", rc);
		return rc;
	}
...
int smblib_reset_inhibit_status(struct smb_charger *chg)
{
	...
	if ((stat&BATTERY_CHARGER_STATUS_MASK) == 0x0) {//detect inhibit status
		rc = smblib_masked_write(chg, CHARGING_ENABLE_CMD_REG,
	...
	smblib_read(chg, BATTERY_CHARGER_STATUS_1_REG, &stat);
...
static int smblib_termination_by_soft(struct smb_charger *chg, int chg_status, int batt_soc)
{
	...
	if (chg_status == POWER_SUPPLY_STATUS_CHARGING) {
		if (batt_soc >= chg->soft_term_capacity_limit) {
			cancel_delayed_work_sync(&chg->soft_term_timer_work);
...
static int smblib_charge_condition_force_5v(struct smb_charger *chg, int chg_status, int batt_soc, int chg_volt_v)
{
	if (chg->direct_charger_enable)
		return 0;

	if (batt_soc >= chg->cc_force_5v_soc_limit ||
			chg_status == POWER_SUPPLY_STATUS_FULL ||
			chg->cc_force_5v_usb_conn_protect) {
		if (chg_volt_v > 5) {
			smblib_dbg(chg, PR_MISC, "cc input adjust to 5v\n");
			chg->cc_force_5v_flag = CCF5_FLAG_SOC_LIMIT|CCF5_FLAG_CHG_FULL|CCF5_FLAG_USB_CONN_PRO;
			chg->cc_force_5v_retries = 0;
			msleep(300);
			force_5v_hvdcp(chg);
		} else {
		...
		if (chg->cc_force_5v_flag & CCF5_FLAG_FLASH_LED) {
			smblib_dbg(chg, PR_MISC, "flash led control,cc input adjust is not need\n");
		} else if (chg->cc_force_5v_flag && chg_volt_v <= 5) {
			if (chg->cc_force_5v_retries < 6) {
				chg->cc_force_5v_retries++;
				smblib_dbg(chg, PR_MISC, "cc input adjust to 9v,retries=%d\n", chg->cc_force_5v_retries);
				force_9v_hvdcp(chg);
			}
		} else {
			smblib_dbg(chg, PR_MISC, "cc input adjust is not need\n");
			chg->cc_force_5v_flag = CCF5_FLAG_UNKNOWN;
			chg->cc_force_5v_retries = 0;
		}
	}
	...
	if (is_parallel_charger_available(chg)) {
		...
		switch (pval.intval) {
			case PL_STATUS_OK:
				...
				if (chg->pl_status == PL_STATUS_I2C_ERROR) {
					if (chg->pl_enable_gpio > 0)
						gpio_direction_output(chg->pl_enable_gpio, 0);
				}
				chg->pl_status = PL_STATUS_OK;
			...
			case PL_STATUS_I2C_ERROR:
				...
				if (chg->chg_voltage_v >= HVDCP_TYPE_9V) {
					...
					msleep(100);
					vote(chg->usb_icl_votable, PL_STATUS_VOTER, true, 1000000);
				}
...
static void smblib_dump_regs(struct smb_charger *chg)
{
	...
	smblib_dump_reg(chg, "SCHG_P_CHGR_ADC_ITERM_UP_THD_MSB", 0x1067);
	smblib_dump_reg(chg, "SCHG_P_CHGR_ADC_ITERM_UP_THD_LSB", 0x1068);
	smblib_dump_reg(chg, "SCHG_P_CHGR_ADC_ITERM_LO_THD_MSB", 0x1069);
	smblib_dump_reg(chg, "SCHG_P_CHGR_ADC_ITERM_LO_THD_LSB", 0x106A);
	smblib_dump_reg(chg, "SCHG_P_CHGR_ADC_TERM_CFG", 0x106C);
	smblib_dump_reg(chg, "SCHG_P_CHGR_ADC_TERM_CFG_2", 0x106D);
	...
	smblib_dump_reg(chg, "SCHG_P_CHGR_ADC_TERM", 0x10C0);
...
static void smblib_dump_otg_regs(struct smb_charger *chg)
{
	...
	/*otg*/
	smblib_dump_reg(chg, "SCHG_P_DCDC_THERMAL_ICL_STATUS", 0x1109);
	smblib_dump_reg(chg, "SCHG_P_DCDC_INT_RT_STS", 0x1110);
	smblib_dump_reg(chg, "SCHG_P_DCDC_CMD_OTG", 0x1140);
	smblib_dump_reg(chg, "SCHG_P_TYPEC_TYPE_C_SNK_STATUS", 0x1506);
...
#define OTG_MONITOR_WORK_PEROID_MS	10000
static void smblib_otg_monitor_work(struct work_struct *work)
{
	...
	if (!chg->direct_charger_enable && !chg->usb_present && !chg->otg_disable_pl && is_parallel_charger_available(chg)) {
		pval.intval = true;
		rc = power_supply_set_property(chg->pl.psy,
	...
	if (chg->otg_monitor_count > 6 && otg_detach) {
		rc = smblib_masked_write(chg, TYPE_C_MODE_CFG_REG,
					TYPEC_POWER_ROLE_CMD_MASK | TYPEC_TRY_MODE_MASK,
					EN_SNK_ONLY_BIT);
	...
	fuelsummary_collect_value(ID_OTG_CNT, chg->otg_monitor_count);
	...
	smblib_read(chg, USB_CMD_PULLDOWN_REG, &stat_pulldown);
	smblib_err(chg, "Read pulldown = %d\n", stat_pulldown);
...
static void smblib_ffc_adjust_function(struct smb_charger *chg, int *work_period_ms)
{
	...
	if (chg->bms_psy &&
		...
		if (bat_temp <= chg->ffc_temperature_range[0] || bat_temp >= chg->ffc_temperature_range[1]
			...
			smblib_dbg(chg, PR_MISC, "ibat_ma=%d,flag=%d,capcity=%d,bat_temp=%d,vbat_uv=%d,fg_vbat_uv=%d,cout_gpio=%d,dchg_status=%d,charge_full=%d,ffc_charge_full=%d,ffc_pmi_charging_count=%d\n",
				...
				chg->ffc_charge_full, chg->ffc_pmi_charging_count);
			...
			if (!chg->ffc_charge_full && chg->dchg_status == DIRECT_CHARGER_UNKNOW
				...
				if ((is_between(ffc_full_chg_ibat_ma, 0, ibat_ma) && (fg_vbat_uv >= chg->ffc_full_chg_vbat_table[FFC_FULL_CHG_VBAT] || battery_cout_value >= 1))
					|| (is_between(-700, 0, ibat_ma) && (fg_capacity == 100 || fg_vbat_uv >= chg->ffc_full_chg_vbat_table[FFC_FULL_CHG_VBAT_TERM]))) {
					vivo_set_chg_term_voltage(chg, chip->dt.batt_profile_fv_uv);
					vivo_set_chg_term_current(chg, chip->dt.term_current_thresh_hi_ma);
					/* disable direct charge */
					if (!chg->dchg_disable_votable)
				...
				&& chg->ffc_pmi_charging_count++ > 1
				&& ((is_between(ffc_full_chg_ibat_ma, 0, ibat_ma) && (vbat_uv >= chg->ffc_full_chg_vbat_table[FFC_FULL_CHG_VBAT_EX] || fg_vbat_uv >= chg->ffc_full_chg_vbat_table[FFC_FULL_CHG_VBAT] || battery_cout_value >= 1))
				|| (is_between(-700, 0, ibat_ma) && (fg_capacity == 100 || fg_vbat_uv >= chg->ffc_full_chg_vbat_table[FFC_FULL_CHG_VBAT_TERM])))) {
				vivo_set_chg_term_voltage(chg, chip->dt.batt_profile_fv_uv);
				vivo_set_chg_term_current(chg, chip->dt.term_current_thresh_hi_ma);
				chg->ffc_charge_full = true;
...
extern void get_votable_info(struct votable *votable);
static void smblib_charge_monitor_work(struct work_struct *work)
{
	...
	get_votable_info(chg->fcc_main_votable);
	...
	fuelsummary_collect_value(ID_CAB_CC, chg->cable_with_cc_float);
	/*WA: duble check Safety Timeout status*/
	...
	smblib_termination_by_soft(chg, chg_status.intval, batt_soc.intval);

	/* check charge recharge by soft */
	smblib_recharge_by_soft(chg, chg_status.intval);

	/* charge condition force 5v*/
	smblib_charge_condition_force_5v(chg, chg_status.intval, batt_soc.intval, chg->chg_voltage_v);
	...
	/* dump pl regs */
	if (!chg->direct_charger_enable && is_parallel_charger_available(chg) && (chg->pl_status == PL_STATUS_OK)) {
		power_supply_get_property(chg->pl.psy,
						POWER_SUPPLY_PROP_DUMP_REG, &pval);
	}

	/* clear usb temporary present after 60s */
	if (9 == chg->charge_monitor_count) {
		...
		if(chg->real_charger_type == POWER_SUPPLY_TYPE_USB_FLOAT ||
			...
			smblib_dbg(chg, PR_MISC, "clear SW_ICL_MAX_VOTER vote \n");
			vote(chg->usb_icl_votable, SW_ICL_MAX_VOTER, false, 0);
		}
	}

	/* when disable pl,voter input 1.45a */
	if (!chg->direct_charger_enable && get_effective_result_locked(chg->pl_disable_votable) &&
				chg->chg_voltage_v >= HVDCP_TYPE_9V){
		vote(chg->usb_icl_votable, HVDCP_CTRL_ICL_VOTER, true, 1450000);
	}else{
		vote(chg->usb_icl_votable, HVDCP_CTRL_ICL_VOTER, false, 0);
	}

	/* when fcc <10000 ,disable pl */
	...
	if (is_client_vote_enabled_locked(chg->usb_icl_votable, USB_PSY_VOTER) && chg->charge_monitor_count > 6) {
		schedule_delayed_work(&chg->usb_psy_voter_clear_work,
				msecs_to_jiffies(8000));
	}

	/* anyway, USER_VOTER vote false */
	if (get_client_vote_locked(chg->usb_icl_votable, USER_VOTER) > 0) {
		smblib_dbg(chg, PR_MISC, "clear USER_VOTER vote input-current...\n");
		vote(chg->usb_icl_votable, USER_VOTER, false, 0);
	}

/*
	if (1 == chg->charge_monitor_count && chg->connector_type == POWER_SUPPLY_CONNECTOR_TYPEC &&
			!(get_client_vote(chg->usb_icl_votable, OTG_VOTER))) {
		smblib_dbg(chg, PR_MISC, "clear OTG_VOTER vote...\n");
		vote(chg->usb_icl_votable, OTG_VOTER, false, 0);
		chg->cable_with_cc_float_otg = true;
		rerun_election(chg->usb_icl_votable);
	}
*/
	/*typec mode is none && current 0, use uclc mode */
	if (2 == chg->charge_monitor_count && chg->connector_type == POWER_SUPPLY_CONNECTOR_TYPEC &&
			(chg->cable_with_cc_float || chg->cable_with_cc_float_otg)) {
		apsd_result = smblib_get_apsd_result(chg);
		if (apsd_result->pst != POWER_SUPPLY_TYPE_USB) {
			smblib_dbg(chg, PR_MISC, "override icl : uclc mode...(cable_with_cc_float_otg=%d)\n", chg->cable_with_cc_float_otg);
	...
	smblib_dbg(chg, PR_MISC, "count=%d,bigdata_chg_voltage_uv=%d,hvdcp_type=%d,usb_input_current=%d,usb_temporary_present=%d,chg_abnormal_flag=0x%02x,vbat_in_iterm=%d,vbat=%duV,charge_full=%d,ffc_charge_full=%d.\n",
				chg->charge_monitor_count, chg->bigdata_chg_voltage_uv, chg->hvdcp_type, usb_input_current.intval,
...
static void smblib_split_current_monitor_work(struct work_struct *work)
{
	...
	/*WA: when plugin and pl not work,the first time icl voter input 1.45a*/
	smblib_check_pl_abnormal_status(chg);
	if (!chg->direct_charger_enable && chg->pl_status == PL_STATUS_I2C_ERROR &&
...
static void smblib_hvdcp_charge_monitor_work(struct work_struct *work)
{
	...
	if (chg->hvdcp_type == HVDCP_TYPE_9V && !chg->cc_force_5v_flag) {
		if (hvdcp_type == HVDCP_TYPE_5V && !chg->factory_10W_charge_test_enabe) {
			if (chg->hvdcp_9v_retries < SWITCH_HVDCP_RETRIES_MAX) {
				...
				if((chg->real_charger_type == POWER_SUPPLY_TYPE_USB ||
					...
					vote(chg->usb_icl_votable, USER_DEFAULT_INPUT_VOTER, true, DEFAULT_FLOAT_UA);
...
static void smblib_usb_conn_pro_monitor_work(struct work_struct *work)
{
	...
	if (usb_conn_temp.intval >= usb_trigger_threshold && chg->usb_conn_pro_flag != 1 && (last_cms_health & 0x4000)) {
		...
		fuelsummary_collect_value(ID_VBUS_PULL, chg->usb_vbus_pull_down_count);
...
static void smblib_check_drp_disable_work(struct work_struct *work)
{
	...
	smblib_dbg(chg, PR_MISC, "OTG switch, timer count = %d, stat=%d, power_role=%d\n", chg->drp_timer_count, stat, chg->power_role);

retry:
	schedule_delayed_work(&chg->check_drp_disable_work, msecs_to_jiffies(CHECK_DRP_DISABLE_TIME));
}
...
int smblib_init(struct smb_charger *chg)
{
	...
	INIT_DELAYED_WORK(&chg->otg_oc_work, otg_oc_work);
	INIT_DELAYED_WORK(&chg->pr_lock_clear_work,
					smblib_pr_lock_clear_work);
	INIT_DELAYED_WORK(&chg->typec_state_verification_work,
					smblib_typec_state_verification_work);
	INIT_DELAYED_WORK(&chg->force_9v_hvdcp_work,
					smblib_force_9v_hvdcp_work);
	...
	//schedule_delayed_work(&chg->check_drp_disable_work, msecs_to_jiffies(5000));
	...
	chg->custom_cutoff_volt_mv = 3400000;
	...
	case PARALLEL_MASTER:
		...
		if (chg->usbsel_gpio >= 0) {
			rc = gpio_request(chg->usbsel_gpio,"usbsel_gpio");
			if (rc) {
				pr_err("failed to request usbsel_gpio,rc=%d\n",rc);
				return rc;
			}
			/*Init usb switcher's OE-pin state.*/
			gpio_direction_output(chg->usbsel_gpio,1);
			smblib_dbg(chg, PR_MISC, "usbsel=%d\n",gpio_get_value(chg->usbsel_gpio));
		}

		if (chg->usboe_gpio > 0) {
			rc = gpio_request(chg->usboe_gpio,"usboe_gpio");
			if (rc) {
				pr_err("failed to request usboe_gpio,rc=%d\n",rc);
				return rc;
			}
			/*Init usb switcher's OE-pin state.*/
			gpio_direction_output(chg->usboe_gpio,0);
			smblib_dbg(chg, PR_MISC, "usboe=%d\n",gpio_get_value(chg->usboe_gpio));
		}

		if (chg->pl_enable_gpio > 0) {
			rc = gpio_request(chg->pl_enable_gpio, "pl_enable_gpio");
			if (rc) {
				pr_err("pl_enable_gpio request failed for %d rc=%d\n",
					chg->pl_enable_gpio, rc);
			} else {
				/*Init pl enable pin state.*/
				gpio_direction_output(chg->pl_enable_gpio, 1);
				smblib_dbg(chg, PR_MISC, "pl_enable_gpio=%d\n", gpio_get_value(chg->pl_enable_gpio));
			}
		}
		/* vivo end */
		...
		if (chg->bms_psy_name) {
			...
			if (!chg->bms_psy && strcmp(chg->bms_psy_name, "bms")) {
				chg->bms_psy = power_supply_get_by_name("bms");
				smblib_err(chg,"get bms_psy (%s) fail, retry get qc bms\n", chg->bms_psy_name);
			}
		} else
			chg->bms_psy = power_supply_get_by_name("bms");
		/*vivo end*/
...
int smblib_deinit(struct smb_charger *chg)
{
	...
	case PARALLEL_MASTER:
		...
		cancel_delayed_work_sync(&chg->pr_swap_detach_work);
		cancel_delayed_work_sync(&chg->otg_oc_work);
		cancel_delayed_work_sync(&chg->force_9v_hvdcp_work);
