...
static int qg_parse_battery_profile(struct qg_battery_data *battery)
{
	...
	for (i = 0; i < TABLE_MAX; i++) {
...
#if 1
		newdata = (int *)kzalloc((rows * cols * sizeof(int)), GFP_KERNEL);
		if (!newdata) {
			rc = -ENOMEM;
			goto cleanup;
		}
		for (j = 0; j < rows; j++) {
			battery->profile[i].data[j] = (newdata + j * cols);
		}
#else
		for (j = 0; j < rows; j++) {
		...
		memset(name, 0x0, 128);
		sprintf(name, "%s-col", table[i].table_name);
		//fuelsummary_of_property_puts(name, STATIC_FIXED, PARAMS_TYPE_INT, battery->profile[i].col_entries, 1, cols);
		...
		if (rows != 1) {
			...
			memset(name, 0x0, 128);
			sprintf(name, "%s-row", table[i].table_name);
			//fuelsummary_of_property_puts(name, STATIC_FIXED, PARAMS_TYPE_INT, battery->profile[i].row_entries, 1, rows);
		}
		...
		memset(name, 0x0, 128);
		sprintf(name, "%s-data", table[i].table_name);
		//fuelsummary_of_property_puts(name, STATIC_FIXED, PARAMS_TYPE_INT, newdata, rows, cols);
...
#if 1
		if (battery->profile[i].data[0])
			kfree(battery->profile[i].data[0]);
#else
...
//vivo add
void qg_batterydata_dump(void)
{
	int i, j, k, rows = 0, cols = 0;
	int *p;
	uint8_t val[24];
	uint8_t str[512];

	pr_err("QG Battery-profile dump:\n");
	if (the_battery) {
		for (i = 0; i < TABLE_MAX; i++) {
			rows = the_battery->profile[i].rows;
			cols = the_battery->profile[i].cols;
			pr_info("%s:row=%d,column=%d\n", the_battery->profile[i].name, rows, cols);
			p = the_battery->profile[i].col_entries;
			memset(str, 0x0, 512);
			for (j = 0; j < cols; j++) {
				memset(val, 0x0, 24);
				sprintf(val, "%d ", *(p + j));
			...
			pr_info("rows:\n%s\n", str);
			if (rows != 1) {
				p = the_battery->profile[i].row_entries;
				memset(str, 0x0, 512);
				for (j = 0; j < rows; j++) {
					memset(val, 0x0, 24);
					sprintf(val, "%d ", *(p + j));
			...
			for (j = 0; j < rows; j++) {
				...
				for (k = 0; k < cols; k++) {
					memset(val, 0x0, 24);
					sprintf(val, "%d ", the_battery->profile[i].data[j][k]);
					strcat(str, val);
				}
				pr_info("%s\n", str);
			}
