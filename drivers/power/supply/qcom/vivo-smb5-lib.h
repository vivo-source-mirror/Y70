...
#define SET_ICL_VOTER		"SET_ICL_VOTER"
...
/*macro definition*/
#define DEFAULT_BATT_CAPACITY_MAH	3000
...
#define HVDCP_CHARGE_MONITOR_MS 200
...
#define CHECK_DRP_DISABLE_TIME  (20000) /*20s */
...
enum ffc_data_type {
	FFC_DATA_TYPE_UNKNOWN = 0,
	FFC_DATA_TYPE_IBAT = 1,
	FFC_DATA_TYPE_VBAT,
	FFC_DATA_TYPE_VBAT_EX,
	FFC_DATA_TYPE_CAPACITY,
	FFC_DATA_TYPE_BAT_TEMP,
};
static const char * const ffc_data_type_strings[] = {
	"unkonwn",
	"Ibat",
	"Vbat",
	"Vbat_ex",
...
enum {
	STRUCT_INT_ARRAY = 0,
	STRUCT_FFC_FULL_CHG_IBAT_TABLE,
};

struct ffc_full_chg_ibat_table {
	int tmin;
	int tmax;
	int ibat;
	int ibat_delta;
...
static int default_ffc_full_chg_ibat_rc[] = {1, 4};
static struct ffc_full_chg_ibat_table default_ffc_full_chg_ibat_table[] = {
	{
		.tmin = 150,
		.tmax = 450,
		.ibat = -1000,
...
enum {
	FFC_FULL_CHG_VBAT = 0,
	FFC_FULL_CHG_VBAT_EX,
	FFC_FULL_CHG_VBAT_TERM,
	FFC_FULL_CHG_MAX,
};

static int default_ffc_full_chg_vbat_table[] = {4400000, 4400000, 4400000};

enum charge_condition_force_5v_flag {
	CCF5_FLAG_UNKNOWN = 0,
	CCF5_FLAG_SOC_LIMIT = 0x0001,
	CCF5_FLAG_CHG_FULL = 0x0002,
	CCF5_FLAG_USB_CONN_PRO = 0x0004,
	CCF5_FLAG_FLASH_LED = 0x0008,
};
/* vivo end*/
...
#define TYPEC_HIGH_CURRENT_UA		2000000/*vivo set max input 2A*/
...
struct smb_iio {
	...
	/*vivo add*/
	struct iio_channel	*pri_temp_chan;
	struct iio_channel	*pl_temp_chan;
	struct iio_channel	*board_temp_chan;
...
struct smb_charger {
	...
	struct mutex		adc_lock;//vivo add
	/* power supplies */
	...
	struct delayed_work	otg_oc_work;
	struct delayed_work	pr_lock_clear_work;
	struct delayed_work	typec_state_verification_work;
	struct delayed_work	force_9v_hvdcp_work;
	...
	/* fg_cycle_count / soc_count */
	int fg_cycle_count;

	/* charge condition force 5v */
	int cc_force_5v_flag;
	int cc_force_5v_retries;
	int cc_force_5v_soc_limit;
	int cc_force_5v_usb_conn_protect;
	bool cc_force_5v_flashled_enable;

	/* otg monitor work */
	struct delayed_work otg_monitor_work;
	bool otg_disable_pl;
	int otg_monitor_count;

	/*add charging spec info */
	int supported_adapter;
	char charging_info[50];
	int current_spec_min;
	int current_spec_max;

	bool enable_slave_charger;
	bool single_charger_enable;
	bool pd1965_flag;
	...
	bool cable_with_cc_float_otg;

	/*battery temp compensation*/
	bool batt_temp_comp_enable;
	int batt_temp_comp_data;
	...
	int ffc_pmi_charging_count;
	int ffc_chg_term_current_ma;
	int ffc_fv_max_uv;
	struct ffc_full_chg_ibat_table *ffc_full_chg_ibat_table;
	int *ffc_full_chg_ibat_rc;
	int ffc_full_chg_iterm_delta_ma;
	int *ffc_full_chg_vbat_table;
	...
	/* custom_cutoff_volt_mv for vbat */
	int custom_cutoff_volt_mv;

	bool pd1957_compatible_flag;

	/* add for factory mode 5V2A test */
	bool factory_10W_charge_test_enabe;
	/*vivo end*/
};
