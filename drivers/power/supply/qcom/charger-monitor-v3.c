...
static int cms_start_bat_id_det(struct cms *chip)
{
	...
	schedule_delayed_work(&chip->bat_id_det_work, 0);
	return 0;
}
/*
...
{
	if (!out) {
		if (chip->batid_pinctrl_enable) {
			if (pinctrl_select_state(chip->pinctrl, chip->batid_gpio_default))
				cms_print(chip, PR_ERROR, "failed to pinctrl_select_state: no pull.\n");
		} else if (gpio_direction_input(chip->bat_id_gpio)) {
			cms_print(chip, PR_ERROR, "failed to set gpio input\n");
		}
	} else {
		if (chip->batid_pinctrl_enable) {
			if (up) {
				if (pinctrl_select_state(chip->pinctrl, chip->batid_gpio_high))
					cms_print(chip, PR_ERROR, "failed to pinctrl_select_state: high\n");
			} else {
				if (pinctrl_select_state(chip->pinctrl, chip->batid_gpio_low))
					cms_print(chip, PR_ERROR, "failed to pinctrl_select_state: low\n");
			}
		} else if (gpio_direction_output(chip->bat_id_gpio, up)) {
...
static int cms_calculate_rc(struct cms *chip)
{
	...
	if(chip->bat_det_method == BAT_DET_BY_R){
		for (index = 0; index < chip->r_items_num; index++) {
			if(is_between(chip->r_items[index].v1min, chip->r_items[index].v1max, rc->v1))
				return chip->r_items[index].id;
		}
    } else if (chip->bat_det_method == BAT_DET_BY_RC ){
		for (index = 0; index < chip->rc_items_num; index++) {
			if(is_between(chip->rc_items[index].v1min, chip->rc_items[index].v1max, rc->v1)
				&& is_between(chip->rc_items[index].v2min, chip->rc_items[index].v2max, rc->v2))
				return chip->rc_items[index].id;
		}
	}
	return 0;
}
...
static int cms_battery_id_limit(struct cms *chip, int batid)
{
	int i;

	if (!chip->bat_id_nums) {
		cms_print(chip, PR_INFO, "bat id limit isn't supported!\n");
		return BAT_ID_LIMIT_NOSUPPORT;
	}

	for (i = 0; i < chip->bat_id_nums; i++) {
		if (batid == chip->bat_id_value[i]) {
			cms_print(chip, PR_INFO, "bat_id(%d) is matching successful!\n", batid);
			return BAT_ID_LIMIT_MATCH;
		}
	}

	cms_print(chip, PR_INFO, "bat_id(%d) is matching error!\n", batid);
	return BAT_ID_LIMIT_ERROR;
}

static int cms_select_tc_type(struct cms *chip)
{
	int index;

	if (!chip->ex_normal_tc_enable_num) {
		cms_print(chip, PR_INFO, "ex_normal_tc isn't enable\n");
		return NORMAL_TC;
	}

	if (!chip->battery_rc.bat_id) {
		cms_print(chip, PR_INFO, "batid isn't detected.use normal_tc.\n");
		return NORMAL_TC;
	}

	for(index = 0; index < chip->ex_normal_tc_enable_num; index++){
		if(chip->battery_rc.bat_id == chip->ex_normal_tc_limit[index]) {
			cms_print(chip, PR_INFO, "detect ex_normal_tc sucessfully!\n");
			return EX_NORMAL_TC;
		}
	}

	cms_print(chip, PR_INFO, "detect normal_tc sucessfully!\n");
	return NORMAL_TC;
}

static void cms_battery_det_done(struct cms *chip)
{
	cms_print(chip, PR_DBG, "battery id=%s\n", chip->bat_id_strings[chip->battery_rc.bat_id]);
	...
	if(chip->battery_rc.counter > 1){
		...
		chip->tc_type = cms_select_tc_type(chip);
		fuelsummary_collect_value(ID_BATID, chip->battery_rc.bat_id);
	}
	if(chip->complete)
		complete(chip->complete);
}

static int cms_batid_extra_handle(struct cms *chip)
{
	int index, bid = 0;
	struct battery_rc *rc = &chip->battery_rc;

	if (!chip->batid_extra_handle_enable || chip->bat_id_new_param_enable) {
		cms_print(chip, PR_INFO, "batid_extra_handle isn't enable. or use new param.\n");
		return 0;
	}

	rc->v1 = div_s64(cms_adc_read(chip,CMS_BAT_ID),1000);

	for(index = 0; index < chip->r_items_num; index++){
		if(is_between(chip->r_items[index].v1min, chip->r_items[index].v1max, rc->v1) &&
			(chip->r_items[index].id != B_THE_4_SUPPLIER)) {
			bid = chip->r_items[index].id;
			break;
		}
	}

	if (chip->battery_rc.counter == 0 && bid) {
		cms_print(chip, PR_INFO, "extra handle bid = %d \n", bid);
		...
		cms_print(chip,PR_INFO,"state:%s, timestamp:%llu, v1:%lld, v2:%lld, battery_id:%s\n",
			bat_id_det_state_strings[chip->battery_rc.state],
			chip->battery_rc.timestamp, chip->battery_rc.v1,
			chip->battery_rc.v2,chip->bat_id_strings[chip->battery_rc.bat_id]);
		return 1;
	}
	return 0;
}
...
static void cms_bat_id_det_work(struct work_struct *work)
{
	...
	if(chip->bat_det_method == BAT_DET_BY_EX_FG){
		...
		case BAT_ID_START:
			...
			if (cms_batid_extra_handle(chip)) {
				cms_print(chip, PR_INFO, "r detect first, batid exit.\n");
				mutex_unlock(&chip->battid_lock);
				return ;
			}
			...
			schedule_delayed_work(&chip->bat_id_det_work,
				msecs_to_jiffies(BAT_ID_RC_CHARGE_PERIOD_MS-100));
			break;
		case BAT_ID_CHARGING_RC:
			...
			cms_print(chip, PR_INFO, "rc delay_ms %d\n", delay_ms);
	...
	cms_print(chip,PR_INFO,"state:%s, timestamp:%llu, v1:%lld, v2:%lld, battery_id:%s, delay_ms:%u\n",
		bat_id_det_state_strings[chip->battery_rc.state],
		chip->battery_rc.timestamp, chip->battery_rc.v1,
		chip->battery_rc.v2,chip->bat_id_strings[chip->battery_rc.bat_id],delay_ms);
}
...
{
	union power_supply_propval jeita_disable = {0,};
	mutex_lock(&chip->status_lock);
	cms_ensure_psy_available(chip);
	if(chip->enabled ^ enable){
        ...
        if(enable && chip->online && !chip->cms_work_started){
			...
			/*enable hw jeita in AT mode*/
			if (is_atboot && chip->batt_psy) {
				jeita_disable.intval = 0;
				power_supply_set_property(chip->batt_psy, POWER_SUPPLY_PROP_HW_JEITA_DISABLE, &jeita_disable);
				cms_print(chip, PR_INFO, "Enable hw jeita in AT mode\n");
			}
			chip->cms_work_started = true;
            schedule_delayed_work(&chip->cms_work,msecs_to_jiffies(1000));
        }else{
			...
			/*disable hw jeita in AT mode*/
			if (is_atboot && chip->batt_psy) {
				jeita_disable.intval = 1;
				power_supply_set_property(chip->batt_psy, POWER_SUPPLY_PROP_HW_JEITA_DISABLE, &jeita_disable);
				cms_print(chip, PR_INFO, "Disable hw jeita in AT mode\n");
			}
			       ...
			       union power_supply_propval *val)
{
	...
	switch(psp){
		...
		case POWER_SUPPLY_PROP_MODEL_NAME:
			val->strval = chip->bat_id_strings[chip->battery_rc.bat_id];
			break;
		...
		case POWER_SUPPLY_PROP_WEIXIN_CALLING_STATE:
			...
			break;
		case POWER_SUPPLY_PROP_CAMERA4K_STATE:
			val->intval = chip->camera4k_state;
			break;
			       ...
			       const union power_supply_propval *val)
{
	...
	switch(psp){
		...
		case POWER_SUPPLY_PROP_SWITCH_STATE:
			...
			cms_print(chip,PR_INFO,"chip->switch_state = %d\n",chip->switch_state);
			break;
		case POWER_SUPPLY_PROP_CALLING_STATE:
			chip->calling_state = val->intval;
			fuelsummary_collect_value(ID_CALLING, chip->calling_state);
		...
		case POWER_SUPPLY_PROP_WEIXIN_CALLING_STATE:
			...
			break;
		case POWER_SUPPLY_PROP_CAMERA4K_STATE:
			chip->camera4k_state = val->intval;
			cms_print(chip, PR_INFO, "chip->camera4k_state = %d\n", chip->camera4k_state);
			break;
		case POWER_SUPPLY_PROP_FACTORY_MODE_STATE:
			chip->factory_mode_state = val->intval;
			if (chip->batt_psy)
				power_supply_set_property(chip->batt_psy, POWER_SUPPLY_PROP_FACTORY_MODE_STATE, val);
			if(chip->mcu_psy)
				power_supply_set_property(chip->mcu_psy,
					POWER_SUPPLY_PROP_FACTORY_MODE_STATE, val);
			cancel_delayed_work(&chip->cms_work);
			schedule_delayed_work(&chip->cms_work, msecs_to_jiffies(2000));
...
static enum power_supply_property cms_power_props[] = {
	...
	POWER_SUPPLY_PROP_CAMERA4K_STATE,
...
static void cms_external_power_changed(struct power_supply *psy)
{
	...
	cms_print(chip, PR_INFO, "usb_in=%d,dc_in=%d,usb_conn_flag=%d\n", usb_in.intval, dc_in.intval, usb_conn_flag.intval);
	if(!chip->online){
    ...
    if (chip->enabled && chip->online && !chip->cms_work_started) {
		...
		if (bsp_test_mode) {
			...
			schedule_delayed_work(&chip->cms_work, msecs_to_jiffies(500));
		} else
			schedule_delayed_work(&chip->cms_work, msecs_to_jiffies(5000));
...
static long cms_handle_health_status(struct cms *chip)
{
	...
	/*clear no need abnormal status to set 0ma current(disable chg)*/
...
static int cms_check_high_temp_chg_cur_control(struct cms *chip,int batt_temp)
{
	...
	for(i=0; i<3; i++){
		power_supply_get_property(chip->batt_psy,
				POWER_SUPPLY_PROP_VOLTAGE_OCV, &volt_now);
...
usb_id:
			...
			if (chip->usbid_mv  <= chip->usb_id_protect_data[USBID_WATER_TRIGGER]) {
                ...
                chip->usbid_mv = get_usbid_adc();
                if (chip->usbid_mv <= chip->usb_id_protect_data[USBID_WATER_TRIGGER]) {
                        fuelsummary_collect_value(ID_USB_WATER, 1);
                } else {
                        fuelsummary_collect_value(ID_USB_WATER, 0);
                }
	}
...
//	if (chip->continue_count > 10) {
		if (usb_conn_temp.intval >= chip->dchg_ntc_data[USB_CONN_TEMP]) {
			if (chip->usb_conn_temp_high_count >= 1) {	//�ӿ� USB_CONN_NTC �����ٶ�
				batt_scale = 0;
	...
	if (chip->adapter_power_derate_enable) {
		...
		if (adapter_power_derate_already.intval && !chip->adapter_power_derate_param_ready) {
			...
			if (adapter_power_derate.intval != 0) {
				...
				if ((adapter_power.intval & 0x0F) == 0x03) {//33W adapter
					adapter_current = 3400;
				} else if ((adapter_power.intval & 0x0F) == 0x02) {//22.5W adapter
					adapter_current = 3400;
				} else if ((adapter_power.intval & 0x0F) == 0x01) {//44W adapter
					adapter_current = 5000;
				} else if ((adapter_power.intval & 0x0F) == 0x05) {//55W adapter
					adapter_current = 4500;
				} else {
					adapter_current = 3400;
				}
				active_current = adapter_current;
				trigger_temp = ((int)(adapter_power_derate.intval) & 0xff)*10;
				/*read trigger_temp error*/
				if (trigger_temp < 600) {
					if ((adapter_power.intval & 0x0F) == 0x03) {
						...
						//44W adapter
						trigger_temp = 1000;
					} else if((adapter_power.intval & 0x0F) == 0x05) {
						//55W adapter
						trigger_temp = 1000;
					} else {
						trigger_temp = 900;
				...
				cms_print(chip, PR_INFO, "notice: trigger %d, release %d, active %d adapter_current %d\n",
		...
		if (chip->adapter_power_derate_param_ready) {
			...
			cms_print(chip, PR_INFO, "power derate adapter_temp=%d[%d, %d, %d], batt_full_mah=%d, batt_scale=%d(%dmA).  Dchg Adapter is %s.\n",
					adapter_temp.intval, trigger_temp, release_temp, active_current, batt_full_mah.intval, batt_scale, (batt_scale * batt_full_mah.intval / 100),
			...
			cms_print(chip,PR_INFO," adapter_temp=%d[%d, %d, %d], batt_full_mah=%d, batt_scale=%d(%dmA).  Dchg Adapter is %s.\n",
					...
					batt_full_mah.intval, batt_scale, (batt_scale * batt_full_mah.intval / 100), (is_client_vote_enabled_locked(chip->user_ibat_votable, DCHG_ADAPTER_COOLDOWN_CHARGE))  ? "cooldown-ing" : "normal");
		}
	} else {
		power_supply_get_property(chip->mcu_psy,
			POWER_SUPPLY_PROP_ADAPTER_TEMP, &adapter_temp);
		...
		if (!batt_full_mah.intval) {
			power_supply_get_property(chip->batt_psy, POWER_SUPPLY_PROP_BATTERY_CAPACITY_MAH, &batt_full_mah);
		}
...
static int cms_check_abnormal_status(struct cms *chip, int batt_temp, int volt_now, int volt_max, int chg_status_now, int dchg_status_now)
{
	...
	int battid_limit = 0;
	int *tc_rc;
	struct tc_data_item_ex *tc_data;
	...
	switch (chip->tc_type) {
		case NORMAL_TC:
				tc_rc = chip->normal_tc_rc;
				tc_data = chip->normal_tc_data;
			break;
		case EX_NORMAL_TC:
				tc_rc = chip->ex_normal_tc_rc;
				tc_data = chip->ex_normal_tc_data;
			break;
	}

	for(index = 0; index < tc_rc[ROW]; index++){
		if(is_between(tc_data[index].min, tc_data[index].max,
			batt_temp)){
			batt_scale = tc_data[index].data;
			break;
		}
	}
	...
	if(battid_done){
		...
		if (batt_present) {
			if(chip->bat_det_method == BAT_DET_BY_EX_FG && !is_atboot && ((!chip->battery_rc.bat_id && chip->battery_rc.counter > 1) || chip->ex_fg_fake_batt_id_start)) {
				chip->battery_rc.bat_id = cms_get_batid_by_ex_fg(chip);
				//if( chip->battery_rc.bat_id > 0)
					//chip->health_status = 0;
			}
			battid_limit = cms_battery_id_limit(chip, chip->battery_rc.bat_id);
			cms_notify_changed(chip, HEALTH_STATUS_BAT_INVALID,
				chip->battery_rc.bat_id == B_UNKOWN || battid_limit == BAT_ID_LIMIT_ERROR);
...
static int cms_check_abnormal_charge_current(struct cms *chip, int batt_temp)
{
	int batt_scale = 100;
	long health = 0;
	union power_supply_propval usb_temporary_present = {1,};
	...
	health = cms_handle_health_status(chip);
    if ((chip->enabled && ((chip->bat_det_method != BAT_DET_BY_EX_FG || !chip->no_bat_id_keep_charging) && chip->battery_rc.bat_id == B_UNKOWN)) ||
		health) {
		batt_scale = 0;
		usb_temporary_present.intval = 0;
		power_supply_set_property(chip->usb_psy, POWER_SUPPLY_PROP_USB_TEMPORARY_PRESENT, &usb_temporary_present);
	}
...
static int cms_get_step_charge_current(struct cms *chip, int index, struct tc_data_item_ex *tc_data)
{
	union power_supply_propval volt_now = {0,};
	...
	if (!tc_data[index].volt2) {	//two step
		if(vbat_uv <= tc_data[index].volt1){
			if(chip->last_step == 1) {
				batt_scale = tc_data[index].data_ex1;
			}else {
				batt_scale = tc_data[index].data;
				chip->last_step = 0;
			}
		}else if(vbat_uv > tc_data[index].volt1){
			batt_scale = tc_data[index].data_ex1;
			chip->last_step = 1;
		}
	} else {	//three step
		if(vbat_uv <= tc_data[index].volt1){
			if(chip->last_step == 2) {
				batt_scale = tc_data[index].data_ex2;
		...
		}else if(is_between(tc_data[index].volt1, tc_data[index].volt2, vbat_uv)){
			if(chip->last_step == 2) {
				batt_scale = tc_data[index].data_ex2;
			}else {
				batt_scale = tc_data[index].data_ex1;
		...
		}else if(vbat_uv > tc_data[index].volt2){
			if(chip->last_step == 0) {
				batt_scale = tc_data[index].data_ex1;
				chip->last_step = 1;
			}else if(chip->last_step == 1) {
...
static int cms_check_normal_charge_current(struct cms *chip,int batt_temp)
{
	...
	for(index = 0; index < tc_rc[ROW]; index++){
		if(is_between(tc_data[index].min, tc_data[index].max,
			batt_temp)){
			if(!tc_data[index].volt1){	// only one step.
				batt_scale = tc_data[index].data;
				chip->last_step = 0;
			}else{
				batt_scale = cms_get_step_charge_current(chip,index,tc_data);
			}
			break;
		}
	}
...
static int cms_check_camera4k_charge_current(struct cms *chip)
{
	int batt_scale = 0;
	...
	if (!chip->camera4k_scale_enable) {
		cms_print(chip, PR_WARN, "camera4k_scale_enable isn't enabled!\n");
		return 0;
	}

	if (cms_is_voter_priority(chip)) {
		cms_print(chip, PR_WARN, "camera4k_scale isn't priority!\n");
		return 0;
	}

	if (chip->camera4k_state) {
		cms_print(chip, PR_INFO, "camera4k_state scale open........\n");
		vote(chip->user_ibat_votable, CAMERA4K_EVENT, true, chip->camera4k_scale_data[CALLING_IBAT]);
		vote(chip->user_input_votable, CAMERA4K_EVENT, true, chip->camera4k_scale_data[CALLING_INPUT]);
		batt_scale = chip->camera4k_scale_data[CALLING_IBAT];
	} else {
		cms_print(chip, PR_INFO, "camera4k_state scale  close........\n");
		vote(chip->user_ibat_votable, CAMERA4K_EVENT, false, 0);
		vote(chip->user_input_votable, CAMERA4K_EVENT, false, 0);
	}
...
// #define BSPTEST_startchg_soc_thre	45
// #define BSPTEST_stopchg_soc_thre	55
static void cms_check_usbin_cutoff_status(struct cms *chip, int soc)
{
	...
	if ((chip->usbin_cutoff_status != current_status) && current_status) {
...
/**************************
	cms  data processing
***************************/
static void cms_data_process_for_batt_temp(struct cms *chip, int *batt_temp)
{
	static int pre_batt_temp = 0;
	static bool flag = 0;

	if (!chip->dp_batt_temp_enable) {
		cms_print(chip, PR_WARN, "data process batt_temp isn't enabled!\n");
		return;
	}

	if (chip->continue_count == 0) {
		pre_batt_temp = *batt_temp;
		flag = false;
		cms_print(chip, PR_INFO, "init dp batt_temp.\n");
		return;
	}

	if (abs(*batt_temp - pre_batt_temp) >= chip->dp_batt_temp_data) {
		if (!flag) {
			cms_print(chip, PR_INFO, "use pre_batt_temp:%d, now:%d\n", pre_batt_temp, *batt_temp);
			flag = true;
			*batt_temp = pre_batt_temp;
			return;
		}
	}
...
static void cms_main_work(struct work_struct *work)
{
	...
	union power_supply_propval scene = {SCENE_CHG,};
	...
	/* data processing */
	cms_data_process_for_batt_temp(chip, &batt_temp_now.intval);
	...
	if (!chip->direct_charger_enable || chip->continue_count >= 2 || fast_chg_state)//wait fast_chg_state update normal
	...
	cms_check_camera4k_charge_current(chip);
	...
	cms_print(chip, PR_INFO, "vbat=%d,continue_count=%d,nr_work=%d,fb_on=%d,temp=%d,chg_status=%d,usb_type=%d,chg_volt_v=%d,input_scale(%s)=%d,batt_scale(%s)=%d,batid=%d(%d),health=0x%X,batt_soc=%d,bsptest=%d[%d, %d],fast_chg_state=%d,dchg_support=%d\n",
		...
		get_effective_result(chip->user_input_votable), get_effective_client(chip->user_ibat_votable), get_effective_result(chip->user_ibat_votable), chip->battery_rc.bat_id, chip->no_bat_id_keep_charging, (int)chip->health_status, batt_soc.intval, 
		bsp_test_mode, chip->bsptest_soc_range[0], chip->bsptest_soc_range[1], fast_chg_state, dchg_support.intval);
out:
	if(!chip->health_status)
		work_period_ms *= 2;
	...
	fuelsummary_collect_value(ID_SCREEN, chip->fb_on);
	fuelsummary_collect_value(ID_FACTORY, chip->factory_mode_state);
	fuelsummary_collect_value(ID_IN_SCALE, get_effective_result(chip->user_input_votable));
	fuelsummary_collect_value(ID_BAT_SCALE, get_effective_result(chip->user_ibat_votable));

	fuelsummary_collect_data(ID_HEALTH, &(chip->health_status));
	if (chip->fuelsummary_psy)
	...
	mutex_lock(&chip->status_lock);
	if(chip->enabled && chip->online){
		schedule_delayed_work(&chip->cms_work,
...
static void cms_reset_voters(struct cms *chip)
{
	...
	if (chip->direct_charger_enable && bsp_test_mode) {
		if (!chip->dchg_disable_votable)
			chip->dchg_disable_votable = find_votable("DCHG_DISABLE");
		if (chip->dchg_disable_votable) {
			vote(chip->dchg_disable_votable, DEBUGFS_EVENT, false, 0);
...
static const struct file_operations cms_debug_fops = {
				...
				size_t count, loff_t *ppos)
{
	...
	cms_ensure_psy_available(chip);
	if (!chip->batt_psy || (chip->direct_charger_enable && !chip->mcu_psy)) {
		cms_print(chip,PR_ERROR,"bat_psy or mcu_psy not registed\n");
		return ret;
	}
	...
	if(supported_adapter.intval == 2 || supported_adapter.intval == 4) { /* add for half charge test */
		...
		if (!strncmp(buf, "disable-fast-charge", 19)) {
			...
			power_supply_set_property(chip->batt_psy,
				POWER_SUPPLY_PROP_FACTORY_10W_CHARGE_TEST, &value);
			cms_print(chip, PR_ERROR, "9v2a disable-fast-charge\n");
		}
	}
	return ret;
}
...
struct device_node *cms_find_support_board_node(struct cms *chip)
{
	...
	for_each_child_of_node(chip->dev->of_node, node) {
		...
		if (!rc) {
			if (board_version[7] == support_board_version[7]) {
				match_node = node;
...
static struct device_node *cms_find_corresponding_mem_size_node(struct cms *chip)
{
	struct device_node *node, *match_node = NULL;
	int rc = 0, mem_version = 0, ram_sz = 0;

	ram_sz = totalram_pages * 4096 / (1024 * 1024 * 1024);
	/* 4G ram: ram_sz-3, 3G ram: ram_sz=2 */
	if (ram_sz == 3)
		mem_version = 1;
	else if (ram_sz == 2)
		mem_version = 2;

	cms_print(chip, PR_INFO, "mem_diff_config, ram_sz = %d, mem_version = %d\n", ram_sz, mem_version);

	for_each_child_of_node(chip->dev->of_node, node) {
		rc = of_property_read_u32(node, "vivo,mem-version", &chip->mem_version);
		if (rc) {
			chip->mem_version = 0;
			continue;
		}
		if (chip->mem_version == mem_version) {
			match_node = node;
			break;
		}
	}

	cms_print(chip, PR_INFO, "chip_mem_version = %d, match_node = %s\n", chip->mem_version, match_node->name);
...
static int cms_parse_dt(struct cms *chip)
{
	...
	mem_diff_config = of_property_read_bool(chip->dev->of_node, "vivo-diff-mem-config");	//add for PD1957 compatibility
	if (mem_diff_config) {
		cms_print(chip, PR_INFO, "PD1957, check mem diff config\n");
		node = cms_find_corresponding_mem_size_node(chip);
		if (!node)
			node = parent_node;
	} else {
		cms_print(chip, PR_INFO, "Not PD1957, check support board version\n");
	...
	if (chip->bat_det_method == BAT_DET_BY_R ||
		...
		/*use new bat-id rc param*/
		chip->bat_id_new_param_enable = of_property_read_bool(node, "vivo,bat-id-new-param-enable");
	}

	if (chip->bat_id_new_param_enable) {
		cms_print(chip, PR_WARN, "use new rc param\n");
		chip->r_items = new_r_items_param;
		chip->r_items_num = ARRAY_SIZE(new_r_items_param);
		chip->rc_items = new_rc_items_param;
		chip->rc_items_num = ARRAY_SIZE(new_rc_items_param);
		chip->bat_id_strings = new_battery_id_strings;
	} else {
		chip->r_items = r_items_param;
		chip->r_items_num = ARRAY_SIZE(r_items_param);
		chip->rc_items = rc_items_param;
		chip->rc_items_num = ARRAY_SIZE(rc_items_param);
		chip->bat_id_strings = battery_id_strings;
	}

	rc = of_property_read_u32(node, "vivo,bat-id-nums", &chip->bat_id_nums);
	if (!rc && chip->bat_id_nums) {
		chip->bat_id_value = (int *)cms_of_get_property_data(chip, node,
			"vivo,bat-id-value", 1, chip->bat_id_nums, ONE_DIMENSION, "bat_id_value");
		if (!chip->bat_id_value) {
			chip->bat_id_nums = 0;
			cms_print(chip, PR_WARN, "bat-id-value not defined, no used\n");
		}
	} else {
		chip->bat_id_nums = 0;
		cms_print(chip, PR_WARN, "bat-id-nums is not defined, no used\n");
	}

	/*rc = of_property_read_u32(node,"vivo,cms-vadc-chan",&chip->vadc_chan);
	if (rc) {
		chip->vadc_chan = P_MUX3_1_1;
		cms_print(chip,PR_WARN,"vivo,cms-vadc-chan is error,used default chan\n");
	}*/

	chip->batid_pinctrl_enable = of_property_read_bool(node, "vivo,bat-id-pinctrl-enable");

	chip->batid_extra_handle_enable = of_property_read_bool(node, "vivo,batid-extra-handle-enable");
	...
	chip->switch_scale_enable = of_property_read_bool(node, "vivo,switch-scale-enable");
	//fuelsummary_of_property_put("vivo,switch-scale-enable", PARAMS_TYPE_BOOL, &(chip->switch_scale_enable));
	if(chip->switch_scale_enable) {
		chip->switch_state = true;
		chip->switch_scale_data = (int*)cms_of_get_property_data(chip, node,
				"vivo,switch-scale-data", 1, 2, ONE_DIMENSION, NULL);
		if(!chip->switch_scale_data){
			chip->switch_scale_enable = false;
		}
	}
	//fuelsummary_of_property_puts("vivo,switch-scale-data", STATIC_FIXED, PARAMS_TYPE_INT, chip->switch_scale_data, 1, 2);

	chip->calling_scale_enable = of_property_read_bool(node, "vivo,calling-scale-enable");
	fuelsummary_of_property_put("vivo,calling-scale-enable", PARAMS_TYPE_BOOL, &(chip->calling_scale_enable));
	if(chip->calling_scale_enable) {
	...
	fuelsummary_of_property_puts("vivo,calling-scale-data", STATIC_FIXED, PARAMS_TYPE_INT, chip->calling_scale_data, 1, 2);

	chip->weixin_calling_scale_enable = of_property_read_bool(node, "vivo,weixin-calling-scale-enable");
	fuelsummary_of_property_put("vivo,weixin-calling-scale-enable", PARAMS_TYPE_BOOL, &(chip->weixin_calling_scale_enable));
	if (chip->weixin_calling_scale_enable) {
	...
	fuelsummary_of_property_puts("vivo,weixin-calling-scale-data", STATIC_FIXED, PARAMS_TYPE_INT, chip->weixin_calling_scale_data, 1, 2);

	chip->camera4k_scale_enable = of_property_read_bool(node, "vivo,camera4k-scale-enable");
	cms_print(chip, PR_ERROR, "camera4k_scale_enable is %d....\n", chip->camera4k_scale_enable);
	if (chip->camera4k_scale_enable) {
		cms_print(chip, PR_ERROR, "camera4k_scale_enable....\n");
		chip->camera4k_state = false;
		chip->camera4k_scale_data = (int *)cms_of_get_property_data(chip, node,
				"vivo,camera4k-scale-data", 1, 2, ONE_DIMENSION, NULL);
		if (!chip->camera4k_scale_data) {
			chip->camera4k_scale_enable = false;
			cms_print(chip, PR_ERROR, "camera4k_scale_data error....\n");
		}
	}
	...
	if (rc)
		chip->vbat_r_comp = 0;
	fuelsummary_of_property_put("vivo,vbat-r-comp", PARAMS_TYPE_INT, &(chip->vbat_r_comp));
	...
	if(!chip->normal_tc_rc || !chip->normal_tc_data){
		...
		fuelsummary_of_property_puts("vivo,normal-tc-data", STATIC_FIXED, PARAMS_TYPE_INT, chip->normal_tc_data,
				chip->normal_tc_rc[ROW], chip->normal_tc_rc[COL]);
	} else {
		fuelsummary_of_property_puts("vivo,normal-tc-row-col", STATIC_FIXED, PARAMS_TYPE_INT, chip->normal_tc_rc, 1, 2);
		fuelsummary_of_property_puts("vivo,normal-tc-data", EXTENSIBLE, PARAMS_TYPE_INT, &(chip->normal_tc_data),
				chip->normal_tc_rc[ROW], chip->normal_tc_rc[COL]);
	}

	/* ex-normal charge battery param */
	rc = of_property_read_u32(node, "vivo,ex-normal-tc-enable-num", &chip->ex_normal_tc_enable_num);
	if (!rc) {
		chip->ex_normal_tc_limit = (int*)cms_of_get_property_data(chip, node,
					"vivo,ex-normal-tc-limit", 1, chip->ex_normal_tc_enable_num, ONE_DIMENSION, NULL);
		chip->ex_normal_tc_rc = (int*)cms_of_get_property_data(chip, node,
					"vivo,ex-normal-tc-row-col", 1, 2, ONE_DIMENSION, NULL);
		if(chip->ex_normal_tc_rc && chip->ex_normal_tc_limit) {
			chip->ex_normal_tc_data = (struct tc_data_item_ex *)cms_of_get_property_data(chip, node,
						"vivo,ex-normal-tc-data", chip->ex_normal_tc_rc[ROW],
						chip->ex_normal_tc_rc[COL], THREE_DIMENSION, "ex_normal_tc_data");
			//fuelsummary_of_property_puts("vivo,ex-normal-tc-row-col", STATIC_FIXED, PARAMS_TYPE_INT, chip->ex_normal_tc_rc, 1, 2);
			//fuelsummary_of_property_puts("vivo,ex-normal-tc-data", EXTENSIBLE, PARAMS_TYPE_INT, &(chip->ex_normal_tc_data),
			//		chip->ex_normal_tc_rc[ROW], chip->ex_normal_tc_rc[COL]);
		}
		if(!chip->ex_normal_tc_limit || !chip->ex_normal_tc_rc || !chip->ex_normal_tc_data){
			chip->ex_normal_tc_enable_num = 0;
		}
	} else {
		chip->ex_normal_tc_enable_num = 0;
	}
	...
	fuelsummary_of_property_put("vivo,intell-charge-enable", PARAMS_TYPE_BOOL, &(chip->intell_charge_enable));
	if(chip->intell_charge_enable) {
		...
		if(!chip->primary_fbon_tc_rc || !chip->primary_fbon_tc_data){
			...
			fuelsummary_of_property_puts("vivo,primary-fbon-tc-data", STATIC_FIXED, PARAMS_TYPE_INT, chip->primary_fbon_tc_data,
					chip->primary_fbon_tc_rc[ROW], chip->primary_fbon_tc_rc[COL]);
		} else {
			fuelsummary_of_property_puts("vivo,primary-fbon-tc-row-col", STATIC_FIXED, PARAMS_TYPE_INT, chip->primary_fbon_tc_rc, 1, 2);
			fuelsummary_of_property_puts("vivo,primary-fbon-tc-data", EXTENSIBLE, PARAMS_TYPE_INT, &(chip->primary_fbon_tc_data),
					chip->primary_fbon_tc_rc[ROW], chip->primary_fbon_tc_rc[COL]);
		}
		...
		if(!chip->parallel_fbon_tc_rc || !chip->parallel_fbon_tc_data){
			...
			fuelsummary_of_property_puts("vivo,parallel-fbon-tc-data", STATIC_FIXED, PARAMS_TYPE_INT, chip->parallel_fbon_tc_data,
					chip->parallel_fbon_tc_rc[ROW], chip->parallel_fbon_tc_rc[COL]);
		} else {
			fuelsummary_of_property_puts("vivo,parallel-fbon-tc-row-col", STATIC_FIXED, PARAMS_TYPE_INT, chip->parallel_fbon_tc_rc, 1, 2);
			fuelsummary_of_property_puts("vivo,parallel-fbon-tc-data", EXTENSIBLE, PARAMS_TYPE_INT, &(chip->parallel_fbon_tc_data),
					chip->parallel_fbon_tc_rc[ROW], chip->parallel_fbon_tc_rc[COL]);
		}
		...
		if(!chip->primary_fboff_tc_rc || !chip->primary_fboff_tc_data){
			...
			fuelsummary_of_property_puts("vivo,primary-fboff-tc-data", STATIC_FIXED, PARAMS_TYPE_INT, chip->primary_fboff_tc_data,
					chip->primary_fboff_tc_rc[ROW], chip->primary_fboff_tc_rc[COL]);
		} else {
			fuelsummary_of_property_puts("vivo,primary-fboff-tc-row-col", STATIC_FIXED, PARAMS_TYPE_INT, chip->primary_fboff_tc_rc, 1, 2);
			fuelsummary_of_property_puts("vivo,primary-fboff-tc-data", EXTENSIBLE, PARAMS_TYPE_INT, &(chip->primary_fboff_tc_data),
					chip->primary_fboff_tc_rc[ROW], chip->primary_fboff_tc_rc[COL]);
		}
		...
		if(!chip->parallel_fboff_tc_rc || !chip->parallel_fboff_tc_data){
			...
			fuelsummary_of_property_puts("vivo,parallel-fboff-tc-data", STATIC_FIXED, PARAMS_TYPE_INT, chip->parallel_fboff_tc_data,
					chip->parallel_fboff_tc_rc[ROW], chip->parallel_fboff_tc_rc[COL]);
		} else {
			fuelsummary_of_property_puts("vivo,parallel-fboff-tc-row-col", STATIC_FIXED, PARAMS_TYPE_INT, chip->parallel_fboff_tc_rc, 1, 2);
			fuelsummary_of_property_puts("vivo,parallel-fboff-tc-data", EXTENSIBLE, PARAMS_TYPE_INT, &(chip->parallel_fboff_tc_data),
					chip->parallel_fboff_tc_rc[ROW], chip->parallel_fboff_tc_rc[COL]);
		}
	}
...
static int cms_batid_pinctrl_init(struct cms *chip)
{
	int rc = 0;

	if (!chip->batid_pinctrl_enable) {
		cms_print(chip, PR_INFO, "batid pinctrl isn't enable\n");
		return 0;
	}

	chip->pinctrl = devm_pinctrl_get(chip->dev);
	if (IS_ERR_OR_NULL(chip->pinctrl)) {
		cms_print(chip, PR_ERROR, "devm_pinctrl_get error.\n");
		chip->batid_pinctrl_enable = false;
		return -EINVAL;
	}

	/*
	chip->batid_adc_default = pinctrl_lookup_state(chip->pinctrl,  "batid_adc_default");
	if  (IS_ERR_OR_NULL(chip->batid_adc_default))  {
		cms_print(chip,  PR_ERROR,  "batid_adc_default error.\n");
		return -EINVAL;
	}
	ret = pinctrl_select_state(chip->pinctrl,  chip->batid_adc_default);
	*/

	chip->batid_gpio_default = pinctrl_lookup_state(chip->pinctrl, "batid_default");
	if (IS_ERR_OR_NULL(chip->batid_gpio_default)) {
		cms_print(chip, PR_ERROR, "batid_gpio_default error.\n");
		chip->batid_pinctrl_enable = false;
		return -EINVAL;
	}

	chip->batid_gpio_high = pinctrl_lookup_state(chip->pinctrl, "batid_high");
	if (IS_ERR_OR_NULL(chip->batid_gpio_high)) {
		cms_print(chip, PR_ERROR, "batid_high error.\n");
		chip->batid_pinctrl_enable = false;
		return -EINVAL;
	}

	chip->batid_gpio_low = pinctrl_lookup_state(chip->pinctrl, "batid_low");
	if (IS_ERR_OR_NULL(chip->batid_gpio_low)) {
		cms_print(chip, PR_ERROR, "batid_low error.\n");
		chip->batid_pinctrl_enable = false;
		return -EINVAL;
	}

	cms_print(chip, PR_INFO, "batid pinctrl init sucessful!\n");
...
static int cms_probe(struct platform_device *pdev)
{
	...
	cms_create_debugfs_entries(chip);
