...
/* vivo add new function */
void get_votable_info(struct votable *votable)
{
	int i;

	if (!votable) {
		pr_err("votable is null~!\n");
		return;
	}

	pr_err("===== votable name =%s =====\n", votable->name);

	pr_err("effective_result : (%d, %d)\n", votable->effective_client_id, votable->effective_result);

	for (i = 0; i < votable->num_clients; i++) {
		if (votable->client_strs[i]) {
			pr_err("%s:enable=%d,value=%d\n",
					votable->client_strs[i],
					votable->votes[i].enabled,
					votable->votes[i].value);
		}
...
int force_active_votable(struct votable *votable, const char *voter, bool enable, int val)
{
	int rc = 0;
	...
	pr_warn("===== votable name:%s, force voter:%s, (%d, %d) =====\n",votable->name, voter, enable, val);
	votable->force_val = val;
	rc = force_active_set(votable, enable);
	return rc;
}
