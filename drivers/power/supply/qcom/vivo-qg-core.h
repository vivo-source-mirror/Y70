...
struct qg_batt_props {
	...
	int			oem_profile_version;/*vivo add*/
};
...
struct qpnp_qg {
	...
	/*vivo add*/
	struct delayed_work qg_monitor_work;
	bool remove_100_special_map;
	int report_qg_iterm;
	int qg_monitor_work_count;
	/* specified for soc smoothen function */
	struct delayed_work dischg_track_work;
	int dischg_track_work_count;

	/*cms batt id*/
	struct power_supply *cms_psy;
	struct delayed_work check_battery_id_work;
	int check_battid_count;
	struct mutex check_battid_lock;
	int cms_batt_id;
	int fixed_batt_id;
	int batt_capacity_mah;
	struct power_supply *fuelsummary_psy;
	/*chg est vbat diff*/
	int chg_est_vbat_diff;

	/*fifo irq status*/
	struct mutex fifo_irq_lock;

	//battery learning cloud config
	bool battery_learning_cloud_config;

	int tbat_now;
	int custom_cutoff_temperature;
	int custom_cutoff_volt_mv;
	int last_cutoff_volt_mv;
	int aging_support;

	/*chg term current delta*/
	int chg_iterm_delta_ma;

	/*data processing : batt-temp*/
	struct mutex dp_batt_temp_lock;
};
