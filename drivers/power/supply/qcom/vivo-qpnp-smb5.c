...
static int smb5_parse_dt(struct smb5 *chip)
{
	...
	chg->pd1957_compatible_flag = of_property_read_bool(node, "vivo,1957-dual-version-compatible");
	if (chg->pd1957_compatible_flag) {
		pr_info("smb5_parse_dt: enter 1957_compatible situation\n");

		ram_sz = totalram_pages * 4096 / (1024 * 1024 * 1024);
		if (!ram_sz) {
			chg->enable_slave_charger = false;
			chg->hvdcp_disable = true;
			pr_info("failed to get ram_sz\n");
		} else {
			/*Dual charger ram_sz: 3(4-1), single charger ram_sz: 2(3-1) */
			pr_info("ram_sz=%d\n", ram_sz);
			if (ram_sz == 3) {
				chg->enable_slave_charger = true;
				val = HVDCP_EN_BIT;
				pr_info("4G ram, dual charger config\n");
			} else {
				chg->enable_slave_charger = false;
				chg->hvdcp_disable = true;
				val = 0;
				pr_info("3G ram, single charger config\n");
			}
			rc = smblib_masked_write(chg, USBIN_OPTIONS_1_CFG_REG,
				(HVDCP_AUTH_ALG_EN_CFG_BIT | HVDCP_EN_BIT |
				HVDCP_AUTONOMOUS_MODE_EN_CFG_BIT), val);
			if (rc < 0) {
		...
		pr_info("Not PD1957 compatible situation\n");
	}

	/* vivo add */
	chg->pd1965_flag = of_property_read_bool(node, "qcom,pd1965-flag");
	if (chg->pd1965_flag) {
		pr_info("czt Enter\n");
		...
		if (!board_version) {
			...
			if (board_version[7] == '1') {
				chg->enable_slave_charger = true;
				val = HVDCP_EN_BIT;
				pr_info("Detect dual charger board\n");
			} else{
				chg->enable_slave_charger = false;
				chg->hvdcp_disable = true;
				val = 0;
				pr_info("Detect single charger board\n");
			}
			rc = smblib_masked_write(chg, USBIN_OPTIONS_1_CFG_REG,
				(HVDCP_AUTH_ALG_EN_CFG_BIT | HVDCP_EN_BIT |
				HVDCP_AUTONOMOUS_MODE_EN_CFG_BIT), val);
	...
	/*vivo add*/
	fuelsummary_of_property_put("qcom,fcc-max-ua", PARAMS_TYPE_INT, &(chip->dt.batt_profile_fcc_ua));
	...
	fuelsummary_of_property_put("qcom,fv-max-uv", PARAMS_TYPE_INT, &(chip->dt.batt_profile_fv_uv));
	...
	//fuelsummary_of_property_put("qcom,usb-icl-ua", PARAMS_TYPE_INT, &(chip->dt.usb_icl_ua));
	/*vivo end*/
	rc = of_property_read_u32(node,
	...
	if (rc < 0)
			...
			(chip->chg.chg_param.smb_version == PMI632_SUBTYPE)
						? MICRO_1PA : MICRO_3PA;
	fuelsummary_of_property_put("qcom,otg-cl-ua", PARAMS_TYPE_INT, &(chg->otg_cl_ua));

	rc = of_property_read_u32(node, "qcom,chg-term-src",
			&chip->dt.term_current_src);
	if (rc < 0)
		chip->dt.term_current_src = ITERM_SRC_UNSPECIFIED;
	//fuelsummary_of_property_put("qcom,chg-term-src", PARAMS_TYPE_INT, &(chip->dt.term_current_src));

	rc = of_property_read_u32(node, "qcom,chg-term-current-ma",
			&chip->dt.term_current_thresh_hi_ma);
	fuelsummary_of_property_put("qcom,chg-term-current-ma", PARAMS_TYPE_INT, &(chip->dt.term_current_thresh_hi_ma));

	if (chip->dt.term_current_src == ITERM_SRC_ADC)
		rc = of_property_read_u32(node, "qcom,chg-term-base-current-ma",
				&chip->dt.term_current_thresh_lo_ma);
	if (rc < 0)
		pr_info("Couldn't read qcom,chg-term-base-current-ma, rc = %d\n", rc);
	...
	if ((!(chg->pd1965_flag || chg->pd1957_compatible_flag)) ||
		(chg->pd1965_flag && chg->enable_slave_charger && (!chg->hvdcp_disable)) ||
		(chg->pd1957_compatible_flag && chg->enable_slave_charger && (!chg->hvdcp_disable))) {
		chip->dt.hvdcp_disable = of_property_read_bool(node,
							"qcom,hvdcp-disable");
		chg->hvdcp_disable = chip->dt.hvdcp_disable;
		//fuelsummary_of_property_put("qcom,hvdcp-disable", PARAMS_TYPE_BOOL, &(chg->hvdcp_disable));
	}
	...
	chg->disable_suspend_on_collapse = chip->dt.disable_suspend_on_collapse;/*vivo add*/

	/*vivo add*/
	chg->soft_recharge_enable = of_property_read_bool(node,
				"vivo,soft-recharge-enable");
	...
	chg->usbsel_gpio = of_get_named_gpio(node, "vivo,usbsel-gpio", 0);
	...
	//fuelsummary_of_property_put("vivo,direct-charger-enable", PARAMS_TYPE_BOOL, &(chg->direct_charger_enable));

	chg->usboe_gpio = of_get_named_gpio(node, "vivo,usboe-gpio", 0);
	if (chg->usboe_gpio < 0) {
		pr_err("failed to get usboe-gpio.\n");
		chg->usboe_gpio = 0;
	}
	...
	fuelsummary_of_property_put("vivo,tracking_soc_enable", PARAMS_TYPE_BOOL, &(chg->tracking_soc_enable));

	rc = of_property_read_u32(node, "vivo,tracking_step_second",&chg->tracking_step_second);
	if (rc)
		chg->tracking_step_second = 20;
	fuelsummary_of_property_put("vivo,tracking_step_second", PARAMS_TYPE_INT, &(chg->tracking_step_second));

	rc = of_property_read_u32(node, "vivo,cc-force-5v-soc-limit",
						   &(chg->cc_force_5v_soc_limit));
	if (rc) {
		pr_err("Unable to read vivo,cc-force-5v-soc-limit, rc=%d.\n", rc);
		chg->cc_force_5v_soc_limit = 200;
	}
	fuelsummary_of_property_put("vivo,cc-force-5v-soc-limit", PARAMS_TYPE_INT, &(chg->cc_force_5v_soc_limit));

	chg->cc_force_5v_flashled_enable = of_property_read_bool(node,
						"vivo,cc-force-5v-flashled-enable");

	rc = of_property_read_u32(node, "vivo,current-spec-min-ma", &chg->current_spec_min);
	if (rc < 0)
		chg->current_spec_min = max(2000, (((chg->primary_fastchg_max_ua / 1000) + (chg->parallel_fastchg_max_ua / 1000) ) - 1000));
	fuelsummary_of_property_put("vivo,current-spec-min-ma", PARAMS_TYPE_INT, &(chg->current_spec_min));

	rc = of_property_read_u32(node, "vivo,current-spec-max-ma", &chg->current_spec_max);
	if (rc < 0)
		chg->current_spec_max = (chg->primary_fastchg_max_ua / 1000) + (chg->parallel_fastchg_max_ua / 1000) + 100;
	fuelsummary_of_property_put("vivo,current-spec-max-ma", PARAMS_TYPE_INT, &(chg->current_spec_max));
	...
	chg->batt_temp_comp_enable = of_property_read_bool(node,
						"vivo,batt-temp-comp-enable");
	fuelsummary_of_property_put("vivo,batt-temp-comp-enable", PARAMS_TYPE_BOOL, &(chg->batt_temp_comp_enable));
	if (chg->batt_temp_comp_enable) {
		rc = of_property_read_u32(node, "vivo,batt-temp-comp-data", &chg->batt_temp_comp_data);
		if (rc)
			chg->batt_temp_comp_data = 0;
		fuelsummary_of_property_put("vivo,batt-temp-comp-data", PARAMS_TYPE_INT, &(chg->batt_temp_comp_data));
	}

	chg->pl_use_batt_temp_enable =  of_property_read_bool(node,
						"vivo,pl-use-batt-temp-enable");

	chg->pl_enable_gpio = of_get_named_gpio(node, "vivo,pl-enable-gpio", 0);
	if (chg->pl_enable_gpio <= 0) {
		pr_err("pl-enable-gpio not defined\n");
	}
		...
		const union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_USB_CONNDONED:
		...
		break;
	case POWER_SUPPLY_PROP_OTG_VBUS_ENABLE:
		rc = vivo_smblib_notify_usb_host(chg, val->intval);
		break;
	case POWER_SUPPLY_PROP_TYPEC_SWITCH:
...
static int smb5_init_usb_psy(struct smb5 *chip)
{
	...
	usb_cfg.supplied_to = smb5_usb_supplied_to;/* vivo add */
	usb_cfg.num_supplicants = ARRAY_SIZE(smb5_usb_supplied_to);/* vivo add */
	...
	case POWER_SUPPLY_PROP_FLASH_ACTIVE:
		if ((chg->chg_param.smb_version == PMI632_SUBTYPE)
			...
			if (chg->direct_charger_enable) {
				rc = smblib_set_came_notify(chg, val->intval);
				smblib_set_usb_suspend(chg, false);
				if (rc < 0)
					pr_err("Failed to notify camera rc=%d\n",
							rc);
			} else {
				if (pval.intval) {
					...
					if (rc < 0)
						...
						chg->pulse_cnt = 0;
					/* flash switches to inactive, try to force_9v */
					if (!chg->flash_active && !chg->direct_charger_enable) {
						cancel_delayed_work_sync(&chg->force_9v_hvdcp_work);
						schedule_delayed_work(&chg->force_9v_hvdcp_work, msecs_to_jiffies(1500));
						pr_info("FLASH_ACTIVE switches to inactive, force 9v again\n");
					}
				} else {
...
static enum power_supply_property smb5_batt_props[] = {
	...
	POWER_SUPPLY_PROP_FLASHLED_STATE,
		...
		union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_STATUS:
		...
		if((chg->recharge_flag == true || chg->charge_full == true || chg->ffc_charge_full == true) && pval.intval == 100 && chg->ui_soc == 100){
			val->intval = POWER_SUPPLY_STATUS_FULL;
		}else if((chg->recharge_flag == true || chg->charge_full == true || chg->ffc_charge_full == true) && (pval.intval < 100 || chg->ui_soc < 100)){
	...
	case POWER_SUPPLY_PROP_HEALTH:
        ...
        if (chg->hvdcp_type >= HVDCP_TYPE_9V &&
	...
	case POWER_SUPPLY_PROP_CAPACITY:
		/*vivo modify*/
		if(chg->tracking_soc_enable)
			val->intval = smblib_handle_soc(chg);
		else
	...
	case POWER_SUPPLY_PROP_TECHNOLOGY:
		val->intval = POWER_SUPPLY_TECHNOLOGY_LIPO;/*vivo modify*/
		break;
	...
	case POWER_SUPPLY_PROP_CHG_TYPE:
		val->intval = chg->bigdata_chg_type;
		break;
	...
	case POWER_SUPPLY_PROP_CHG_EST_VBAT_DIFF:
		rc = smblib_get_prop_batt_chg_est_vbat_diff(chg, val);
		break;
	...
	case POWER_SUPPLY_PROP_EX_FG_STATE:
		...
		break;
	case POWER_SUPPLY_PROP_FLASHLED_STATE:
		val->intval = (chg->cc_force_5v_flag & CCF5_FLAG_FLASH_LED)?1:0;
		break;
	...
	case POWER_SUPPLY_PROP_AGING_SUPPORT:
		rc = smblib_get_prop_from_bms(chg,
				POWER_SUPPLY_PROP_AGING_SUPPORT, val);
		break;
		...
		const union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_HW_CHG_TIMEOUT:
		...
		break;
	case POWER_SUPPLY_PROP_USB_CONN_PROTECT:
		pr_err("### cc_force_5v_usb_conn_protect %d ###\n", val->intval);
		chg->cc_force_5v_usb_conn_protect = val->intval;
		break;
	case POWER_SUPPLY_PROP_VOLTAGE_MIN:
		if (val->intval >= 3000000 && val->intval <= 3400000)
			chg->custom_cutoff_volt_mv = val->intval;
		else
			chg->custom_cutoff_volt_mv = 3400000;
		break;
	case POWER_SUPPLY_PROP_FLASHLED_STATE:
		pr_err("### flash led state = %d ###\n", val->intval);
		if (chg->cc_force_5v_flashled_enable) {
			if (val->intval)
				rc = smblib_set_chg_vbus_voltage(chg, HVDCP_TYPE_5V);
			else
				rc = smblib_set_chg_vbus_voltage(chg, HVDCP_TYPE_9V);
		} else {
			pr_err("cc_force_5v_flashled isn't enable.\n");
		}
		break;
	/* add for vivo super FlashCharge */
	case POWER_SUPPLY_PROP_SOC_DECIMAL:
	...
	case POWER_SUPPLY_PROP_FACTORY_10W_CHARGE_TEST:
		chg->factory_10W_charge_test_enabe = val->intval;
		rc = force_5v_hvdcp(chg);
		pr_err("### factory_10W_charge_test_enabe = %d, rc = %d ###\n", chg->factory_10W_charge_test_enabe, rc);
		break;
	/* add for closing HW_JETTA in AT-mode */
	case POWER_SUPPLY_PROP_HW_JEITA_DISABLE:
		if (val->intval)
			...
			enum dual_role_property prop, const unsigned int *val)
{
	...
	case DUAL_ROLE_PROP_MODE:
		...
		pr_err("dual_role set property,mode=%d!\n", *val);
...
*/
		//vivo add iterm wirte fail
		rc = smblib_masked_write(chg, CHGR_ADC_ITERM_UP_THD_MSB_REG,
				0xff, buf[0]);
		if (rc < 0)
			pr_err("Couldn't configure CHGR_ADC_ITERM_UP_THD_MSB_REG\n");

		rc = smblib_masked_write(chg, CHGR_ADC_ITERM_UP_THD_LSB_REG,
				0xff, buf[1]);
		if (rc < 0)
			pr_err("Couldn't configure CHGR_ADC_ITERM_UP_THD_LSB_REG\n");

		pr_err("write 0x1067=0x%x, 0x1068=0x%x\n", buf[0], buf[1]);
		//vivo add end
	}
...
static int smb5_init_hw(struct smb5 *chip)
{
	...
	if (!(chg->pd1965_flag || chg->pd1957_compatible_flag)) {
		...
		val = (chg->hvdcp_disable) ? 0 :
			/*(HVDCP_AUTH_ALG_EN_CFG_BIT | HVDCP_EN_BIT);*/ /*vivo modify*/
			(HVDCP_EN_BIT);
	...
	/*vote(chg->fv_votable, HW_LIMIT_VOTER,
		chip->dt.batt_profile_fv_uv > 0, chip->dt.batt_profile_fv_uv);*//*vivo delete*/
	...
	rc = smblib_write(chg, AICL_RERUN_TIME_CFG_REG,
				/*AICL_RERUN_TIME_12S_VAL);*//*vivo modify*/
	...
	/*vivo add*/
	/* set aicl threshold to 4.2v */
	rc = smblib_masked_write(chg, USBIN_5V_AICL_THRESHOLD_CFG_REG,
				 USBIN_5V_AICL_THRESHOLD_CFG_MASK, 0x2);
	if (rc < 0) {
		dev_err(chg->dev,
			"Couldn't USBIN_5V_AICL_THRESHOLD_CFG_REG rc=%d\n",
			rc);
		return rc;
	}

	rc = smblib_masked_write(chg, USBIN_CONT_AICL_THRESHOLD_CFG_REG,
				 USBIN_CONT_AICL_THRESHOLD_CFG_MASK, 0x2);
	if (rc < 0) {
		dev_err(chg->dev,
			"Couldn't USBIN_CONT_AICL_THRESHOLD_CFG_REG rc=%d\n",
			rc);
		return rc;
	}

	/* disable slow plugin timeout irq and vadp irq  */
	...
	/* configure  VPH_TRACK_SEL 100mv*/
	rc = smblib_masked_write(chg, DCDC_VPH_TRACK_SEL_REG, VPH_TRACK_SEL_MASK,
				VPH_TRACK_SEL_100MV);
	if (rc < 0) {
		dev_err(chg->dev, "Couldn't configure DCDC_VPH_TRACK_SEL_REG rc=%d\n",
			rc);
	}

	/*Vsysmin configure 3.2v*/
	rc = smblib_write(chg, SCHG_P_DCDC_VSYSMIN_CFG, VSYSMIN_3P2_V);
	/*vivo end*/
...
static int smb5_post_init(struct smb5 *chip)
{
	...
	/*pval.intval = POWER_SUPPLY_TYPEC_PR_DUAL;*/
	pval.intval = POWER_SUPPLY_TYPEC_PR_SINK;/*vivo modify*/
	rc = smblib_set_prop_typec_power_role(chg, &pval);
	if (rc < 0) {
...
static struct smb_irq_info smb5_irqs[] = {
	...
	[OTG_OC_DISABLE_SW_IRQ] = {
		...
		.handler	= otg_oc_disable_sw_irq_handler,
	},
	[OTG_OC_HICCUP_IRQ] = {
		.name		= "otg-oc-hiccup",
	},
...
/****************************
 *        vivo define function            *
 ****************************/
static int smb5_init_adc(struct smb5 *chip)
{
	struct smb_charger *chg = &chip->chg;
	struct device_node *node = chg->dev->of_node;
	struct iio_channel *chan;
	char *pri_name = "pri_ntc_temp";
	char *pl_name = "pl_ntc_temp";
	int rc = 0;

	/*primary board temp chan*/
	rc = of_property_match_string(node,
					"io-channel-names", pri_name);
	if (rc < 0)
		return 0;

	chan = iio_channel_get(chg->dev, pri_name);
	if (IS_ERR(chan)) {
		rc = PTR_ERR(chan);
		if (rc != -EPROBE_DEFER)
			pr_err("%s channel unavailable, %d\n",
							pri_name, rc);
		chan = NULL;
		return rc;/*pri must be successful!*/
	}
	chg->iio.pri_temp_chan = chan;

	/*parallel board temp chan*/
	rc = of_property_match_string(node,
					"io-channel-names", pl_name);
	if (rc < 0)
		return 0;

	chan = iio_channel_get(chg->dev, pl_name);
	if (IS_ERR(chan)) {
		rc = PTR_ERR(chan);
		if (rc != -EPROBE_DEFER)
			pr_err("%s channel unavailable, %d\n",
	...
	chg->iio.pl_temp_chan = chan;

	return 0;/*pl maybe failed!*/
}
...
static int smb5_probe(struct platform_device *pdev)
{
	...
	pr_info("QPNP SMB5 probed start..\n");
	...
	/*vivo end*/
	mutex_init(&chg->adc_lock);//vivo add
	...
	/* vivo add */
	rc = smb5_init_adc(chip);
	if (rc < 0) {
		if (rc != -EPROBE_DEFER)
			pr_err("Couldn't smb5_init_adc rc=%d\n", rc);
		return rc;
	}
	/* vivo end */
...
free_gpio:
	if (chg->pl_enable_gpio > 0)
		gpio_free(chg->pl_enable_gpio);
	if (chg->usbsel_gpio >= 0)
		gpio_free(chg->usbsel_gpio);
	if (chg->usboe_gpio > 0)
		gpio_free(chg->usboe_gpio);
/*vivo end*/
...
static void smb5_shutdown(struct platform_device *pdev)
{
	...
	if (chg->usbsel_gpio >= 0) {
		...
		pr_info("usbsel gpio set high\n");
	}
	if (chg->pl_enable_gpio > 0) {
		gpio_direction_output(chg->pl_enable_gpio, 1);
		pr_info("pl_enable_gpio set high\n");
	}
	/* vivo end */
