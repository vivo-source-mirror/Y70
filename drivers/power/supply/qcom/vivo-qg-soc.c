...
#include "fg-alg.h"
#include "qg-sdam.h"
#include "vivo-qg-core.h" /*vivo modify*/
#include "qg-reg.h"
#include "qg-util.h"
#include "qg-defs.h"
...
static int qg_process_tcss_soc(struct qpnp_qg *chip, int sys_soc)
{
	...
	if (abs(chip->chg_iterm_ma) >= chip->dt.iterm_ma)
		qg_iterm_ua = (-1 * (abs(chip->chg_iterm_ma) + chip->chg_iterm_delta_ma) * 1000);
	else
		qg_iterm_ua = (-1 * chip->dt.iterm_ma * 1000);

	pr_err("iterm_ma : qg_iterm_ua=%d\n", qg_iterm_ua);
	/*vivo end*/
...
int qg_adjust_sys_soc(struct qpnp_qg *chip)
{
	...
	int backup_sys_soc = chip->sys_soc;
	...
	if(chip->remove_100_special_map){
		...
		if(backup_sys_soc >= QG_MAX_SOC + 80){
			if (chip->chg_iterm_ma == INT_MIN){
				pr_err("ok\n");
				chip->report_qg_iterm = chip->dt.iterm_ma;
			}else{
				pr_err("no\n");
				chip->report_qg_iterm = abs(chip->chg_iterm_ma) + 30;
			}
		}else if (backup_sys_soc <= QG_MAX_SOC - 200){
				chip->report_qg_iterm = chip->dt.iterm_ma;
		}else {
			if(input_present){
				chip->report_qg_iterm = chip->dt.iterm_ma;
		...
		pr_err("chip->sys_soc=%d, chip->chg_iterm_ma=%d, input_present=%d\n", chip->sys_soc, chip->chg_iterm_ma, input_present);
	}else{
		if (chip->sys_soc < 100) {
			/* Hold SOC to 1% of VBAT has not dropped below cutoff */
			rc = qg_get_battery_voltage(chip, &vbat_uv);
	...
	qg_dbg(chip, QG_DEBUG_SOC, "last_adj_sys_soc=%d  adj_sys_soc=%d report_qg_iterm=%d\n",
					chip->last_adj_ssoc, soc, chip->report_qg_iterm);
...
static bool is_scaling_required(struct qpnp_qg *chip)
{
	...
	if (rc >= 0) {
		qg_dbg(chip, QG_DEBUG_SOC, "catch_up_soc %d, msoc %d, batt_curr %d\n", chip->catch_up_soc, chip->msoc, batt_curr);
		if (chip->catch_up_soc < chip->msoc && input_present && batt_curr < 0) {
			/* USB is present and current enter battery, SOC donot allow drop*/
			return false;
		}
	}
	/*vivo end*/
