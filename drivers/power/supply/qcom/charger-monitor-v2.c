...
static void cms_check_usbin_cutoff_status(struct cms *chip, int soc)
{
	...
	if (soc < BSPTEST_startchg_soc_thre) {
		current_status = false;
	} else if (soc > BSPTEST_stopchg_soc_thre) {
		current_status = true;
	} else {
		//return;
		goto OUT;
...
static void cms_main_work(struct work_struct *work)
{
	...
	cms_print(chip, PR_INFO, "vbat=%d,continue_count=%d,nr_work=%d,fb_on=%d,temp=%d,chg_status=%d,usb_type=%d,chg_volt_v=%d,input_scale(%s)=%d,batt_scale(%s)=%d,batid=%d(%d),health=0x%X,batt_soc=%d,bsptest=%d,fast_chg_state=%d,dchg_support=%d\n",
		...
		get_effective_result(chip->user_input_votable), get_effective_client(chip->user_ibat_votable), get_effective_result(chip->user_ibat_votable), chip->battery_rc.bat_id, chip->no_bat_id_keep_charging, (int)chip->health_status, batt_soc.intval, bsp_test_mode, fast_chg_state, dchg_support.intval);
out:
	if(!chip->health_status)
		work_period_ms *= 2;
...
struct device_node *cms_find_support_board_node(struct cms *chip)
{
	...
	for_each_child_of_node(chip->dev->of_node, node) {
		...
		if (!rc) {
			if (!strncmp(support_board_version, "xxxxxx", 6) || !strncmp(support_board_version, "XXXXXX", 6)) {	//default board
				if (match_node == NULL) {
					match_node = node;
			...
			bit_count = 0;
			for (i = 0; i < 6; i++) {	//6bits bbk_board_version
