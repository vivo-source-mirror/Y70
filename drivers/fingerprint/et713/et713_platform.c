...
int egistec_platformFree(struct egis_data *egistec);
...
#else
	wake_lock_timeout(&et713_wake_lock, msecs_to_jiffies(1500));
#endif
	return IRQ_HANDLED;
}
...
{
	disable_irq_nosync(g_data->gpio_irq);
...
int Interrupt_Init(struct egis_data *egis, int int_mode, int detect_period, int detect_threshold)
{
	...
	if (egis->request_irq_done == 0) {
		...
		if (int_mode == EDGE_TRIGGER_RISING) {
			...
			err = request_irq(egis->gpio_irq, fp_eint_func, IRQ_TYPE_EDGE_RISING, "fp_detect-eint", egis);
			if (err)
				DEBUG_PRINT("[egis] request_irq failed==========%s, %d\n", __func__, __LINE__);
		} else if (int_mode == EDGE_TRIGGER_FALLING) {
			DEBUG_PRINT("[egis] %s EDGE_TRIGGER_FALLING\n", __func__);
			err = request_irq(egis->gpio_irq, fp_eint_func, IRQ_TYPE_EDGE_FALLING, "fp_detect-eint", egis);
			if (err)
				DEBUG_PRINT("[egis] request_irq failed==========%s, %d\n", __func__, __LINE__);
		} else if (int_mode == LEVEL_TRIGGER_LOW) {
			DEBUG_PRINT("[egis] %s LEVEL_TRIGGER_LOW\n", __func__);
			err = request_irq(egis->gpio_irq, fp_eint_func_ll, IRQ_TYPE_LEVEL_LOW, "fp_detect-eint", egis);
			if (err)
				DEBUG_PRINT("[egis] request_irq failed==========%s, %d\n", __func__, __LINE__);
		} else if (int_mode == LEVEL_TRIGGER_HIGH) {
			DEBUG_PRINT("[egis] %s LEVEL_TRIGGER_HIGH\n", __func__);
			err = request_irq(egis->gpio_irq, fp_eint_func_ll, IRQ_TYPE_LEVEL_HIGH, "fp_detect-eint", egis);
			if (err)
				DEBUG_PRINT("[egis] request_irq failed==========%s, %d\n", __func__, __LINE__);
		}
		DEBUG_PRINT("[egis] %s: gpio_to_irq return: %d\n", __func__, egis->gpio_irq);
		DEBUG_PRINT("[egis] %s: request_irq return: %d\n", __func__, err);
		fps_ints.drdy_irq_flag = DRDY_IRQ_ENABLE;
		enable_irq_wake(egis->gpio_irq);
		egis->request_irq_done = 1;
	}

	if (fps_ints.drdy_irq_flag == DRDY_IRQ_DISABLE) {
		fps_ints.drdy_irq_flag = DRDY_IRQ_ENABLE;
		enable_irq_wake(egis->gpio_irq);
		enable_irq(egis->gpio_irq);
	}
done:
	return 0;
}
...
static void egis_reset_set_high(struct egis_data *egis)
{
	DEBUG_PRINT("[egis] %s\n", __func__);
	gpio_set_value(egis->rstPin, 1);
}

static void egis_reset_set_low(struct egis_data *egis)
{
	DEBUG_PRINT("[egis] %s\n", __func__);
...
static long egis_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	case FP_RESET_SET_HIGH:
		DEBUG_PRINT("[egis] %s: FP_RESET_SET_HIGH\n", __func__);
		egis_reset_set_high(egis);
		break;

	case FP_RESET_SET_LOW:
		DEBUG_PRINT("[egis] %s: FP_RESET_SET_LOW\n", __func__);
		egis_reset_set_low(egis);
		break;
...
int egistec_platformFree(struct egis_data *egistec)
{
	...
	if (egistec != NULL) {
		if (egistec->request_irq_done == 1) {
			free_irq(egistec->gpio_irq, NULL);
		...
		DEBUG_PRINT("%s free gpio successful.\n", __func__);
	}
...
int egis_platformInit(struct egis_data *egis)
{
	...
	if (!egis->egistec_platformInit_done) {
		...
		if (egis != NULL) {
			...
			if (status < 0) {
				...
				goto egis_platformInit_gpio_init_irq_failed;
			}
			
			status = gpio_request(egis->rstPin, "rst-gpio");
			if (status < 0) {
				DEBUG_PRINT("[egis] %s gpio_request Reset Pin failed\n", __func__);
				goto egis_platformInit_gpio_init_irq_failed;
			}
			status = gpio_direction_output(egis->rstPin, 0);
			if (status < 0) {
				DEBUG_PRINT("[egis] %s gpio_direction_output Reset Pin failed\n", __func__);
				goto egis_platformInit_gpio_init_rst_failed;
			}
			
			status = gpio_request(egis->vcc_33v_Pin, "vcc-gpio");
			if (status < 0) {
				DEBUG_PRINT("[egis] %s gpio_request vcc_33v_Pin failed\n", __func__);
				goto egis_platformInit_gpio_init_rst_failed;
			}
			status = gpio_direction_output(egis->vcc_33v_Pin, 0);
			if (status < 0) {
				DEBUG_PRINT("[egis] %s gpio_direction_output vcc_33v_Pin failed\n", __func__);
				goto egis_platformInit_gpio_init_vcc_failed;
			}

			DEBUG_PRINT("[egis] %s: successful, status = %d\n", __func__, status);
			egis->egistec_platformInit_done = 1;
		}

	} else {
		DEBUG_PRINT("[egis] %s: platform already init \n", __func__);		
	}

	return status;

egis_platformInit_gpio_init_vcc_failed:
	gpio_free(egis->vcc_33v_Pin);
egis_platformInit_gpio_init_rst_failed:
	gpio_free(egis->rstPin);
egis_platformInit_gpio_init_irq_failed:
...
static int egis_parse_dt(struct device *dev, struct egis_data *data)
{
	...
	gpio = of_get_named_gpio(np, "egistec,gpio_reset", 0);
	if (gpio < 0) {
		errorno = gpio;
		goto dt_exit;
	} else {
	...
	gpio = of_get_named_gpio(np, "egistec,gpio_vcc_en", 0);
	if (gpio < 0) {
		errorno = gpio;
		goto dt_exit;
	} else {
		data->vcc_33v_Pin = gpio;
		DEBUG_PRINT("[egis] %s: gpio_vcc_en = %d\n", __func__, data->vcc_33v_Pin);
	}
...
static int egis_remove(struct platform_device *pdev)
{
	...
	if (g_data->request_irq_done == 1)
		free_irq(g_data->gpio_irq, g_data);
	del_timer_sync(&fps_ints.timer);
#if SDM855_EGIS
...
static int egis_probe(struct platform_device *pdev)
{
	...
	egis->egistec_platformInit_done = 0;
	egis->request_irq_done = 0;
	...
	/* platform init */
	//status = egis_platformInit(egis); /* move to egis_open */
