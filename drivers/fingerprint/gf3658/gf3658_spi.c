...
static long gf_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	case GF_IOC_WAKE_LOCK:
		...
		if (retval == 0) {
			wake_lock_timeout(&vivo_wakeup_t.fp_wakelock, msecs_to_jiffies(delay_ms));
    ...
    case GF_IOC_GET_POWER_STATE:
			...
			retval = gf3658_hw_get_power_state(gf_dev);
			break;
	default:
...
static irqreturn_t gf_irq(int irq, void *handle)
{
	...
	if (GF_FF_MODE == vivo_wakeup_t.current_mode) {
		wake_lock_timeout(&vivo_wakeup_t.fp_wakelock, msecs_to_jiffies(WAKELOCK_HOLD_TIME));
...
#endif
{
	...
	wake_lock_init(&vivo_wakeup_t.fp_wakelock, WAKE_LOCK_SUSPEND, "fp_wakelock");
	...
	if (status == 0) {
		...
		for (i = 0; i < ARRAY_SIZE(gf3658_maps); i++)
			input_set_capability(gf_dev->input, gf3658_maps[i].type, gf3658_maps[i].code);
	...
	gf_dev->irq = gf3658_irq_num(gf_dev);
...
#endif
{
	...
	wake_lock_destroy(&vivo_wakeup_t.fp_wakelock);
