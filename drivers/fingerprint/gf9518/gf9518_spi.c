...
#define GF_SPIDEV_NAME		"goodix,udfingerprint"
...
struct gf_key_map gf9518_key_maps[] = {
...
static long gf_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	case GF_IOC_WAKE_LOCK:
		...
		if (retval == 0) {
			...
			pr_info("gf,wake lock delay %d ms\n", delay_ms);
			/*} */
		}
		break;
	/*vivo qishuangcheng add end */
	case GF_IOC_GET_POWER_STATE:
		pr_info("%s: GF_IOC_GET_POWER_STATE ======\n", __func__);
		retval = gf9518_hw_get_power_state(gf_dev);
		break;
	case GF_IOC_REQUEST_SEOURCE:
		pr_info("%s: GF_IOC_REQUEST_SEOURCE ======\n", __func__);
		if (1 == support_soft_fpid) {
			retval = gf9518_request_resource(gf_dev, gf_irq);
		}
		break;
	case GF_IOC_RELEASE_SEOURCE:
		pr_info("%s: GF_IOC_RELEASE_SEOURCE ======\n", __func__);
		if (1 == support_soft_fpid) {
			gf9518_release_resource(gf_dev);
		}
		break;
	default:
...
#endif
{
	...
	wakeup_source_init(&vivo_wakeup_t.fp_wakelock, "fp_wakelock");
	...
	if (0 == support_soft_fpid) {
		status = gf9518_request_resource(gf_dev, gf_irq);
		if (status) {
	...
	if (status == 0) {
		...
		for (i = 0; i < ARRAY_SIZE(gf9518_key_maps); i++)
			input_set_capability(gf_dev->input, gf9518_key_maps[i].type, gf9518_key_maps[i].code);
...
#endif
{
	...
	wakeup_source_trash(&vivo_wakeup_t.fp_wakelock);
...
// extern uint8_t is_support_soft_fpid(void);
...
static int __init gf_init(void)
{
	...
	*/
	...
	if (0 == support_soft_fpid) {
		if ((get_fp_id() != GOODIX_GF9518) && (get_fp_id() != GOODIX_GF9518N)) {
			printk("%s: wrong fp id, not gf9518 driver, exit\n", __func__);
			return 0;
		}
	}
#endif
