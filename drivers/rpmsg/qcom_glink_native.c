...
#define DEFAULT_INTENT_DATA_SIZE	128
				    ...
				    struct glink_channel *channel)
{
	...
	spin_unlock_irqrestore(&glink->idr_lock, flags);
	if (ret < 0) {
		dev_err(glink->dev, "%s failed to idr_alloc_cyclic\n", __func__);
		return ret;
	}
			       ...
			       struct glink_core_rx_intent *intent)
{
	...
	if (glink->intentless) {
		if (intent->size > DEFAULT_INTENT_DATA_SIZE)
			kfree(intent->data);
		#if 0
		kfree(intent->data);
		kfree(intent);
...
static int qcom_glink_rx_defer(struct qcom_glink *glink, size_t extra)
{
	...
	if (!dcmd) {
		dev_err(glink->dev, "%s failed to kmalloc dcmd\n", __func__);
		return -ENOMEM;
	}
...
static int qcom_glink_rx_data(struct qcom_glink *glink, size_t avail)
{
	...
	int ret = 0;
	unsigned long flags;
	struct glink_core_rx_intent m_intent = {0};
	char intent_data[DEFAULT_INTENT_DATA_SIZE] = {0};
	...
	if (glink->intentless) {
		...
		if (!channel->buf) {
			#if 1
			intent = &m_intent;
			if (chunk_size + left_size <= DEFAULT_INTENT_DATA_SIZE)
				intent->data = (void *)&intent_data[0];
			else {
				intent->data = kmalloc(chunk_size + left_size,
					       GFP_ATOMIC);
				if (!intent->data) {
					pr_err("%s: failed to alloc for intent->data\n", __func__);
					return -ENOMEM;
				}
			}
			#else
				     ...
				     size_t avail)
{
	...
	if (!msg) {
		dev_err(glink->dev, "failed to malloc for msg\n");
		return;
	}
	...
	for (i = 0; i < count; ++i) {
		intent = kzalloc(sizeof(*intent), GFP_ATOMIC);
		if (!intent) {
			dev_err(glink->dev, "failed to malloc for intent\n");
			break;
		}
