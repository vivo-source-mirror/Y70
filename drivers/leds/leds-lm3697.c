...
static int lm3697_init_registers(void)
{
	...
	if (is_PD2007)
		lm3697_write_reg(client,  LM3697_CTRL_A_FS_CURR_REG,  0x16,  __func__);
	else
...
static __init int lm3697_get_product_name(char *str)
{
	if (strnstr(str, "PD2007F_EX", PRODUCT_VERSION_LEN))
		is_PD2007 = 1;
	pr_err("dsi lm3697_get_product_name is pd2007F_EX status = %d\n", is_PD2007);
	return 0;
}
early_param("product.version", lm3697_get_product_name);
