...
static char *gadget_uevent[GADGET_TYPE_MAX][2] = {
	[GADGET_CONNECT_DETECT] = {"USB_EXCEPTION=GADGET_CONNECT_DETECT", NULL},
	[GADGET_RESET_DETECT] = {"USB_EXCEPTION=GADGET_RESET_DETECT", NULL},
	[GADGET_EP0_DETECT] = {"USB_EXCEPTION=GADGET_EP0_DETECT", NULL},
};

static void dwc3_uevent_work(struct work_struct *w)
{
	struct dwc3 *dwc = container_of(w, struct dwc3, uevent_work);
	enum gadget_uevent_type type;
	unsigned long flags;

	spin_lock_irqsave(&dwc->lock, flags);
	type = dwc->uevent_type;
	if (type >= GADGET_TYPE_MAX) {
		spin_unlock_irqrestore(&dwc->lock, flags);
		return;
	}
	spin_unlock_irqrestore(&dwc->lock, flags);

	pr_info("%s: %s uevent\n", __func__, gadget_uevent[type][0]);
	dwc3_regdump(dwc);
	kobject_uevent_env(&dwc->dev->kobj,
			   KOBJ_CHANGE, gadget_uevent[type]);
}
#endif
...
static int dwc3_core_soft_reset(struct dwc3 *dwc)
{
	...
	pr_err("dwc3_core_reset:usb parameter separation dwc->host_state=%d\n", dwc->host_state);
	if (dwc->host_state) {
		dwc->usb2_phy->otg_host_state = true;
		dwc->usb3_phy->otg_host_state = true;
	} else {
		dwc->usb2_phy->otg_host_state = false;
		dwc->usb3_phy->otg_host_state = false;
	}
...
#if !defined(VIVO_PROJECT_MODEL)
	INIT_WORK(&dwc->uevent_work, dwc3_uevent_work);
#endif
	dwc->regs	= regs;
...
#if !defined(VIVO_PROJECT_MODEL)
	cancel_work_sync(&dwc->uevent_work);
#endif
