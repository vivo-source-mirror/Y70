...
void dwc3_regdump(struct dwc3 *dwc)
{
	int i, size = 0;
	char *buf;

	if (atomic_read(&dwc->in_lpm)) {
		pr_info("%s: USB device is powered off\n", __func__);
		return;
	}
	...
	for (i = 0; i < ARRAY_SIZE(dwc3_regs); i++) {
		size += scnprintf(buf + size, PAGE_SIZE - size - 1,
				  "%s=0x%08x ", dwc3_regs[i].name,
				  dwc3_readl(dwc->regs, dwc3_regs[i].offset));
		if ((!((i + 1) % 4)) || (i == ARRAY_SIZE(dwc3_regs) - 1)) {
			pr_info("%s: %s\n", __func__, buf);
			size = 0;
		}
	}
...
void dwc3_debugfs_init(struct dwc3 *dwc)
{
	...
	if (IS_ENABLED(CONFIG_USB_DWC3_DUAL_ROLE) ||
		...
		file = debugfs_create_file("vlink_state", S_IRUGO | S_IWUSR,
				root, dwc, &dwc3_link_state_fops);
		if (!file)
			dev_dbg(dwc->dev, "Can't create debugfs vlink_state\n");
#endif
