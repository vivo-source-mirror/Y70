...
static int ep0_detect_time = 5;
module_param(ep0_detect_time, int, S_IRUGO | S_IWUSR);
MODULE_PARM_DESC(ep0_detect_time, "usb device mode ep0 timeout");
#endif
		...
		const struct dwc3_event_depevt *event)
{
	...
	if ((ctrl->bRequestType & USB_RECIP_MASK) == USB_RECIP_DEVICE)
		...
		dwc->dbg_ep0 = false;
	if (dwc->dbg_ep0) {
		dev_info(dwc->dev, "%s: setup packet process start\n", __func__);
		dev_info(dwc->dev, "control req%02x.%02x v%04x i%04x l%d\n",
	...
	if ((ctrl->bRequestType & USB_TYPE_MASK) == USB_TYPE_STANDARD)
		...
		ret = dwc3_ep0_delegate_req(dwc, ctrl);
	if (dwc->dbg_ep0)
		dev_info(dwc->dev, "%s: setup packet process end: %d\n", __func__, ret);
			...
			const struct dwc3_event_depevt *event)
{
...
#if !defined(VIVO_PROJECT_MODEL)
		mod_timer(&dwc->ep0_detect_timer, jiffies + ep0_detect_time * HZ);
#endif
	...
	case EP0_DATA_PHASE:
		if (dwc->dbg_ep0)
			dev_info(dwc->dev, "%s: ep0 state: data\n", __func__);
		dwc3_ep0_complete_data(dwc, event);
		break;

	case EP0_STATUS_PHASE:
		if (dwc->dbg_ep0)
			dev_info(dwc->dev, "%s: ep0 state: status\n", __func__);
