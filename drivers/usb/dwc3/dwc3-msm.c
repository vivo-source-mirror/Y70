...
#define MDWC3_POWER_COLLAPSE		BIT(2)
	...
	/*miaoqiang add for otg*/
	struct wake_lock switchlock;
	struct timer_list otg_disable_timer;
	bool        usbid_adc_used;
    bool        typec_switch_mode;
	int usbid_adc_volt_gpio;
	struct delayed_work usbid_adc_work;
	struct iio_channel  *usbid_adc_chan;
	bool last_id_pin_state;
	bool id_state_change;
	/*miaoqiang add for otg end*/
};
...
static void dwc3_ext_event_notify(struct dwc3_msm *mdwc)
{
	...
	if (mdwc->id_state == DWC3_ID_FLOAT) {
		...
		if (host_mode_enabled && mdwc->id_state_change) {
			mod_timer(&mdwc->otg_disable_timer, jiffies + OTG_DISABLE_DELAY_TIME);
			pr_warn("usb20_host add_timer \n");
		}
	} else {
		...
		if (host_mode_enabled) {
			del_timer_sync(&mdwc->otg_disable_timer);
			pr_warn("usb20_host del_timer \n");
		}
	}
...
static int msm_otg_read_usbid_voltage(struct dwc3_msm *motg, int *temp)
{
	int rc = 0;
	int usbid_mv = USBID_DEFAULT_VALUE_MV;

    if (!motg->usbid_adc_chan) {
		pr_err(" adc channel is null\n");
		*temp = usbid_mv;
		return rc;
	}
    rc = iio_read_channel_processed(motg->usbid_adc_chan, temp);
    if (rc < 0) {
		pr_err("Failed reading usbid_mv over ADC rc=%d\n", rc);
		*temp = usbid_mv;
		return rc;
    }
	*temp /= 1000;
    pr_debug("usbid value in mv = %d\n", *temp);
    return rc;
}
static bool msm_otg_read_usbid_status(struct dwc3_msm *motg)
{
	...
	while (++i <= USBID_READ_RETRIES) {
		rc = msm_otg_read_usbid_voltage(motg, &usbid_mv);
		if (rc < 0) {
			pr_err("Failed reading usbid over ADC rc=%d\n", rc);
		}
		total_mv += usbid_mv;
		msleep(20);
	}
	...
	if (usbid_mv < usbid_otg_triggged_thr_mv)
		...
		usbid_high = true;
	pr_warn("otg_thr_mv=%d usbid_mv=%d usbid_high=%d\n", usbid_otg_triggged_thr_mv, usbid_mv, usbid_high);
	return usbid_high;
}
static void msm_otg_usbid_adc_work(struct work_struct *w)
{
	struct dwc3_msm *motg = container_of(w, struct dwc3_msm,
						usbid_adc_work.work);

	bool id_pin_state = true;
	int delay = 500;

	if (!motg->usbid_adc_used) {
		pr_debug("usbid adc not used\n");
		return;
	}

	/* read otg id pin state  */
	id_pin_state = msm_otg_read_usbid_status(motg);
	motg->id_state = id_pin_state;
	if (id_pin_state == motg->last_id_pin_state) {
		motg->id_state_change = false;
		goto restart;
	} else {
		motg->id_state_change = true;
		pr_debug("otg trigger\n");
		printk(KERN_ALERT "%s otg trigger\n", __func__);
	}

	if (motg->vbus_active && !id_pin_state) {
		pr_err("otg trigger in charging,disable\n");
		goto restart;
	}

	//if (!aca_id_turned_on && changed)
		/*schedule delayed work for 5msec for ID line state to settle*/
		//queue_delayed_work(motg->otg_wq, &motg->id_status_work,
		//		msecs_to_jiffies(MSM_ID_STATUS_DELAY));
	if (motg->id_state_change)
		schedule_work(&motg->resume_work);

restart:
	motg->last_id_pin_state = id_pin_state;
	if (host_mode_enabled)
		...
		struct device_attribute *attr, const char *buf, size_t size)
{
	...
	pm_runtime_resume(motg->dev);
	queue_delayed_work(motg->dwc3_wq, &motg->sm_work, 0);
out:
	return status;
}
		...
		struct device_attribute *attr, char *buf)
{
    ...
    } else if (motg->typec_switch_mode) {
		if (!motg->usb_psy) {
			motg->usb_psy = power_supply_get_by_name("usb");
			if (!motg->usb_psy) {
				pr_info("Could not get usb power_supply\n");
				return -EINVAL;
			}
		}
...
{
	struct dwc3_msm *motg = (struct dwc3_msm *)data;
	host_mode_enabled = false;
	cancel_delayed_work(&motg->usbid_adc_work);
	/*set_bit(ID, &motg->inputs);*/
	if (motg->usbid_adc_used && motg->usbid_adc_volt_gpio)
		gpio_direction_output(motg->usbid_adc_volt_gpio, 0);
	motg->id_state = DWC3_ID_FLOAT;
	if (wake_lock_active(&motg->switchlock))
		wake_unlock(&motg->switchlock);
	pr_warn("add otg_disable_func work! \n");
	printk(KERN_ALERT "%s work!\n", __func__);
	/*if(motg->drd_state == DRD_STATE_PERIPHERAL)
	//		return;
	//pm_runtime_resume(motg->dev);
	//queue_delayed_work(motg->otg_wq, &motg->sm_work,0);*/
}

static void host_mode_enable(struct dwc3_msm *motg, bool enabled)
{
	...
	if (host_mode_enabled ^ enabled) {
		...
		if (enabled) {
			if (motg->usbid_adc_used) {
				...
				motg->id_state_change = false;
				schedule_delayed_work(&motg->usbid_adc_work, msecs_to_jiffies(500));
				if (motg->usbid_adc_volt_gpio)
					gpio_direction_output(motg->usbid_adc_volt_gpio, 1);

				init_timer(&motg->otg_disable_timer);
				motg->otg_disable_timer.expires = jiffies + OTG_DISABLE_DELAY_TIME;
				motg->otg_disable_timer.function = otg_disable_func;
				motg->otg_disable_timer.data = (unsigned long)motg;

				add_timer(&motg->otg_disable_timer);
				printk(KERN_ALERT " %s usb phy otg init otg_switch_disable_timer!\n", __func__);

				id_state = msm_otg_read_usbid_status(motg);
				if (!id_state) {
					clear_bit(ID, &motg->inputs);
					pr_warn("ID clear\n");
			...
			if (motg->usbid_adc_used) {
			    ...
			    del_timer_sync(&motg->otg_disable_timer);
			    pr_warn("usb phy otg del otg_switch_disable_timer \n");
			    printk(KERN_ALERT "%s usb phy otg del otg_switch_disable_timer!\n", __func__);
				if (wake_lock_active(&motg->switchlock))
				    wake_unlock(&motg->switchlock);
		...
		struct device_attribute *attr, const char *buf, size_t size)
{
	...
	printk(KERN_ALERT " add otg_host_mode_enable_store start! \n");

	if (!motg) {
		return -ENODEV;
	}
	if (motg->usbid_adc_used) {
		printk(KERN_ALERT "otg_host_mode_enable_store start in  ADC MODE! \n");
		if (sscanf(buf, "%s", value) != 1) {
			return -EINVAL;
		}
		if (!strncmp(value, "enabled", 6)) {
			pr_info("otg enabled \n");
			printk(KERN_ALERT "%s otg enabled!\n", __func__);
			host_mode_enable(motg, true);
		} else if (!strncmp(value, "disabled", 6)) {
			pr_info("otg disabled \n");
			printk(KERN_ALERT "%s otg disabled!\n", __func__);
			host_mode_enable(motg, false);
		} else {
			return -EINVAL;
		}
	} else if (motg->typec_switch_mode) {
		printk(KERN_ALERT "otg_host_mode_enable_store start in  TYPEC MODE! \n");
		if (!motg->usb_psy) {
			motg->usb_psy = power_supply_get_by_name("usb");
			if (!motg->usb_psy) {
				pr_info("Could not get usb power_supply\n");
...
static ssize_t usb_type_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t size)
{
	return -EPERM;
}
...
static DEVICE_ATTR(usb_type, S_IRUGO | S_IWUSR, usb_type_show, usb_type_store);

/* vivo add end */

static ssize_t usbid_value_enable_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	ssize_t size = 0;
	struct dwc3_msm *motg;
	...
	if (!motg->usbid_adc_used) {
		pr_debug("usbid adc not used\n");
		size = snprintf(buf, PAGE_SIZE, "%s", "1800\n");
		return size;
	}
    if (motg->usbid_adc_volt_gpio)
		gpio_direction_output(motg->usbid_adc_volt_gpio, 1);
...
static ssize_t usbid_value_enable_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t size)
{
	char value[32];
	...
	printk(KERN_ALERT "%s: write usbid_mv value %s\n", __func__, value);

	return size;
}


static DEVICE_ATTR(usbid_value, S_IRUGO | S_IWUSR, usbid_value_enable_show, usbid_value_enable_store);
...
static int dwc3_msm_probe(struct platform_device *pdev)
{
	...
	INIT_DELAYED_WORK(&mdwc->usbid_adc_work, msm_otg_usbid_adc_work);
	...
	mdwc->usbid_adc_used = of_property_read_bool(node,
		"qcom,usbid_adc_used");
	if (mdwc->usbid_adc_used)
		pr_err("%s:current otg use usb id pin\n", __func__);

	mdwc->typec_switch_mode = of_property_read_bool(node,
		"qcom,typec_switch_mode");
    if (mdwc->typec_switch_mode)
		pr_err("%s:current use typec otg\n", __func__);
	...
	device_create_file(&pdev->dev, &dev_attr_usbid_value);
	device_create_file(&pdev->dev, &dev_attr_usb_type);
	wake_lock_init(&mdwc->switchlock, WAKE_LOCK_SUSPEND, "otg_switch");
	
	if (mdwc->usbid_adc_used) {
		mdwc->usbid_adc_volt_gpio = of_get_named_gpio(node, "qcom,usbidvol-gpio", 0);
		if (mdwc->usbid_adc_volt_gpio <= 0)
			pr_err("usbidvol-gpio is not available\n");
		mdwc->usbid_adc_chan = iio_channel_get(&pdev->dev, "usbid_pin");
		if (IS_ERR(mdwc->usbid_adc_chan)) {
			ret = PTR_ERR(mdwc->usbid_adc_chan);
			if (ret != -EPROBE_DEFER) {
				pr_err("usbid_adc_chan channel unavailable, ret=%d\n", ret);
				mdwc->usbid_adc_chan = NULL;
			}
		}
	}
	pr_info("otg host mode disabled by default usbid_adc_volt_gpio=%d\n",
			mdwc->usbid_adc_volt_gpio);
	if (mdwc->usbid_adc_volt_gpio)
		gpio_direction_output(mdwc->usbid_adc_volt_gpio, 0);
	host_mode_enabled = false;
	return 0;
...
static int dwc3_otg_start_host(struct dwc3_msm *mdwc, int on)
{
	...
	if (mdwc->usbid_adc_used) {
		...
		if (!usb_psy) {
			dev_err(mdwc->dev, "unable to get usb power supply\n");
			return -ENODEV;
		}
	}
	...
	if (on) {
		...
		if (mdwc->usbid_adc_used) {
			pval.intval = true;
			dev_err(mdwc->dev, "start to enable vbus_reg\n");
		...
		if (ret) {
			...
			if (!IS_ERR_OR_NULL(mdwc->vbus_reg))
				regulator_disable(mdwc->vbus_reg);
			pval.intval = false;
			dev_err(mdwc->dev, "start to disable vbus_reg\n");
			ret = power_supply_set_property(usb_psy,
						POWER_SUPPLY_PROP_OTG_VBUS_ENABLE, &pval);
			if (ret) {
		...
		if (mdwc->usbid_adc_used) {
			pval.intval = false;
			dev_err(mdwc->dev, "start to enbale vbus_reg\n");
			ret = power_supply_set_property(usb_psy,
					POWER_SUPPLY_PROP_OTG_VBUS_ENABLE, &pval);
			if (ret) {
				dev_err(mdwc->dev, "unable to disable vbus_reg\n");
