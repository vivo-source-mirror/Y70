...
static int connect_detect_time = 15;
module_param(connect_detect_time, int, S_IRUGO | S_IWUSR);
MODULE_PARM_DESC(connect_detect_time, "usb device mode connect timeout");
#endif
...
#if !defined(VIVO_PROJECT_MODEL)
	del_timer(&dwc->connect_detect_timer);
	del_timer(&dwc->ep0_detect_timer);
#endif
}
...
#if !defined(VIVO_PROJECT_MODEL)
	if (!timer_pending(&dwc->connect_detect_timer))
		mod_timer(&dwc->connect_detect_timer,
			jiffies + connect_detect_time * HZ);

	if (dwc->gadget.state == USB_STATE_CONFIGURED) {
		if (!dwc->b_suspend) {
			dwc->uevent_type = GADGET_RESET_DETECT;
			schedule_work(&dwc->uevent_work);
		}
	}
#endif
	...
	case DWC3_DEVICE_EVENT_EOPF:
		...
		if (dwc->revision >= DWC3_REVISION_230A) {
			...
			if (dwc->gadget.state >= USB_STATE_CONFIGURED)
				...
				usb_gadget_vbus_draw(&dwc->gadget, 100);
				/* miaoqiang add for from 2ma->100ma, for charger disconnect */
		}
		break;
...
#if !defined(VIVO_PROJECT_MODEL)
static void dwc3_connect_detect_func(unsigned long data)
{
	struct dwc3 *dwc = (struct dwc3 *)data;
	unsigned long flags;

	spin_lock_irqsave(&dwc->lock, flags);
	dwc->uevent_type = GADGET_CONNECT_DETECT;
	schedule_work(&dwc->uevent_work);
	spin_unlock_irqrestore(&dwc->lock, flags);
}

static void dwc3_ep0_detect_func(unsigned long data)
{
	struct dwc3 *dwc = (struct dwc3 *)data;
	unsigned long flags;

	spin_lock_irqsave(&dwc->lock, flags);
	dwc->uevent_type = GADGET_EP0_DETECT;
	schedule_work(&dwc->uevent_work);
	spin_unlock_irqrestore(&dwc->lock, flags);
}
#endif
...
#if !defined(VIVO_PROJECT_MODEL)
	init_timer(&dwc->connect_detect_timer);
	dwc->connect_detect_timer.function = dwc3_connect_detect_func;
	dwc->connect_detect_timer.data = (unsigned long)dwc;

	init_timer(&dwc->ep0_detect_timer);
	dwc->ep0_detect_timer.function = dwc3_ep0_detect_func;
	dwc->ep0_detect_timer.data = (unsigned long)dwc;
#endif
...
#if !defined(VIVO_PROJECT_MODEL)
	del_timer_sync(&dwc->connect_detect_timer);
	del_timer_sync(&dwc->ep0_detect_timer);
#endif
	dwc3_gadget_free_endpoints(dwc);
	dma_free_coherent(dwc->sysdev, DWC3_BOUNCE_SIZE, dwc->bounce,
			  dwc->bounce_addr);
