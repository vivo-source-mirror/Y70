...
struct usbpd {
	...
	/*vivo add*/
	bool			manual_change_Rp_to_Rd_WA;	/*add for OTG test*/
};
...
static void usbpd_sm(struct work_struct *w)
{
	...
	if (pd->current_pr == PR_NONE) {
		...
		/*vivo add*/
		if (pd->typec_mode == POWER_SUPPLY_TYPEC_SINK && pd->manual_change_Rp_to_Rd_WA) {
			usbpd_info(&pd->dev, "manual_change_Rp_to_Rd_WA = false.\n");
			pd->manual_change_Rp_to_Rd_WA = false;
	...
	case PE_SNK_READY:
		if (IS_DATA(rx_msg, MSG_SOURCE_CAPABILITIES)) {
			...
			/*miaoqiang add otg switch is not enabled, donot in host_mode*/
			ret = power_supply_get_property(pd->usb_psy,
					POWER_SUPPLY_PROP_TYPEC_SWITCH, &val);
			if (!ret && val.intval == 1) {

				usbpd_err(&pd->dev, "Otg switch is not enabled, go out host mode\n");
				break;
			}
			/*miaoqiang add otg switch is not enabled, donot in host_mode*/
...
static int psy_changed(struct notifier_block *nb, unsigned long evt, void *ptr)
{
	...
	case POWER_SUPPLY_TYPEC_NONE:
		pd->manual_change_Rp_to_Rd_WA = false;	/*vivo add*/
		if (pd->in_pr_swap) {
	...
	case POWER_SUPPLY_TYPEC_SINK:
		...
		if (pd->current_pr == PR_SRC)
			return 0;
		/*vivo add*/
		/*manual change the Rp to Rd which not follow the PR_Swap process*/
		if (pd->current_pr == PR_SINK && typec_mode == POWER_SUPPLY_TYPEC_SINK) {
			usbpd_info(&pd->dev, "manual_change_Rp_to_Rd_WA = true.\n");
			pd->manual_change_Rp_to_Rd_WA = true;
			pd->current_pr = PR_NONE;
			break;
		}
		/*vivo end*/
