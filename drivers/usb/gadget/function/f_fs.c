			       ...
			       bool config0)
{
	...
	case USB_RECIP_ENDPOINT:
		idx = ffs_func_revmap_ep(func, le16_to_cpu(creq->wIndex));
		if (idx < 0)
			return false;
		dir = le16_to_cpu(creq->wIndex) & USB_ENDPOINT_DIR_MASK;
		return ((func->ffs->eps_addrmap[idx] & USB_ENDPOINT_DIR_MASK) == dir);
	default:
