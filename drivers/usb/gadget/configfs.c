...
#include <linux/power_supply.h>/*vivo add*/
#include "configfs.h"
#include "u_f.h"
#include "u_os_desc.h"
	...
	struct config_item *usb_func_ci)
{
	...
	pr_info("%s: func_instance is %s", __func__, fi->fd->name);
	...
	ret = 0;
out:
	pr_info("%s: func is %s: %d", __func__, !ret ? f->name : "null", ret);
	mutex_unlock(&gi->lock);
	return ret;
}
	...
	struct config_item *usb_func_ci)
{
	...
	list_for_each_entry(f, &cfg->func_list, list) {
		if (f->fi == fi) {
			pr_info("%s: func is %s", __func__, f->name);
			list_del(&f->list);
			usb_put_function(f);
			mutex_unlock(&gi->lock);
			return;
...
#ifdef CONFIG_USB_CONFIGFS_UEVENT
/*vivo add*/
static void usb_enumerate_conndoned(void)
{
	...
	if (usb_psy) {
		pr_err("%s:##### usb connect successfully! #####\n", __func__);
		pval.intval = true;
		power_supply_set_property(usb_psy,
				POWER_SUPPLY_PROP_USB_CONNDONED, &pval);
	} else {
		pr_err("%s:##### usb psy null #####\n", __func__);
	}
}
/*vivo end*/
			...
			const struct usb_ctrlrequest *c)
{
	...
	list_for_each_entry(fi, &gi->available_func, cfs_list) {
		if (fi != NULL && fi->f != NULL && fi->f->setup != NULL) {
			...
			if (value >= 0) {
				pr_info("%s: %s fi->f->setup: %d\n",
					__func__, fi->f->name, value);
				break;
			}
		}
	}
	 ...
	 */
	if (value == 0) {
		pr_info("%s: ncm ncm_ctrlrequest: %d\n",
			__func__, value);
		return value;
	}
#endif
	...
	if (value < 0) {
		...
		if (value >= 0)
			pr_info("%s: acc acc_ctrlrequest: %d\n", __func__, value);
	}
#endif
