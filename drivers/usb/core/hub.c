...
static bool disable_lpm_for_usb2 = 1;
module_param(disable_lpm_for_usb2, bool, S_IRUGO | S_IWUSR);
MODULE_PARM_DESC(disable_lpm_for_usb2,
		 "Disable LPM for USB2");
...
loop:
		...
		if (i == (SET_CONFIG_TRIES / 2))
			usb_host_uevent(udev, HOST_ENUM_DETECT);
#endif
		usb_ep0_reinit(udev);
		release_devnum(udev);
		hub_free_dev(udev);
...
int usb_reset_device(struct usb_device *udev)
{
	...
	usb_unlock_port(port_dev);
#if !defined(VIVO_PROJECT_MODEL)
	if (reset_detect && ret != -ENOTCONN &&
	    ret != -ENODEV && ret != -EINVAL)
		usb_host_uevent(udev, HOST_RESET_DETECT);
#endif
