...
static unsigned int tunes[5];
module_param_array(tunes, uint, NULL, 0644);
MODULE_PARM_DESC(tunees, "QUSB PHY TUNES");
...
static void qusb_phy_get_tune2_param(struct qusb_phy *qphy)
{
	...
	if (qphy->phy.otg_host_state) {
		if (qphy->tune2_efuse_correction_host && qphy->tune2_val) {
			if (qphy->tune2_efuse_correction_host > 5 ||
					qphy->tune2_efuse_correction_host < -10)
		...
		pr_debug("%s(): usb parameter separation @usb host paragram(tune2:%d efuse:%d)\n", 
				__func__, qphy->tune2_val, qphy->tune2_efuse_correction_host);
	} else {
		...
		pr_debug("%s(): usb parameter separation @usb device paragram(tune2:%d efuse:%d)\n",
				__func__, qphy->tune2_val, qphy->tune2_efuse_correction);
	}
...
static int qusb_phy_init(struct usb_phy *phy)
{
	...
	if (qphy->emulation) {
		...
		if (qphy->phy.otg_host_state) {
			...
			pr_debug("qusb_phy_init:usb parameter separation @usb host paragram \n");
		} else if (qphy->qusb_phy_init_seq) {
			qusb_phy_write_seq(qphy->base, qphy->qusb_phy_init_seq,
					qphy->init_seq_len, 0);
			pr_debug("qusb_phy_init:usb parameter separation @usb device paragram \n");
		}
	...
	tunes[0] = readl_relaxed(qphy->base + QUSB2PHY_PORT_TUNE1);
	tunes[1] = readl_relaxed(qphy->base + QUSB2PHY_PORT_TUNE2);
	tunes[2] = readl_relaxed(qphy->base + QUSB2PHY_PORT_TUNE3);
	tunes[3] = readl_relaxed(qphy->base + QUSB2PHY_PORT_TUNE4);
	tunes[4] = readl_relaxed(qphy->base + QUSB2PHY_PORT_TUNE5);

	pr_debug("%s: (mode: %d)TUNE1:%08x TUNE2:%08x TUNE3:%08x TUNE4:%08x TUNE5:%08x\n", __func__, qphy->phy.otg_host_state,
		tunes[0], tunes[1], tunes[2], tunes[3], tunes[4]);
...
static int qusb_phy_probe(struct platform_device *pdev)
{
	...
	if (res) {
		...
		if (!IS_ERR_OR_NULL(qphy->tune2_efuse_reg)) {
			...
			of_property_read_u32(dev->of_node,
						...
						&qphy->tune2_efuse_correction);
			of_property_read_u32(dev->of_node,
						"qcom,tune2-efuse-host-correction",
						&qphy->tune2_efuse_correction_host);
	...
	/* add by wutianwen for usb paragram separation*/
	size = 0;
