...
static bool is_lcdb_enabled(struct qpnp_lcdb *lcdb)
{
	...
	pr_err("lcdb_reg val 0x%02x\n", val);

	return rc ? false : !!(val & MODULE_EN_BIT);
}

bool check_lcdb_reg(void)
{
	return is_lcdb_enabled(g_lcdb);
}
EXPORT_SYMBOL(check_lcdb_reg);
...
int dump_full_lcdb_registers(void)
{
	int rc, i = 0;
	u8 reg = 0;
	struct qpnp_lcdb *lcdb = g_lcdb;
	if (lcdb == NULL)
		return -EDQUOT;
	for (i = 0; i <= 0xFF; i++) {
		rc = qpnp_lcdb_read(lcdb, lcdb->base + i, &reg, 1);
		if (rc < 0)
			pr_err("Failed to read lcdb register rc=%d\n", rc);
		else
			pr_err("[0x%x]=0x%02x\n", lcdb->base + i, reg);
	}
...
EXPORT_SYMBOL(dump_full_lcdb_registers);
...
void qpnp_lcdb_set_force_pd_enable_wa(int enable)
{
	if (!g_lcdb || g_lcdb->pmic_rev_id->pmic_subtype != PMI632_SUBTYPE)
		return;

	if (enable)
		g_lcdb->wa_flags |= FORCE_PD_ENABLE_WA;
	else
		g_lcdb->wa_flags &= ~FORCE_PD_ENABLE_WA;
}
