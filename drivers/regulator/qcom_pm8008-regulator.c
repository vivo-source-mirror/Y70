...
static int pm8008_regulator_enable(struct regulator_dev *rdev)
{
	...
	pm8008_err(pm8008_reg,
		"failed to enable regulator VREG_READY not set and status reg = 0x%02x\n", reg);
out:
						...
						const char *name)
{
	...
	/*vivo huangchen add to set debounce time from 240us to 960us begin*/
	...
	/*vivo huangchen add to set debounce time from 240us to 960us end*/
...
static int pm8008_chip_probe(struct platform_device *pdev)
{
	...
	if (gpio_is_valid(rst_data.reset_gpio)) {
		rc = gpio_request(rst_data.reset_gpio,
				"reset_gpio");
		if (rc) {
