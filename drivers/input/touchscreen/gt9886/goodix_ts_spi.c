...
#define TS_DOZE_DISABLE_RETRY_TIMES	3
		...
		int touch_num)
{
	...
	for (i = 0; i < touch_num; i++) {
		...
		if (bdata->x2x){
			panel_max_x = VTS_PROPERTY_DIMENTION_X;
			coords->x = panel_max_x - coords->x;
		}
		if (bdata->y2y){
			panel_max_y = VTS_PROPERTY_DIMENTION_Y;
...
static int goodix_hw_suspend(struct goodix_ts_device *dev)
{
	...
	int r = 0;
	if(core_data->charge_sta){//in charge state ,enter into sleep mode instead of power off
...
static int goodix_hw_resume(struct goodix_ts_device *dev)
{
	...
	struct goodix_ts_core *core_data = platform_get_drvdata(goodix_pdev);
	u8 temp_buf[256], checksum;
	const struct firmware *temp_firmware = NULL;
		...
		if (dev->reg.version_base && dev->reg.version_len < 256) {
			...
			if (!checksum) {
				...
				if(0x00 == temp_buf[17] && 
					...
					0x01 == temp_buf[20] ){
					VTE("Need upgrade FW");
					r = request_firmware(&temp_firmware, "TP-FW-PD2067F_EX-LCMID33-VER0x606040023.bin", dev->dev);
					if (r) {
						VTE("%s: request_firmware failed, ret %d", __func__, r);
						goto exit;
					}
					r =  bbk_goodix_fw_update_V2(temp_firmware);
				}
				break;
			}
		}
...
static int __init goodix_gt9885_init(void)
{
	VTI("goodix_gt9885_init spi add driver !");
	if (spi_register_driver(&goodix_spi_driver))
		return -1;
...
static void __exit goodix_gt9885_exit(void)
{
	VTI("goodix_gt9885_exit spi del driver !");
...
module_init(goodix_gt9885_init);
module_exit(goodix_gt9885_exit);
