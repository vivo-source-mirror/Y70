...
int goodix_cfg_bin_proc_V2(struct goodix_ts_core *core_data, const struct firmware *firmware)
{
	...
	r =  bbk_goodix_fw_update_V2(firmware);
	if (r < 0) {
...
int goodix_read_cfg_bin_V2(struct goodix_ts_core *coredata, struct device *dev, struct goodix_cfg_bin *cfg_bin)
{
	...
	r = request_firmware(&firmware,"TP-CONFIG-FW-PD2038-LCMID32-VER0x0022.bin", dev);
	if(r < 0){
		VTE(" request_firmware config fail");
		goto exit;
	}
