...
/*0:Header update; 1:request firmware update*/
atomic_t fw_update_mode_V2 = ATOMIC_INIT(0);
...
static int goodix_parse_firmware(struct firmware_data *fw_data)
{
	...
	if (firmware->size < FW_SUBSYS_INFO_OFFSET) {
		VTE("Invalid firmware size:%zu", firmware->size);
		r = -EINVAL;
		goto err_size;
	}
	...
	if (firmware->size != fw_info->size + 6) {
		VTE("Bad firmware, size not match");
		r = -EINVAL;
		goto err_size;
	}
	...
	if (checksum != fw_info->checksum) {
		VTE("Bad firmware, cheksum error");
		r = -EINVAL;
		goto err_size;
	}

	if (fw_info->subsys_num > FW_SUBSYS_MAX_NUM) {
		VTE("Bad firmware, invalid subsys num: %d",
		       fw_info->subsys_num);
		r = -EINVAL;
		goto err_size;
	}
	...
	for (i = 0; i < fw_info->subsys_num; i++) {
		...
		if (fw_offset > firmware->size) {
			VTE("Sybsys offset exceed Firmware size");
			r = -EINVAL;
			goto err_size;
		}
	...
	VTI("Firmware package protocol: V%u", fw_info->protocol_ver);
	VTI("Fimware PID:GT%s", fw_info->fw_pid);
	VTI("Fimware VID:%02X%02X%02X%02X", fw_info->fw_vid[0],
				fw_info->fw_vid[1], fw_info->fw_vid[2], fw_info->fw_vid[3]);
	VTI("Firmware chip type:%02X", fw_info->chip_type);
	VTI("Firmware size:%u", fw_info->size);
	VTI("Firmware subsystem num:%u", fw_info->subsys_num);
#ifdef CONFIG_GOODIX_DEBUG
	for (i = 0; i < fw_info->subsys_num; i++) {
		VTI("------------------------------------------");
		VTI("Index:%d", i);
		VTI("Subsystem type:%02X", fw_info->subsys[i].type);
		VTI("Subsystem size:%u", fw_info->subsys[i].size);
		VTI("Subsystem flash_addr:%08X", fw_info->subsys[i].flash_addr);
		VTI("Subsystem Ptr:%p", fw_info->subsys[i].data);
	}
	VTI("------------------------------------------");
#endif
		...
		const struct firmware_info *fw_info)
{
	...
	VTI("===enter===");
    if(!dev->hw_ops || !dev->hw_ops->read || !dev->hw_ops->set_doze_mode){
		VTE("HW ops is null");
		return -EINVAL;
	}
	...
	r = dev->hw_ops->set_doze_mode(dev, false);
	if (r == -EINVAL)
		goto need_update;

	if (atomic_read(&fw_update_mode_V2) == 1)
		goto need_update;
	...
	r = dev->hw_ops->read(dev, PID_REG, buffer, PID_LEN);
	if (r < 0) {
		VTE("Read pid failed");
		goto exit;
	}
	VTI("PID:%02x %02x %02x %02x", buffer[0], buffer[1],
				buffer[2], buffer[3]);

	if (memcmp(buffer, fw_info->fw_pid, PID_LEN)) {
		VTE("Product ID is not match,may header's fw is other product's");		
		goto need_update;
	}

	/* read and compare VID */
	r = dev->hw_ops->read(dev, VID_REG, buffer, VID_LEN);
	if (r < 0) {
		VTE("Read vid failed");
		goto exit;
	}
	VTI("VID:%02x %02x %02x %02x", buffer[0], buffer[1],
				buffer[2], buffer[3]);
	...
	if (res == 0) {
		VTE("FW version is equal to the IC's");		
		r = -EPERM;
		goto exit;
	} else if (res > 0) {
		VTI("Warning: fw version is lower the IC's");
	}
	 /* else invalid firmware, update firmware */

need_update:
	VTI("Firmware needs to be updated");
	r = 0;
exit:
	/*enable doze mode, just valid for normandy
	* this func must be used in pairs*/
	dev->hw_ops->set_doze_mode(dev, true);
	return r;
}
...
static int goodix_load_mask(struct goodix_ts_device *ts_dev)
{
	...
	if (r < 0) {
		VTE("Firmware image [%s] not available,errno:%d", mask_name, r);
		return r;
	} else {
		VTI("Firmware image [%s] is ready, size = %zu", mask_name,
			mask_fw->size);
	}
	...
	if (r) {
		VTE("Failed enbale AHB access");
		goto mask_exit;
	}
	VTI("Success enable AHB access, Set 0x2049 --> 0x01");

	/* switch to bank4 */
	reg_val[0] = 0x04;
	r = goodix_reg_write(ts_dev, 0x2048, reg_val, 1);
	if (r) {
		VTE("Failed switch to bank4");
		goto mask_exit;
	}
	VTI("Success switch to bank4, Set 0x2048 -->0x04");
	...
	while (total_size > 0) {
		...
		VTI("Flash firmware to %08x,size:%u bytes",
		...
		for (i = 0; i < 3; i++) {
			...
			if (!r)
				...
				VTI("Failed write mask data retry..");
				msleep(20);
			}
		}
		if (r) {
			VTE("Failed send mask");
			goto mask_exit;
		}
		offset += data_size;
		...
		if (index == 1) {
			...
			r = goodix_reg_write(ts_dev, 0x2048, reg_val, 1);
			if (r) {
				VTE("Failed switch to bank5");
				goto mask_exit;
			}
			VTI("Success switch to bank5, Set 0x2048-->0x05");
		}
		index++;
	}
	/* disable AHB access */
	...
	r = goodix_reg_write(ts_dev, 0x2049, reg_val, 1);
	if (r) {
		VTE("Failed disbale AHB access");
		goto mask_exit;
	}
	VTI("Success diable AHB access, Set 0x2049-->0x00");
	VTI("Success loak mask");
mask_exit:
			   ...
			   struct firmware_data *fw_data)
{
	...
	VTI("Loading ISP start");
	/* select bank0 */
	reg_val[0] = 0x00;
	r = goodix_reg_write(ts_dev, HW_REG_BANK_SELECT,
				     reg_val, 1);
	if (r < 0) {
		VTE("Failed to select bank0");
		return r;
	}
	VTI("Success select bank0, Set 0x%x -->0x00", HW_REG_BANK_SELECT);
	...
	r = goodix_reg_write(ts_dev, HW_REG_ACCESS_PATCH0,
				     reg_val, 1);
	if (r < 0) {
		VTE("Failed to enable patch0 access");
		return r;
	}
	VTI("Success select bank0, Set 0x%x -->0x01", HW_REG_ACCESS_PATCH0);

	r = goodix_reg_write_confirm(ts_dev, HW_REG_ISP_ADDR,
				     (u8 *)fw_isp->data, fw_isp->size);
	if (r < 0) {
		VTE("Loading ISP error");
		return r;
	}

	VTI("Success send ISP data to IC");
	...
	r = goodix_reg_write_confirm(ts_dev, HW_REG_ACCESS_PATCH0,
				     reg_val, 1);
	if (r < 0) {
		VTE("Failed to disable patch0 access");
		return r;
	}
	VTI("Success forbit bank0 accedd, set 0x%x -->0x00", HW_REG_ACCESS_PATCH0);


	/*debug add, clear 0x6006, 20180507*/
	reg_val[0] = 0x00;
	reg_val[1] = 0x00;
	r = goodix_reg_write(ts_dev, HW_REG_ISP_RUN_FLAG,
				     reg_val, 2);
	if (r < 0) {
		VTE("Failed to clear 0x%x", HW_REG_ISP_RUN_FLAG);
		return r;
	}
	VTD("Success clear 0x%0x", HW_REG_ISP_RUN_FLAG);



	/* TODO: change address 0xBDE6 set ISP program flag HW_REG_CPU_RUN_FROM */
	memset(reg_val, 0x55, 8);
	r = goodix_reg_write(ts_dev, HW_REG_CPU_RUN_FROM,
				     reg_val, 8);
	if (r < 0) {
		VTE("Failed set ISP program flag");
		return r;
	}
	VTI("Success write [8]0x55 to 0x%x", HW_REG_CPU_RUN_FROM);
	...
	r = goodix_reg_write_confirm(ts_dev, HW_REG_EC_SRM_START,
				     reg_val, 1);
	if (r < 0) {
		VTE("Failed to set CPU Emulation Code SRM start");
		return r;
	}
	VTI("Success set CPU Emulation code start, set 0x%x-->0x01",
		 HW_REG_EC_SRM_START);*/
	...
	r = goodix_reg_write(ts_dev, HW_REG_CPU_CTRL,
				     reg_val, 1);
	if (r < 0) {
		VTE("Failed to run isp");
		return r;
	}
	VTI("Success run isp, set 0x%x-->0x00", HW_REG_CPU_CTRL);
	...
	if (reg_val[0] == 0xAA && reg_val[1] == 0xBB) {
		...
		VTE("ISP not work,0x%x=0x%x, 0x%x=0x%x",
...
static int goodix_update_prepare(struct fw_update_ctrl *fwu_ctrl)
{
	...
	ts_dev->hw_ops->write(ts_dev, 0x4506, temp_buf, 8);


	fwu_ctrl->allow_reset = true;
	VTI("normandy firmware update, reset");
	...
	r = goodix_reg_write_confirm(ts_dev, HW_REG_ILM_ACCESS,
			reg_val, 1);
	if (r < 0) {
		VTE("Failed to enable ILM access");
		return r;
	}*/
	...
	*/
	...
	/*temp_buf[0] = 0x00;
	goodix_reg_write(ts_dev, 0x2328, temp_buf, 1);
	temp_buf[0] = 0x01;
	goodix_reg_write(ts_dev, 0x2321, temp_buf, 1);
	temp_buf[0] = 0x01;
	goodix_reg_write(ts_dev, 0x2265, temp_buf, 1);
	udelay(200);*/
	...
	do {
		...
		if (r < 0) {
			VTE("Failed read back addr 0x%x", addr + index);
			goto err_out;
		}*/
		reg_val[0] = 0x24;
		r = goodix_reg_write_confirm(ts_dev, HW_REG_CPU_CTRL, reg_val, 1);
		if (r < 0) {
			VTI("Failed to hold ss51, retry");
			msleep(20);
		} else {
			break;
		}
	...
	if (!retry) {
		VTE("Failed hold ss51,return =%d", r);
		return -EINVAL;
	}
	VTI("Success hold ss51");

	/* enable DSP & MCU power */
	reg_val[0] = 0x00;
	r = goodix_reg_write_confirm(ts_dev, HW_REG_DSP_MCU_POWER, reg_val, 1);
	if (r < 0) {
		VTE("Failed enable DSP&MCU power");
		return r;
	}
	VTI("Success enabled DSP&MCU power,set 0x%x-->0x00",
		 HW_REG_DSP_MCU_POWER);

	/* disable watchdog timer */
	reg_val[0] = 0x00;
	r = goodix_reg_write(ts_dev, 0x204B, reg_val, 1);
	if (r < 0) {
		VTE("Failed to clear cache");
		return r;
	}
	VTI("Success clear cache");

	reg_val[0] = 0x95;
	r = goodix_reg_write(ts_dev, 0x2318, reg_val, 1);
	reg_val[0] = 0x00;
	r |= goodix_reg_write(ts_dev, 0x20B0, reg_val, 1);

	reg_val[0] = 0x27;
	r |= goodix_reg_write(ts_dev, 0x2318, reg_val, 1);
	if (r < 0) {
		VTE("Failed to disable watchdog");
		return r;
	}
	VTI("Success disable watchdog");

	/* soft reset */
	/*reg_val[0] = 0x01;
	r = goodix_reg_write(ts_dev, HW_REG_RESET, reg_val, 1);
	if (r < 0) {
		VTE("Soft reset falied");
		return r;
	}
	VTI("Success soft reset");*/

	/* set scramble */
	reg_val[0] = 0x00;
	r = goodix_reg_write(ts_dev, HW_REG_SCRAMBLE, reg_val, 1);
	if (r < 0) {
		VTE("Failed to set scramble");
		return r;
	}
	VTI("Succcess set scramble");

	/* load mask for emulation IC */
	/*r = goodix_load_mask(ts_dev);
	if (r < 0) {
		VTE("Failed load mask");
		return r;
	}
	VTI("Success load mask");*/

	/* load ISP code and run form isp */
	r = goodix_load_isp(ts_dev, &fwu_ctrl->fw_data);
	if (r < 0)
		VTE("Failed lode and run isp");
				  ...
				  u32 len, const char *dst)
{
	...
	while (need_compare_len > 0) {
		...
		if (memcmp(temp_buf, dst + index, rlen)) {
			VTE("Read back compare failed, len = %d", rlen);
			//goto err_out;
		}
				 ...
				 u8 *pkt, u32 len)
{
	...
	if (r < 0) {
		VTE("Failed to write firmware packet");
		return r;
	}
	...
	if (r < 0) {
		VTE("Failed write subsystem type to IC");
		return r;
	}
	...
	for (i = 0; i < 200; i++) {
		...
		if (r < 0) {
			VTE("Failed read flash state");
			return r;
		}

		/* flash haven't end */
		if (reg_val[0] == 0xAA && reg_val[1] == 0xAA) {
			VTI("Flash not ending...");
		...
		if (reg_val[0] == 0xBB && reg_val[1] == 0xBB) {
			...
			r = goodix_reg_read(dev, HW_REG_FLASH_FLAG, reg_val, 2);
			if (!r && reg_val[0] == 0xBB && reg_val[1] == 0xBB) {
				/*r = goodix_read_back_check(dev, 0x8104,
					len - 6, (char*)pkt + 4);
				if (r) {
					VTE("Read back compare failed");
				} else {
					VTI("Read back compare OK");
				}*/
				VTI("Flash subsystem ok");
				return 0;
			}
		}
		if (reg_val[0] == 0xCC && reg_val[1] == 0xCC) {
			VTE(" Flash subsystem failed");
			return -EINVAL;
		}
		if (reg_val[0] == 0xDD) {
			VTE("Subsystem checksum err");
			return -EINVAL;
		}

		usleep_range(250, 260);
	}
	VTI("0x6022= %x, %x", reg_val[0], reg_val[1]);
	VTE("Wait for flash end timeout");
	return -EINVAL;
}
				...
				struct firmware_data *fw_data)
{
	...
	for (i = 1; i < fw_num && retry;) {
		VTI("--- Start to flash subsystem[%d] ---", i);
		fw_x = &fw_info->subsys[i];
		r = goodix_flash_subsystem(dev, fw_x);
		if (r == 0) {
			VTI("--- End flash subsystem[%d]: OK ---", i);
			fw_ctrl->progress += prog_step;
			i++;
		} else if (r == -EAGAIN) {
			retry--;
			VTE("--- End flash subsystem%d: Fail, errno:%d, retry:%d ---",
				i, r, GOODIX_BUS_RETRY_TIMES - retry);
		} else if (r < 0) { /* bus error */
			VTE("--- End flash subsystem%d: Fatal error:%d exit ---",
				i, r);
			goto exit_flash;
		}
	}
...
/*	debug_buf = kzalloc(4096, GFP_KERNEL);
	...
	if (r) {
		VTE("Faild send read command");
		goto exit_debug;
	}

	r = goodix_reg_read(dev, 0x8100, debug_buf, 4086);
	if (!r) {
		VTI("success read 4096bytes");
		VTI("data is: %*ph", 128, debug_buf);
	} else {
		VTE("Failed read 0x8100,4096 bytes");
	}
...
static int goodix_flash_config(struct goodix_ts_device *ts_dev)
{
	...
	if (!ts_dev->normal_cfg || !ts_dev->highsense_cfg) {
		VTE("no valid config data for flash");
		return -EINVAL;
	}
	...
	VTI("normal config length %d", ts_dev->normal_cfg->length);
	...
	VTD("fw_pack:%*ph", 10, fw_packet);
	VTI("try flash config");
	ret = goodix_send_fw_packet(ts_dev, FLASH_SUBSYS_TYPE_CONFIG,
				  fw_packet, ISP_MAX_BUFFERSIZE + 6);
	if (ret)
		VTE("failed flash config, ret %d", ret);
	else
		VTI("success flash config with isp");
...
/**
 * goodix_fw_update_proc_V2 - firmware update process, the entry of
 *  firmware update flow
 * @fwu_ctrl: firmware control
 * return: 0 ok, < 0 error
 */
int goodix_fw_update_proc_V2(struct fw_update_ctrl *fwu_ctrl)
{
#define FW_UPDATE_RETRY		2
	...
	if (fwu_ctrl->status == UPSTA_PREPARING ||
		...
		VTE("Firmware update already in progress");
		return 0;
	}
...
start_update:
	...
	if ((r == -EBUS || r == -EAGAIN) && retry0-- > 0) {
		VTE("Bus error, retry prepare ISP:%d",
				FW_UPDATE_RETRY - retry0);
		goto start_update;
	} else if (r < 0) {
		VTE("Failed to prepare ISP, exit update:%d", r);
		fwu_ctrl->status = UPSTA_FAILED;
		goto err_fw_prepare;
	}
	
	if (atomic_read(&fw_update_mode_V2) == 0) {
			VTI("need flash config with isp");
	...
	if ((r == -EBUS || r == -ETIMEOUT) && retry1-- > 0) {
		 ...
		 * flashing
		 */
		VTE("Bus error, retry firmware update:%d",
				FW_UPDATE_RETRY - retry1);
		goto start_update;
	} else if (r < 0) {
		VTE("Fatal error, exit update:%d", r);
		fwu_ctrl->status = UPSTA_FAILED;
		goto err_fw_flash;
	}
...
err_parse_fw:
	if (fwu_ctrl->status == UPSTA_SUCCESS)
		VTI("Firmware update successfully");
	else if (fwu_ctrl->status == UPSTA_FAILED)
		VTE("Firmware update failed");
...
struct goodix_ext_module goodix_fwu_module_V2;
				...
				const char *name)
{
	...
	VTI("Request firmware image [%s]", name);
	r = request_firmware(&fw_data->firmware, name, dev);
	if (r < 0)
		VTE("Firmware image [%s] not available,errno:%d", name, r);
	else
		VTI("Firmware image [%s] is ready", name);
	return r;
}
...
static int goodix_fw_update_thread(void *data)
{
	...
	if (!fwu_ctrl) {
		VTE("Invaid thread params");
		goodix_unregister_ext_module_V2(&goodix_fwu_module_V2);
		return 0;
	}
	VTI("===enter===");
	...
	if (!fwu_ctrl->fw_from_sysfs) {
		if (atomic_read(&fw_update_mode_V2) == 0) {
			...
			*/
		} else if (atomic_read(&fw_update_mode_V2) == 1) {
			VTI("Firmware request_firmware update start.");
		...
		if (!fwu_ctrl->fw_data.firmware) {
			VTE("Invalid firmware from sysfs");
			fwu_ctrl->status = UPSTA_ABORT;
			fwu_ctrl->progress = 100;
			goto out;
		}
	...
	if (!fwu_ctrl->core_data->cfg_group_parsed)
		goodix_cfg_bin_proc_V2(fwu_ctrl->core_data);
	*/
out:
	goodix_unregister_ext_module_V2(&goodix_fwu_module_V2);
	mutex_unlock(&fwu_lock);
	atomic_set(&fw_update_mode_V2, 0);
	return 0;
}
		...
		const char *buf, size_t count)
{
	...
	if (r) {
		atomic_set(&fw_update_mode_V2, 1);
		if (goodix_register_ext_module_V2(&goodix_fwu_module_V2))
			return -EIO;
	}
		...
		const char *buf, size_t count)
{
	...
	if (sscanf(buf, "%zu", &size) < 0 || !size) {
		VTE("Failed to get fwsize");
		return -EFAULT;
	}

	fw = vmalloc(sizeof(*fw) + size);
	if (fw == NULL) {
		VTE("Failed to alloc memory,size:%zu",
				size + sizeof(*fw));
		return -ENOMEM;
	}
		...
		char *buf, loff_t pos, size_t count)
{
	...
	if (!fw_data->firmware) {
		VTE("Need set fw image size first");
		return -ENOMEM;
	}

	if (fw_data->firmware->size == 0) {
		VTE("Invalid firmware size");
		return -EINVAL;
	}
...
int goodix_syfs_init_V2(struct goodix_ts_core *core_data,
		struct goodix_ext_module *module)
{
	struct fw_update_ctrl *fw_ctrl = module->priv_data;
	...
	ktype = goodix_get_default_ktype_V2();
	ret = kobject_init_and_add(&module->kobj,
			ktype,
			&core_data->pdev->dev.kobj,
			"fwupdate");
	if (ret) {
		VTE("Create fwupdate sysfs node error!");
...
int goodix_create_fw_sys_V2(struct goodix_ts_core *core_data,
				struct goodix_ext_module *module)
{
	struct fw_update_ctrl *fwu_ctrl;
	...
	goodix_syfs_init_V2(core_data, module);

/*
	if (module->priv_data) {
		kfree(module->priv_data);
		module->priv_data = NULL;
	}
*/
	return 0;
}
				...
				struct goodix_ext_module *module)
{
	...
	/* create sysfs interface */
	if (init_sysfs) {
		if (!goodix_syfs_init_V2(core_data, module))
			init_sysfs = false;
	}
...
struct goodix_ext_module_funcs goodix_ext_funcs_V2 = {
