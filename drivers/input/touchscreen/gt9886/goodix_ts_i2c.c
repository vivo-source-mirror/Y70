										  ...
										  u8 *buf)
{
	...
	for (i = 0; i < sub_bags; i++) {
		...
		offset += subbag_len + 3;
		VTD("sub bag %d, data:%*ph", buf[offset], buf[offset + 1] + 3, buf + offset);
	}
	ret = offset;
...
static int __init goodix_gt9886_init(void)
{
	VTI("goodix_gt9886_init i2c add driver !");
...
static void __exit goodix_gt9886_exit(void)
{
	VTI("goodix_gt9886_exit i2c del driver !");
...
module_init(goodix_gt9886_init);
module_exit(goodix_gt9886_exit);
