...
#define VTS_PROPERTY_DIMENTION_X 1080
#define VTS_PROPERTY_DIMENTION_Y 2400

#define VTD(fmt, x...) 	if(0){\
				printk("[VIVO_TS_INF][%s]:%d: "fmt , __func__, __LINE__, ##x);\
			}
#define VTI(fmt, x...) printk("[VIVO_TS_INF][%s]:%d: "fmt , __func__, __LINE__, ##x)
#define VTE(fmt, x...) printk("[VIVO_TS_ERR][%s]:%d: "fmt , __func__, __LINE__, ##x)
/*
...
struct goodix_ts_core {
	...
	unsigned int fingerPressRecord[10][4];
	int face_detect_cmd;
	int lcd_state;
	bool large_press;
...
extern void goodix_msg_printf_V2(const char *fmt, ...);
