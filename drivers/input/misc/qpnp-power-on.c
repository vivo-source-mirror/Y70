...
#define QPNP_PON_WD_RST_PET(pon)		((pon)->base + 0x58)
...
struct qpnp_pon {
	...
	/*Begin add kpdpwr monitor hankecai 2017-08-28*/
	struct delayed_work kpdpwr_work;
	/*End add kpdpwr monitor hankecai 2017-08-28*/
	u16			base;
	u8			subtype;
	u8			pon_ver;
...
#define PREASON_HARD            "HARD"
#define PREASON_SMPL            "SMPL"
#define PREASON_RTC             "RTC"
#define PREASON_DC              "DC"
#define PREASON_USB             "USB"
#define PREASON_PON1            "PON1"
#define PREASON_CBL             "CBL"
#define PREASON_KPD             "KPD"

static const char * const qpnp_pon_reason[][2] = {
	[0] = {"Triggered from Hard Reset", PREASON_HARD},
	[1] = {"Triggered from SMPL (sudden momentary power loss)", PREASON_SMPL},
	[2] = {"Triggered from RTC (RTC alarm expiry)", PREASON_RTC},
	[3] = {"Triggered from DC (DC charger insertion)", PREASON_DC},
	[4] = {"Triggered from USB (USB charger insertion)", PREASON_USB},
	[5] = {"Triggered from PON1 (secondary PMIC)", PREASON_PON1},
	[6] = {"Triggered from CBL (external power supply)", PREASON_CBL},
	[7] = {"Triggered from KPD (power key press)", PREASON_KPD},
};

#define PREASON_NA                          "NA"
#define PREASON_SOFT                        "SOFT"
#define PREASON_PS_HOLD                     "PS_HOLD"
#define PREASON_PMIC_WD                     "PMIC_WD"
#define PREASON_GP1                         "GP1"
#define PREASON_GP2                         "GP2"
#define PREASON_KPDPWR_AND_RESIN            "KPDPWR_AND_RESIN"
#define PREASON_RESIN_N                     "RESIN_N"
#define PREASON_KPDPWR_N                    "KPDPWR_N"
#define PREASON_CHARGER                     "CHARGER"
#define PREASON_TFT                         "TFT"
#define PREASON_UVLO                        "UVLO"
#define PREASON_OTST3                       "OTST3"
#define PREASON_STAGE3                      "STAGE3"
#define PREASON_GP_FAULT0                   "GP_FAULT0"
#define PREASON_GP_FAULT1                   "GP_FAULT1"
#define PREASON_GP_FAULT2                   "GP_FAULT2"
#define PREASON_GP_FAULT3                   "GP_FAULT3"
#define PREASON_MBG_FAULT                   "MBG_FAULT"
#define PREASON_OVLO                        "OVLO"
#define PREASON_AVDD_RB                     "AVDD_RB"
#define PREASON_FAULT_FAULT_N               "FAULT_FAULT_N"
#define PREASON_FAULT_PBS_WATCHDOG_TO       "FAULT_PBS_WATCHDOG_TO"
#define PREASON_FAULT_PBS_NACK              "FAULT_PBS_NACK"
#define PREASON_FAULT_RESTART_PON           "FAULT_RESTART_PON"
#define PREASON_S3_RESET_FAULT_N            "S3_RESET_FAULT_N"
#define PREASON_S3_RESET_PBS_WATCHDOG_TO    "S3_RESET_PBS_WATCHDOG_TO"
#define PREASON_S3_RESET_PBS_NACK           "S3_RESET_PBS_NACK"
#define PREASON_S3_RESET_KPDPWR_ANDOR_RESIN "S3_RESET_KDPWR_ANDOR_RESIN"

#define POFF_REASON_FAULT_OFFSET	16
#define POFF_REASON_S3_RESET_OFFSET	32
static const char * const qpnp_poff_reason[][2] = {
	/* QPNP_PON_GEN1 POFF reasons */
	[0] = {"Triggered from SOFT (Software)", PREASON_SOFT},
	[1] = {"Triggered from PS_HOLD (PS_HOLD/MSM controlled shutdown)", PREASON_PS_HOLD},
	[2] = {"Triggered from PMIC_WD (PMIC watchdog)", PREASON_PMIC_WD},
	[3] = {"Triggered from GP1 (Keypad_Reset1)", PREASON_GP1},
	[4] = {"Triggered from GP2 (Keypad_Reset2)", PREASON_GP2},
	[5] = {"Triggered from KPDPWR_AND_RESIN (Simultaneous power key and reset line)", PREASON_KPDPWR_AND_RESIN},
	[6] = {"Triggered from RESIN_N (Reset line/Volume Down Key)", PREASON_RESIN_N},
	[7] = {"Triggered from KPDPWR_N (Long Power Key hold)", PREASON_KPDPWR_N},
	[8] = {"N/A", PREASON_NA},
	[9] = {"N/A", PREASON_NA},
	[10] = {"N/A", PREASON_NA},
	[11] = {"Triggered from CHARGER (Charger ENUM_TIMER, BOOT_DONE)", PREASON_CHARGER},
	[12] = {"Triggered from TFT (Thermal Fault Tolerance)", PREASON_TFT},
	[13] = {"Triggered from UVLO (Under Voltage Lock Out)", PREASON_UVLO},
	[14] = {"Triggered from OTST3 (Overtemp)", PREASON_OTST3},
	[15] = {"Triggered from STAGE3 (Stage 3 reset)", PREASON_STAGE3},

	/* QPNP_PON_GEN2 FAULT reasons */
	[16] = {"Triggered from GP_FAULT0", PREASON_GP_FAULT0},
	[17] = {"Triggered from GP_FAULT1", PREASON_GP_FAULT1},
	[18] = {"Triggered from GP_FAULT2", PREASON_GP_FAULT2},
	[19] = {"Triggered from GP_FAULT3", PREASON_GP_FAULT3},
	[20] = {"Triggered from MBG_FAULT", PREASON_MBG_FAULT},
	[21] = {"Triggered from OVLO (Over Voltage Lock Out)", PREASON_OVLO},
	[22] = {"Triggered from UVLO (Under Voltage Lock Out)", PREASON_UVLO},
	[23] = {"Triggered from AVDD_RB", PREASON_AVDD_RB},
	[24] = {"N/A", PREASON_NA},
	[25] = {"N/A", PREASON_NA},
	[26] = {"N/A", PREASON_NA},
	[27] = {"Triggered from FAULT_FAULT_N", PREASON_FAULT_FAULT_N},
	[28] = {"Triggered from FAULT_PBS_WATCHDOG_TO", PREASON_FAULT_PBS_WATCHDOG_TO},
	[29] = {"Triggered from FAULT_PBS_NACK", PREASON_FAULT_PBS_NACK},
	[30] = {"Triggered from FAULT_RESTART_PON", PREASON_FAULT_RESTART_PON},
	[31] = {"Triggered from OTST3 (Overtemp)", PREASON_OTST3},
	...
	[34] = {"N/A", PREASON_NA},
	[35] = {"N/A", PREASON_NA},
	[36] = {"Triggered from S3_RESET_FAULT_N", PREASON_S3_RESET_FAULT_N},
	[37] = {"Triggered from S3_RESET_PBS_WATCHDOG_TO", PREASON_S3_RESET_PBS_WATCHDOG_TO},
	[38] = {"Triggered from S3_RESET_PBS_NACK", PREASON_S3_RESET_PBS_NACK},
	[39] = {"Triggered from S3_RESET_KPDPWR_ANDOR_RESIN (power key and/or reset line)",
			PREASON_S3_RESET_KPDPWR_ANDOR_RESIN},
};

/*Begin hankecai modify Volume down key dependent on is_debug_mode 2016-10-12*/
extern unsigned int is_debug_mode;
static u32 kpdpwr_monitor_timeout = 8;
static bool kpdpwr_monitor_state;
static int g_dump_trigger_s2_src = PON_KPDPWR_RESIN;	/*default set to PON_KPDPWR_RESIN*/
static int g_system_reset_s2_src = PON_KPDPWR_RESIN;	/*default set to PON_KPDPWR_RESIN*/
extern char *vivo_get_pmic_status(void);

static int kpdpwr_resin_en;
module_param(kpdpwr_resin_en, int, S_IRUSR);

char qpnp_pon_reason_str[32];
char qpnp_poff_reason_str[48];

#ifdef CONFIG_VIVO_WDT_MONITOR
extern int is_vivolog_flag;
extern unsigned int is_normal_mode;
static bool wdt_monitor_setup_done;

#define VIVO_WDT_MONITOR_PET_TIME 100
#define VIVO_WDT_MONITOR_BARK_TIME 120

extern int vivo_wdt_monitor_pet(void);
extern int vivo_wdt_monitor_stop(void);
extern int vivo_wdt_monitor_start_kernel(long timeout);
extern void vivo_wdt_monitor_setup(bool g_force_offline_init);

extern unsigned int power_off_charging_mode;
static void vivo_wdt_monitor_work_handler(struct work_struct *work);
DECLARE_DELAYED_WORK(vivo_wdt_monitor_work, vivo_wdt_monitor_work_handler);
static void vivo_wdt_monitor_work_handler(struct work_struct *work)
{
	//vivo_wdt_monitor_pet();
	schedule_delayed_work(&vivo_wdt_monitor_work, VIVO_WDT_MONITOR_PET_TIME*HZ);
	printk("PMIC Power-on/Power-off reason=%s/%s/%s\n", qpnp_pon_reason_str, qpnp_poff_reason_str, vivo_get_pmic_status());
}
#endif	/*CONFIG_VIVO_WDT_MONITOR*/

/*vivo sensor team add start*/
static bool b_kpdpwr_volume_down_wake = true;
static u32  volume_dowm_state_irq;
/*vivo sensor team add end*/
...
static int qpnp_pon_disable_s2_reset(struct qpnp_pon *pon,
...
static int qpnp_pon_kpdpwr_resin_cfg(struct qpnp_pon *pon,
		struct qpnp_pon_config *cfg, bool enable)
{
	int rc = 0;

	if (NULL == cfg || NULL == pon) {
		pr_err("%s cfg or pon is NULL\n", __func__);
		return -EINVAL;
	}

	/*print to remind the adjust*/
	pr_err("enable:%d, s2_src:%d, s2_type:%d\n",
			enable, g_dump_trigger_s2_src, cfg->s2_type);

	/*if not the global system reset s2 src, disable the reset directly*/
	if (!enable && (cfg->pon_type != g_system_reset_s2_src))
		return qpnp_pon_disable_s2_reset(pon, cfg);

	/*Here means g_system_reset_s2_src will adjust*/
	if (enable) {
		cfg->s1_timer = 6720;
		cfg->s2_timer = 500;
		cfg->s2_type = 1;
	...
	if (rc)
		pr_err("%s Unable to config pon reset\n", __func__);
	else
		pr_err("%s success to enable:%d, s2_src:%d, s2_type:%d\n",
			__func__, enable, g_dump_trigger_s2_src, cfg->s2_type);

	return rc;
}

static ssize_t qpnp_pon_kpdpwr_resin_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	struct qpnp_pon *pon = dev_get_drvdata(dev);
	...
	/*PON_KPDPWR_RESIN */
	cfg = qpnp_get_cfg(pon, g_dump_trigger_s2_src);
	if (NULL != cfg)
		s2_type = cfg->s2_type;
	else
		s2_type = 7;/*de	fault HARD(7)*/

	return snprintf(buf, sizeof(s2_type), "%d\n", s2_type);
}

static ssize_t qpnp_pon_kpdpwr_resin_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t size)
{
	...
	cfg = qpnp_get_cfg(pon, g_dump_trigger_s2_src);
	if (NULL == cfg) {
		pr_err("KPDPWR_RESIN is NULL\n");
		return -EINVAL;
	}

	if (value == 7)
		qpnp_pon_kpdpwr_resin_cfg(pon, cfg, 0);
	else if (value == 1)
		qpnp_pon_kpdpwr_resin_cfg(pon, cfg, 1);

	return size;
}

/*add by hkc begin*/
static ssize_t qpnp_pon_s2_src_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	char s2_buf[32] = {0};

	switch (g_dump_trigger_s2_src) {
	case PON_KPDPWR:
		snprintf(s2_buf, sizeof(s2_buf), "%s", "0: PON_KPDPWR");
		break;

	case PON_RESIN:
		snprintf(s2_buf, sizeof(s2_buf), "%s", "1: PON_RESIN");
		break;

	case PON_KPDPWR_RESIN:
		snprintf(s2_buf, sizeof(s2_buf), "%s", "3: PON_KPDPWR_RESIN");
		break;

	default:
		snprintf(s2_buf, sizeof(s2_buf), "%s", "Invalid");
		break;
	}
	return snprintf(buf, sizeof(s2_buf), "%s\n", s2_buf);
}

static ssize_t qpnp_pon_s2_src_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t size)
{
	struct qpnp_pon *pon = dev_get_drvdata(dev);
	struct qpnp_pon_config *cfg = NULL;
	...
	if (value > PON_KPDPWR_RESIN || value < PON_KPDPWR)
		return -EINVAL;

	g_dump_trigger_s2_src = value;

	/*re-config the s2_sre to take effect */
	cfg = qpnp_get_cfg(pon, g_dump_trigger_s2_src);
	if (NULL == cfg) {
		pr_err("KPDPWR_RESIN is NULL\n");
		return -EINVAL;
...
static DEVICE_ATTR(kpdpwr_resin_s2_type, 0664, qpnp_pon_kpdpwr_resin_show,
		qpnp_pon_kpdpwr_resin_store);
static DEVICE_ATTR(kpdpwr_resin_s2_src, 0664, qpnp_pon_s2_src_show,
		qpnp_pon_s2_src_store);

int qpnp_pon_set_s3_timer(u32 time_in_sec)
{
	int rc = 0;
	struct qpnp_pon *pon = sys_reset_dev;

	if (time_in_sec > QPNP_PON_S3_TIMER_SECS_MAX) {
		dev_info(pon->dev,
			"Exceeded S3 max value, set it to max\n");
		time_in_sec = QPNP_PON_S3_TIMER_SECS_MAX;
	}

	/* 0 is a special value to indicate instant s3 reset */
	if (time_in_sec != 0)
		time_in_sec = ilog2(time_in_sec);
	...
	rc = qpnp_pon_masked_write(pon, QPNP_PON_S3_DBC_CTL(pon),
			QPNP_PON_S3_DBC_DELAY_MASK, time_in_sec);
	if (rc) {
		dev_err(pon->dev,
...
void qpnp_pon_kpdpwr_resin_ex(int enable)
{
	struct qpnp_pon *pon = sys_reset_dev;
	struct qpnp_pon_config *cfg = NULL;
	...
	pr_err("KPDPWR_RESIN qpnp_pon_kpdpwr_resin_ex enable:%d\n", !!enable);
	qpnp_pon_kpdpwr_resin_cfg(pon, cfg, !!enable);

	if (enable)
		qpnp_pon_set_s3_timer(64);
	else
		qpnp_pon_set_s3_timer(32);

#if defined(vos)
	/* PON_KPDPWR */
	cfg = qpnp_get_cfg(pon, PON_KPDPWR);
	if (NULL == cfg) {
		pr_err("PON_KPDPWR is NULL\n");
		return;
	}

	pr_err("PON_KPDPWR s2_type enable:%d\n", !!enable);
	qpnp_pon_kpdpwr_resin_cfg(pon, cfg, !!enable);
#endif
}
EXPORT_SYMBOL(qpnp_pon_kpdpwr_resin_ex);

static void kpdpwr_monitor_work_func(struct work_struct *work)
{
	struct qpnp_pon_config *kpdpwr_cfg;
	char *monitor_state[2] = {"kpdpwr_monitor_state=1", NULL};
	struct qpnp_pon *pon =
		container_of(work, struct qpnp_pon, kpdpwr_work.work);
	int rc;
	u32 key_status = 0;
	uint pon_rt_sts = 0;

	kpdpwr_cfg = qpnp_get_cfg(pon, PON_KPDPWR);
	if (!kpdpwr_cfg) {
		dev_err(pon->dev, "Invalid kpdpwr_cfg pointer\n");
		return;
	}
	...
	key_status = pon_rt_sts & (QPNP_PON_KPDPWR_N_SET | QPNP_PON_RESIN_N_SET);
	dev_err(pon->dev, "key_status2 0x%x\n", key_status);

	if (key_status == (QPNP_PON_KPDPWR_N_SET | QPNP_PON_RESIN_N_SET)) {
		/*modify state*/
		kpdpwr_monitor_state = 1;
		pr_err("%s\n", monitor_state[0]);
		kobject_uevent_env(&pon->dev->kobj, KOBJ_CHANGE, monitor_state);
	}
}

static ssize_t qpnp_pon_kpdpwr_monitor_state_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	struct qpnp_pon *pon = dev_get_drvdata(dev);
	struct qpnp_pon_config *cfg = NULL;
	...
	temp_val = !!kpdpwr_monitor_state;
	return snprintf(buf, sizeof(temp_val), "%d\n",
			temp_val);
}
static DEVICE_ATTR(kpdpwr_monitor_state, 0444,
		qpnp_pon_kpdpwr_monitor_state_show, NULL);

static ssize_t kpdpwr_monitor_timeout_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	struct qpnp_pon *pon = dev_get_drvdata(dev);
	struct qpnp_pon_config *cfg = NULL;
	...
	return snprintf(buf, sizeof(kpdpwr_monitor_timeout), "%d\n",
			kpdpwr_monitor_timeout);
}

static ssize_t kpdpwr_monitor_timeout_store(struct device *dev,
		struct device_attribute *attr,
		const char *buf, size_t size)
{
	struct qpnp_pon *pon = dev_get_drvdata(dev);
	...
	if (5 <= value && 10 >= value)
		kpdpwr_monitor_timeout = value;

	return size;
}
static DEVICE_ATTR(kpdpwr_monitor_timeout, 0664,
		kpdpwr_monitor_timeout_show,
		kpdpwr_monitor_timeout_store);
...
EXPORT_SYMBOL(qpnp_pon_wd_set_boot_stage_flags);
...
EXPORT_SYMBOL(qpnp_pon_wd_enable);
...
int qpnp_pon_wd_cfg_timer(u8 s1_timer, u8 s2_timer)
{
	...
	rc = qpnp_pon_masked_write(pon, addr,
				0x0F, PON_POWER_OFF_DVDD_HARD_RESET); /*dvdd hard reset*/
...
int qpnp_pon_wd_get_force_offline(void)
{
	...
	wdt_force_offline_value = wdt_force_offline_value & VIVO_WDT_FORCE_OFFLINE_BIT;

	if (wdt_force_offline_value)
		return 1;
	else
		return 0;
}
EXPORT_SYMBOL(qpnp_pon_wd_get_force_offline);
...
EXPORT_SYMBOL(qpnp_pon_wd_set_force_offline);

int qpnp_pon_wd_get_test_mode(void)
{
	int rc = 0;
	u16 addr;
	int wdt_test_mode_ctl_value = 0;
	struct qpnp_pon *pon = sys_reset_dev;
	...
	addr = QPNP_PON_XVDD_RB_SPARE(pon);
	rc = qpnp_pon_read(pon, addr, &wdt_test_mode_ctl_value);
	wdt_test_mode_ctl_value = wdt_test_mode_ctl_value & VIVO_WDT_TEST_MODE_BIT;

	if (wdt_test_mode_ctl_value)
		return 1;
	else
		return 0;
}
EXPORT_SYMBOL(qpnp_pon_wd_get_test_mode);

int qpnp_pon_wd_set_test_mode(bool test_mode)
{
	int rc = 0;
	u16 addr;
	struct qpnp_pon *pon = sys_reset_dev;
	...
	addr = QPNP_PON_XVDD_RB_SPARE(pon);
	rc = qpnp_pon_masked_write(pon, addr,
			VIVO_WDT_TEST_MODE_BIT, test_mode ? VIVO_WDT_TEST_MODE_BIT : 0);

	//rc = qpnp_pon_wd_enable(0, 0);
	return rc;
}
EXPORT_SYMBOL(qpnp_pon_wd_set_test_mode);

void vivo_wdt_monitor_prepare_and_start(void)
{
	bool g_force_offline_init = 0;
	int wdt_force_offline_value = 0;
	int wdt_test_mode_ctl_value = 0;

	if (wdt_monitor_setup_done)
		return;

	if (sys_reset_dev == NULL)
		return;

	qpnp_pon_wd_enable(0, 0);
	wdt_force_offline_value = qpnp_pon_wd_get_force_offline();
	wdt_test_mode_ctl_value = qpnp_pon_wd_get_test_mode();

	/*说明
	 *g_force_offline_init标记是否使wdt_monitor生效，情况如下:
	 *1、打开了vivolog，wdt不生效
	 *2、调试模式(eng、bsptmode)，wdt不生效
	 *3、强制force_offline标记被置上了，wdt不生效
	 *4、在force_offline没有被置上的情况下，可以被wdt_test标记强制生效
	 */

	/*vivolog is open,, force_offline*/
	if (is_vivolog_flag)
		g_force_offline_init = 1;

	/*debug mode, force_offline*/
	if (is_debug_mode)
		g_force_offline_init = 1;

	/*value from regs, take the Supreme priority*/
	if (wdt_force_offline_value)
		g_force_offline_init = 1;

	/*wdt_test_mode is enabled, force_online*/
	if (wdt_test_mode_ctl_value)
		g_force_offline_init = 0;

	printk("wdt_monitor: vivolog[%d], debug_mode[%d], wdtest_mode[%d], force_offline_bit[%d], is_normal_mode[%d]\n",
				is_vivolog_flag, is_debug_mode, wdt_test_mode_ctl_value, wdt_force_offline_value, is_normal_mode);
	vivo_wdt_monitor_setup(g_force_offline_init);

	if (power_off_charging_mode) {
		//vivo_wdt_monitor_start_kernel(VIVO_WDT_MONITOR_BARK_TIME);
		schedule_delayed_work(&vivo_wdt_monitor_work, VIVO_WDT_MONITOR_PET_TIME*HZ);
	} else if (is_normal_mode) {
		//vivo_wdt_monitor_start_kernel(300);	/*300 seconds*/
		vivo_wdt_monitor_stop();	/* temp disable*/
	}

	//vivo_wdt_monitor_stop();	/*temp disable*/
	wdt_monitor_setup_done = 1;
}
/*vivo added for wdt_monitor end*/
#endif /*CONFIG_VIVO_WDT_MONITOR*/
...
/*Begin hankecai add Monitor kpdpwr state 2017-08-08*/
static irqreturn_t qpnp_kpdpwr_thread_fn(int irq, void *_pon)
{
	int rc;
	...
	u32 key_status = 0;
	uint pon_rt_sts = 0;
	char *monitor_state[2] = {"kpdpwr_monitor_state=0", NULL};

	cfg = qpnp_get_cfg(pon, PON_KPDPWR);
	if (!cfg) {
		pr_err("PON_KPDPWR cfg is NULL");
		goto err_return;
	}
	/* Check if key reporting is supported */
	if (!cfg->key_code) {
		pr_err("key_code:%d", cfg->key_code);
		goto err_return;
	}
	if (NULL == pon->dev) {
		pr_err("pon->dev device is NULL\n");
		goto err_return;
	}
	...
	key_status = pon_rt_sts & (QPNP_PON_KPDPWR_N_SET | QPNP_PON_RESIN_N_SET);
	if (key_status)
		schedule_delayed_work(&pon->kpdpwr_work,
				kpdpwr_monitor_timeout*HZ);
	else {
		cancel_delayed_work_sync(&pon->kpdpwr_work);
		if (kpdpwr_monitor_state) {
			kpdpwr_monitor_state = 0;
			kobject_uevent_env(&pon->dev->kobj,
					KOBJ_CHANGE, monitor_state);
		}
	}
...
/*End hankecai add Monitor kpdpwr state 2017-08-08*/
...
qpnp_pon_request_irqs(struct qpnp_pon *pon, struct qpnp_pon_config *cfg)
{
	...
	case PON_KPDPWR:
		/*Begin hankecai add Monitor kpdpwr state 2017-08-08*/
		rc = devm_request_threaded_irq(pon->dev, cfg->state_irq,
							qpnp_kpdpwr_irq, qpnp_kpdpwr_thread_fn,
	...
	if (cfg->key_code) {
		...
		if (cfg->pon_type == PON_RESIN)
			volume_dowm_state_irq = cfg->state_irq;
		/*vivo sensor team add end*/
				...
				struct platform_device *pdev)
{
	...
	for (i = 0; i < pon->num_pon_config; i++) {
		...
		/*Begin hankecai modify re-Configure the reset-configuration 2016-11-22*/
		if (cfg->pon_type == g_dump_trigger_s2_src) {
			if (NULL == sys_reset_dev)
				sys_reset_dev = pon;
			if (is_debug_mode || kpdpwr_resin_en)
			qpnp_pon_kpdpwr_resin_ex(1);
		}
		/*End hankecai modify re-Configure the reset-configuration  2016-11-22*/
...
#ifdef CONFIG_VIVO_WDT_MONITOR
	if (*reason == 0) {
		dev_err(pon->dev, "Warning!!!, gen2_poff_reason=0, index_offset=%d\n", *reason_index_offset);
		rc = qpnp_pon_read(pon, QPNP_POFF_REASON1(pon), buf);
		if (rc)
			return rc;
		*reason = (u8)buf[0];
...
/*vivo sensor team add start*/
static ssize_t kpdpwr_volume_down_wake_store(struct device *dev, struct device_attribute *attr, const char *buf,
				   size_t count)
{
	int n = 0;
	int val = 1;
	...
	n = sscanf(buf, "%d", &val);
	if (!n)
		pr_err("kpdpwr_volume_down_wake_store nothing\n");
	else
		b_kpdpwr_volume_down_wake = (val == 0 ? false:true);
	if (volume_dowm_state_irq != 0) {
		if (b_kpdpwr_volume_down_wake == false)
			disable_irq_wake(volume_dowm_state_irq);
		else
			enable_irq_wake(volume_dowm_state_irq);
	}
	return count;
}

static ssize_t kpdpwr_volume_down_wake_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	return snprintf(buf, QPNP_PON_BUFFER_SIZE, "%d\n", b_kpdpwr_volume_down_wake);
}

static DEVICE_ATTR(kpdpwr_volume_down_wake, 0644, kpdpwr_volume_down_wake_show, kpdpwr_volume_down_wake_store);
/*vivo sensor team add end*/
...
static int qpnp_pon_read_hardware_info(struct qpnp_pon *pon, bool sys_reset)
{
	...
	if (index >= ARRAY_SIZE(qpnp_pon_reason) || index < 0) {
		...
		if (to_spmi_device(dev->parent)->usid == 0)
			strlcpy(qpnp_pon_reason_str, "unknown", sizeof(qpnp_pon_reason_str));
	} else {
		int i = 0;
		...
		dev_info(dev, "PMIC@SID%d Power-on reason: %s and '%s' boot\n",
			 to_spmi_device(dev->parent)->usid,
			 qpnp_pon_reason[index][0],
			 cold_boot ? "cold" : "warm");
		if (to_spmi_device(dev->parent)->usid == 0) {
			strlcpy(qpnp_pon_reason_str, qpnp_pon_reason[index][1], sizeof(qpnp_pon_reason_str));
			i = index;
			while (++i < ARRAY_SIZE(qpnp_pon_reason)) {
				if (pon_sts & (1 << i)) {
					dev_info(dev,
							...
							qpnp_pon_reason[i][0],
							cold_boot ? "cold" : "warm");
					strlcat(qpnp_pon_reason_str, "+", sizeof(qpnp_pon_reason_str));
					strlcat(qpnp_pon_reason_str, qpnp_pon_reason[i][1], sizeof(qpnp_pon_reason_str));
				}
			}
		}
	}
	...
	if (index >= ARRAY_SIZE(qpnp_poff_reason) || index < 0) {
		dev_info(dev, "PMIC@SID%d: Unknown power-off reason\n",
			 to_spmi_device(dev->parent)->usid);
		if (to_spmi_device(dev->parent)->usid == 0)
			strlcpy(qpnp_poff_reason_str, "unknown", sizeof(qpnp_poff_reason_str));
	} else {
		int i = 0, length = 0;
		...
		dev_info(dev, "PMIC@SID%d: Power-off reason: %s\n",
			 to_spmi_device(dev->parent)->usid,
				qpnp_poff_reason[index][0]);
		if (to_spmi_device(dev->parent)->usid == 0) {
			strlcpy(qpnp_poff_reason_str, qpnp_poff_reason[index][1], sizeof(qpnp_poff_reason_str));
			if (reason_index_offset == 0) {
				length = POFF_REASON_FAULT_OFFSET;
			} else if (reason_index_offset == POFF_REASON_FAULT_OFFSET) {
				length = POFF_REASON_S3_RESET_OFFSET;
			} else if (reason_index_offset == POFF_REASON_S3_RESET_OFFSET) {
				length = ARRAY_SIZE(qpnp_poff_reason);
			}
			i = index;
			while (++i < length) {
				if (poff_sts & (1 << i)) {
					dev_info(dev,
							...
							to_spmi_device(dev->parent)->usid,
							qpnp_poff_reason[i][0]);
					strlcat(qpnp_poff_reason_str, "+", sizeof(qpnp_poff_reason_str));
					strlcat(qpnp_poff_reason_str, qpnp_poff_reason[i][1], sizeof(qpnp_poff_reason_str));
				}
			}
		}
	}
...
static int qpnp_pon_probe(struct platform_device *pdev)
{
	...
	/*Begin hankecai Monitor kpdpwr state*/
	INIT_DELAYED_WORK(&pon->kpdpwr_work, kpdpwr_monitor_work_func);
	/*End hankecai Monitor kpdpwr state*/
	...
	/*Begin hankecai add set dloadmode to debug*/
	rc = device_create_file(&pdev->dev, &dev_attr_kpdpwr_resin_s2_type);
	if (rc) {
		dev_err(&pdev->dev, "sys file kpdpwr_resin_s2_type creation failed\n");
		return rc;
	}
	rc = device_create_file(&pdev->dev, &dev_attr_kpdpwr_monitor_state);
	if (rc) {
		dev_err(&pdev->dev, "sys file kpdpwr_monitor_state creation failed\n");
		return rc;
	}
	rc = device_create_file(&pdev->dev, &dev_attr_kpdpwr_monitor_timeout);
	if (rc) {
		dev_err(&pdev->dev, "sys file monitor_timeout_secs creation failed\n");
		return rc;
	}
	rc = device_create_file(&pdev->dev, &dev_attr_kpdpwr_resin_s2_src);
	if (rc) {
		dev_err(&pdev->dev, "sys file kpdpwr_resin_s2_src creation failed\n");
		return rc;
	}
	/*vivo sensor team add to enable disable volume dowm to wakeup*/
	rc = device_create_file(&pdev->dev, &dev_attr_kpdpwr_volume_down_wake);
	if (rc) {
		dev_err(&pdev->dev, "sys file monitor_timeout_secs creation failed\n");
		return rc;
	}
...
#ifdef CONFIG_VIVO_WDT_MONITOR
	vivo_wdt_monitor_prepare_and_start();
#else
...
static int qpnp_pon_remove(struct platform_device *pdev)
{
	...
	/*Begin hankecai add the key_voldown set dloadmode to debug 2016-01-16*/
	device_remove_file(&pdev->dev, &dev_attr_kpdpwr_resin_s2_type);
	device_remove_file(&pdev->dev, &dev_attr_kpdpwr_monitor_state);
	device_remove_file(&pdev->dev, &dev_attr_kpdpwr_monitor_timeout);
	device_remove_file(&pdev->dev, &dev_attr_kpdpwr_volume_down_wake);/*vivo sensor team add */
	cancel_delayed_work_sync(&pon->kpdpwr_work);
	device_remove_file(&pdev->dev, &dev_attr_kpdpwr_resin_s2_src);

#ifdef CONFIG_VIVO_WDT_MONITOR
	if (power_off_charging_mode) {
		cancel_delayed_work(&vivo_wdt_monitor_work);
	}
#endif
