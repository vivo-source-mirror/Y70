...
static char bbk_pcb_version[16] = "0";
...
static __init int set_bbk_pcb_version(char *str)
{
	strlcpy(bbk_pcb_version, str, sizeof(bbk_pcb_version));
	pr_err("bbk pcb version is %s\n", bbk_pcb_version);
	return 0;
}
early_param("bbk_pcb_version", set_bbk_pcb_version);
...
static ssize_t devices_pcb_vsersion_show(struct kobject *kobj,
							struct kobj_attribute *attr, char *buf)
{
	return snprintf(buf, PAGE_SIZE, "%s", bbk_pcb_version);
}
...
static struct debug_sysfs_entry pcb_version =
	__ATTR(pcb_version, S_IRUGO,
			devices_pcb_vsersion_show, NULL);
