...
#define MMC_BLK_TIMEOUT_MS  (8 * 1000)        /* 8 secs timeout */
				 ...
				 struct mmc_queue_req *mqrq)
{
	if (card && card->host && mmc_card_sd(card)) {
		...
		mmc_detect_change(card->host, msecs_to_jiffies(10));
	}
	if (mmc_card_removed(card))
	...
	do {
		...
		case MMC_BLK_DATA_ERR: {
			...
			if (!err) {
				...
				printk("%s: %s removing bad card:%d\n", mmc_hostname(card->host), __func__, status);
				mmc_blk_rw_cmd_abort(mq, card, old_req, mq_rq);
				mmc_blk_rw_try_restart(mq, new_req, mqrq_cur);
				return;
			}
...
static int mmc_ext_csd_open(struct inode *inode, struct file *filp)
{
	...
	for (i = 0; i < 512; i++) {
		if (i%16 == 0)
			n += snprintf(buf + n, EXT_CSD_STR_LEN,
				"0x%04x: %02x ", i, ext_csd[i]);
		else if (i%16 == 15)
			n += snprintf(buf + n, EXT_CSD_STR_LEN,
				"%02x\n", ext_csd[i]);
		else
