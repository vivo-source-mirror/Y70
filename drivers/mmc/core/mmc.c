...
static int mmc_decode_cid(struct mmc_card *card)
{
	...
	fmea_notifys("eMMC", "cid:%08x%08x%08x%08x prv:0x%x\n",
		card->raw_cid[0], card->raw_cid[1], card->raw_cid[2],
			card->raw_cid[3], card->cid.prv);
#endif
...
static int mmc_decode_ext_csd(struct mmc_card *card, u8 *ext_csd)
{
	...
	card->ext_csd.supported_modes = ext_csd[EXT_CSD_SUPPORTED_MODES];
	...
	if (card->cid.manfid == CID_MANFID_MICRON) {
		card->ext_csd.firmware_version =
				...
				ext_csd[262] << 24;	//This field provides the device version
	} else {
		card->ext_csd.firmware_version =
				ext_csd[EXT_CSD_FIRMWARE_VERSION + 0] << 0 |
				ext_csd[EXT_CSD_FIRMWARE_VERSION + 1] << 8 |
	...
	if (card->ext_csd.rev >= 6) {
		...
		card->ext_csd.dev_left_time_b =
			ext_csd[DEVICE_LIFE_TIME_EST_TYP_B];
		card->ext_csd.dev_left_time_a =
			ext_csd[DEVICE_LIFE_TIME_EST_TYP_A];
...
#ifdef CONFIG_FMEA
	fmea_notifys("eMMC", "FirmwareVersion[0x%08x],EOLinfo[0x%02x],\
		dev_left_time_a[0x%02x],dev_left_time_b[0x%02x] \n",
			card->ext_csd.firmware_version, card->ext_csd.dev_left_time,
...
MMC_DEV_ATTR(cid, "%08x%08x%08x%08x\n", card->raw_cid[0], card->raw_cid[1],
	card->raw_cid[2] & 0xFF00FFFF, card->raw_cid[3] & 0xFFFFFF00);
MMC_DEV_ATTR(cid_real, "%08x%08x%08x%08x\n", card->raw_cid[0], card->raw_cid[1],
...
reinit:
	...
	if (oldcard) {
		...
		emmc_raw_cid_point = card->raw_cid;
		host->card = card;
