...
static int cmdq_enable(struct mmc_host *mmc)
{
	...
	if (mmc->card->cid.manfid == CID_MANFID_MICRON)
		cmdq_writel(cq_host, SEND_QSR_INTERVAL_MIRCON, CQSSC1);
	else
		cmdq_writel(cq_host, SEND_QSR_INTERVAL, CQSSC1);

	/* Ignore uecc error for hynix device if bsp test mode not set */
	if ((mmc->card->cid.manfid == CID_MANFID_HYNIX) && (!bsp_test_mode))
		cmdq_writel(cq_host, 
			cmdq_readl(cq_host, CQRMEM) & (~CQ_ECC_FAILED_BIT), CQRMEM);
...
ring_doorbell:
	...
	mrq->cmdq_req->start_time = sched_clock();
#endif
		...
		for_each_set_bit(tag, &comp_status, cq_host->num_slots) {
			...
			if (!((mrq->cmd && mrq->cmd->error) ||
				...
				MMC_TRACE(mmc, "%s: completing tag -> %lu\n",
						__func__, tag);
#ifdef CONFIG_MMC_IO_STATS
				if (mmc_card_mmc(cq_host->mmc->card))
					mmc_iostat_add(cq_host->mmc->card, mrq);
#endif
				cmdq_finish_data(mmc, tag);
			}
		}
