...
static int partition_major;
static int partition_minor;
	...
	while ((part = disk_part_iter_next(&piter))) {
		if (strcmp(part->info->volname, part_name) == 0) {
			...
			*nr_sect = part->nr_sects;
			...
			//printk("[%s][%d] partition_major %d, minor %d\n", __FUNCTION__, __LINE__,
			//		partition_major, target_disk->first_minor);
			break;
		}
	}
...
static int sd_block_rw(int write, sector_t index, sector_t index_offset, void *buffer, size_t len)
{
	...
	//printk("[%s][%d] index %x offset %x\n", __FUNCTION__, __LINE__, index, index_offset);
	...
	if (bh) {
		...
		//printk("[%s][%d] bh->b_blocknr %x bh->b_size %x\n", __FUNCTION__, __LINE__,
