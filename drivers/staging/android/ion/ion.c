...
static int ion_proc_stat_show(struct seq_file *s, void *unused)
{
	...
	for (n = rb_first(&dev->buffers); n; n = rb_next(n)) {
		...
		total_all_buffer += buffer->size;
		if (buffer->stat_pid > 0) {
			stat = find_list(&pid_list, buffer->stat_pid);
			if (!stat) {
				stat = kzalloc(sizeof(*stat), GFP_KERNEL);
				if (stat) {
					stat->pid = buffer->stat_pid;
					stat->size = 0;
					stat->orph_size = 0;
...
static void ion_proc_init_for_buffer(struct ion_buffer *buffer)
{
	struct ion_device *dev = buffer->dev;

	if (!dev->proc_debug_root) {
		return;
	}
	...
	buffer->stat_pid = task_tgid_nr(current);;
}
#endif
...
#ifdef CONFIG_RMS_ION_STAT
	ion_proc_init_for_buffer(buffer);
#endif
	return buffer;
...
#ifdef CONFIG_RMS_ION_STAT
	proc_node = debugfs_create_dir("proc", idev->debug_root);
	if (!proc_node) {
		pr_err("ion: failed to create debugfs proc directory.\n");
	} else {
		idev->proc_debug_root = debugfs_create_file("stat", 0444, proc_node, idev, &ion_proc_stat_ops);
		if (!idev->proc_debug_root)
			pr_err("ion: failed to create debugfs proc/stat node.\n");
	}
#endif
