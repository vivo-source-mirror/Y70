...
static unsigned long lowmem_scan(struct shrinker *s, struct shrink_control *sc)
{
	...
	for_each_process(tsk) {
		...
		/*added by caichangshui start*/
		if (unlikely(selected_oom_score_adj == 0 && !strncmp(tsk->comm, "com.vivo.rms", 12)))
			continue;
		/*added by caichangshui end*/

		/* luruichao add begin */
		if (unlikely(bsp_test_mode)) {
			if (strnstr(tsk->comm, "bsptest", strlen(tsk->comm)) || strnstr(tsk->comm, "stressapptest", strlen(tsk->comm))) {
				lowmem_print(3, "do not kill process %s(pid %d) for low memory\n", tsk->comm, tsk->pid);
				continue;
			}
		}
		/* luruichao add end */

		if (strnstr(tsk->comm, "om.vivo.apitest", strlen(tsk->comm)))
			continue;
