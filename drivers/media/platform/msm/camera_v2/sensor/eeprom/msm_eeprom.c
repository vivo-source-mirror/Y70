...
static int eeprom_parse_memory_map_Sunsum(struct msm_eeprom_ctrl_t *e_ctrl,
	struct msm_eeprom_memory_map_array *eeprom_map_array)
{
	int rc =  0, i, j, k;
	uint8_t *memptr;
	struct msm_eeprom_mem_map_t *eeprom_map;
	e_ctrl->cal_data.mapdata = NULL;
	e_ctrl->cal_data.num_data = 0x1000;
	e_ctrl->cal_data.mapdata =
		kzalloc(e_ctrl->cal_data.num_data, GFP_KERNEL);
	if (!e_ctrl->cal_data.mapdata)
		return -ENOMEM;
	...
	for (k = 0; k < 64; k++) {
		...
		e_ctrl->i2c_client.i2c_func_tbl->i2c_write(&(e_ctrl->i2c_client), 0x0A02, (0x15 + k), MSM_CAMERA_I2C_BYTE_DATA);//choose page
		e_ctrl->i2c_client.i2c_func_tbl->i2c_write(&(e_ctrl->i2c_client), 0x0A00, 0x01, MSM_CAMERA_I2C_BYTE_DATA);//Read Command
		msleep(1);
		rc = e_ctrl->i2c_client.i2c_func_tbl->i2c_read_seq(&(e_ctrl->i2c_client), 0x0A04, memptr, 64);
		if (rc < 0) {
			pr_err("%s: read failed\n", __func__);
			goto clean_up;
		}
		e_ctrl->i2c_client.i2c_func_tbl->i2c_write(&(e_ctrl->i2c_client), 0x0A00, 0x04, MSM_CAMERA_I2C_BYTE_DATA);
		e_ctrl->i2c_client.i2c_func_tbl->i2c_write(&(e_ctrl->i2c_client), 0x0A00, 0x00, MSM_CAMERA_I2C_BYTE_DATA);//Initial command
		memptr +=  64;
	}
	memptr = e_ctrl->cal_data.mapdata;
	for (i = 0; i < e_ctrl->cal_data.num_data; i++)
		CDBG("Sunsum memory_data[%d] = 0x%X\n", i, memptr[i]);
	return rc;
	...
	void *argp)
{
	...
	isSunsumType = cdata->cfg.eeprom_info.isSunsumMode;
	...
	if (isSunsumType == 1)
		rc = eeprom_parse_memory_map_Sunsum(e_ctrl, memory_map_arr);
	else
	...
	void *argp)
{
	...
	isSunsumType = cdata32->cfg.eeprom_info.isSunsumMode;
	...
	if (isSunsumType == 1)
		rc = eeprom_parse_memory_map_Sunsum(e_ctrl, mem_map_array);
	else
