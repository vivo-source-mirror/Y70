	...
	uint16_t power_setting_size)
{
	...
	for (i = 0; i < power_setting_size; i++) {
		...
		case CAM_S7A:
			for (j = 0; j < num_vreg; j++) {
				if (!strcmp(cam_vreg[j].reg_name, "cam_s7a")) {
					CDBG("%s:%d i %d j %d cam_vana\n",
						__func__, __LINE__, i, j);
					power_setting[i].seq_val = j;
					if (VALIDATE_VOLTAGE(
	...
	*power_setting_size = count;
	...
	for (i = 0; i < count; i++) {
		...
		case SENSOR_GPIO:
			...
			else if (!strcmp(seq_name, "sensor_gpio_s7a"))
				ps[i].seq_val = SENSOR_GPIO_S7A;
	...
	uint16_t gpio_array_size)
{
	...
	rc = of_property_read_u32(of_node, "qcom,gpio-s7a", &val);
	if (rc != -EINVAL) {
		if (rc < 0) {
			pr_err("%s:%d read qcom,gpio-s7a failed rc %d\n",
				__func__, __LINE__, rc);
			goto ERROR;
		} else if (val >= gpio_array_size) {
			pr_err("%s:%d qcom,gpio-s7a invalid %d\n",
				__func__, __LINE__, val);
			rc = -EINVAL;
			goto ERROR;
		}
		gconf->gpio_num_info->gpio_num[SENSOR_GPIO_S7A] =
			gpio_array[val];
		gconf->gpio_num_info->valid[SENSOR_GPIO_S7A] = 1;
		CDBG("%s qcom,gpio-s7a %d\n", __func__,
			gconf->gpio_num_info->gpio_num[SENSOR_GPIO_S7A]);
	} else {
		rc = 0;
	}
	...
	for (index = 0; index < ctrl->power_setting_size; index++) {
		...
		case SENSOR_GPIO:
			...
			if (!ctrl->gpio_conf->gpio_num_info->valid
				[power_setting->seq_val])
				continue;
			CDBG("%s:%d power_up gpio set val %d\n", __func__, __LINE__,
