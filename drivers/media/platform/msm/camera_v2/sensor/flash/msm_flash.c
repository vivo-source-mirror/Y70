...
static struct msm_flash_ctrl_t *g_flash_ctrl;
				...
				struct kobj_attribute *attr, char *buf)
{
	...
	for (i = 0; i < g_flash_ctrl->torch_num_sources; i++)
		if (g_flash_ctrl->torch_trigger[i])
			led_trigger_event(g_flash_ctrl->torch_trigger[i], 255);
		...
		struct kobj_attribute *attr, const char *buf, size_t size)
{
	...
	if (flashlightstate == 0) {
		...
		for (i = 0; i < g_flash_ctrl->torch_num_sources; i++)
			if (g_flash_ctrl->torch_trigger[i])
				led_trigger_event(g_flash_ctrl->torch_trigger[i], 75);
		if (g_flash_ctrl->switch_trigger)
			led_trigger_event(g_flash_ctrl->switch_trigger, 1);
	}
	return size;
...
__ATTR(EXT_REAR_FLASH_LED, 0644, ext_flash_led_show, ext_flash_led_store);
...
//vivo add ext_flash_led interface end
...
static int32_t msm_flash_platform_probe(struct platform_device *pdev)
{
	...
	/* add for vivo  ext_flash_led interface for flashlight */
