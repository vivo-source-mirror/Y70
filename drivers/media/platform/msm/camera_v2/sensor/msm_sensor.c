...
static bool gc2375_probe_success;
static bool ov02a10_probe_success;
...
int msm_sensor_power_up(struct msm_sensor_ctrl_t *s_ctrl)
{
	...
	struct msm_camera_i2c_reg_setting i2c_settings;
	struct msm_camera_i2c_reg_array reg_setting_ov02a10[] = {
		{0xfd, 0x01, 0x00},
		{0xac, 0x00, 0x00},
		{0xfd, 0x00, 0x00},
		{0x2f, 0x80, 0x00},
	...
	struct msm_camera_i2c_reg_array reg_setting_gc2375[] = {
		{0xef, 0x00, 0x00},
		{0xfc, 0x8f, 0x00},
		{0xf9, 0x43, 0x00},
	};
	...
	pr_err("Sensor %d tagged as %s\n", s_ctrl->id,
		(s_ctrl->is_secure)?"SECURE":"NON-SECURE");

	if ((s_ctrl->is_probe_succeed == 0) && ((strcmp(sensor_name, "gc2375h_PD1945") == 0) || (strcmp(sensor_name, "ov02a10_PD1945") == 0))) {
		int index;
		for (index = 0; index < power_info->power_setting_size; index++) {
			CDBG("%s index %d\n", __func__, index);
			CDBG("%s type %d seq_val = %d\n", __func__, power_info->power_setting[index].seq_type, power_info->power_setting[index].seq_val);
			if (power_info->power_setting[index].seq_val == SENSOR_GPIO_CUSTOM2) {
				power_info->power_setting[index].config_val = 2;
				}
			}
	}
	...
	for (retry = 0; retry < 3; retry++) {
		...
		/*xuliang add for two camera share the same mipi phy and another one should enter standby to let mipi pin go into high-ohm status begin*/
		if ((strcmp(sensor_name, "gc2375h_PD1945") == 0) && (s_ctrl->is_probe_succeed == 1) && (ov02a10_probe_success == true)) {
			i2c_settings.reg_setting = reg_setting_ov02a10;
			i2c_settings.addr_type = MSM_CAMERA_I2C_BYTE_ADDR;
			i2c_settings.data_type = MSM_CAMERA_I2C_BYTE_DATA;
			i2c_settings.size = ARRAY_SIZE(reg_setting_ov02a10);
			i2c_settings.delay = 5;

			if (s_ctrl->sensor_i2c_client->cci_client) {
			orig_slave_addr =
				s_ctrl->sensor_i2c_client->cci_client->sid;
			}
			CDBG("DEBUG SSS s_ctrl->sensor_i2c_client->cci_client->sid = %x\n", s_ctrl->sensor_i2c_client->cci_client->sid);
			s_ctrl->sensor_i2c_client->cci_client->sid = 0x7a>>1;
			rc = s_ctrl->sensor_i2c_client->i2c_func_tbl->i2c_write_table(
			s_ctrl->sensor_i2c_client, &i2c_settings);
			if (rc < 0) {
				pr_err("DEBUG SSS write ov02a10 array failed\n");
			} else {
				pr_err("DEBUG SSS success let ov02a10 sensor enter standby mode\n");
			}
			s_ctrl->sensor_i2c_client->cci_client->sid = orig_slave_addr;
			CDBG("DEBUG SSS orig_slave_addr = %x\n", s_ctrl->sensor_i2c_client->cci_client->sid);
		} else if (strcmp(sensor_name, "ov02a10_PD1945") == 0 && (s_ctrl->is_probe_succeed == 1) && (gc2375_probe_success == true)) {
			i2c_settings.reg_setting = reg_setting_gc2375;
			i2c_settings.addr_type = MSM_CAMERA_I2C_BYTE_ADDR;
			i2c_settings.data_type = MSM_CAMERA_I2C_BYTE_DATA;
			i2c_settings.size = ARRAY_SIZE(reg_setting_gc2375);
			i2c_settings.delay = 5;
			...
			CDBG("DEBUG SSS s_ctrl->sensor_i2c_client->cci_client->sid = %x\n", s_ctrl->sensor_i2c_client->cci_client->sid);
			s_ctrl->sensor_i2c_client->cci_client->sid = 0x2e>>1;
			rc = s_ctrl->sensor_i2c_client->i2c_func_tbl->i2c_write_table(
			s_ctrl->sensor_i2c_client, &i2c_settings);
			if (rc < 0) {
				pr_err("DEBUG SSS write gc2375 array failed\n");
			} else {
				pr_err("DEBUG SSS success let gc2375 sensor enter standby mode\n");
			}
			s_ctrl->sensor_i2c_client->cci_client->sid = orig_slave_addr;
			CDBG("DEBUG SSS orig_slave_addr = %x\n", s_ctrl->sensor_i2c_client->cci_client->sid);
		}
	...
	if ((strcmp(sensor_name, "gc2375h_PD1945") == 0) || (strcmp(sensor_name, "ov02a10_PD1945") == 0)) {
		if (s_ctrl->is_probe_succeed == 1) {
			if ((strcmp(sensor_name, "gc2375h_PD1945") == 0)) {
					gc2375_probe_success = true;
			}
			if ((strcmp(sensor_name, "ov02a10_PD1945") == 0)) {
					ov02a10_probe_success = true;
			}
		} else {
			if ((strcmp(sensor_name, "gc2375h_PD1945") == 0)) {
					gc2375_probe_success = false;
					pr_err("DEBUG SSS:%s probe fail", sensor_name);
			}
			if ((strcmp(sensor_name, "ov02a10_PD1945") == 0)) {
					ov02a10_probe_success = false;
					pr_err("DEBUG SSS:%s probe fail", sensor_name);
			}
		}
	}
	...
	if ((s_ctrl->is_probe_succeed == 0) && ((strcmp(sensor_name, "gc2375h_PD1945") == 0) || (strcmp(sensor_name, "ov02a10_PD1945") == 0))) {
		int index;
		for (index = power_info->power_setting_size-1; index > 0; index--) {
			CDBG("%s index %d\n", __func__, index);
			CDBG("%s type %d seq_val = %d\n", __func__, power_info->power_setting[index].seq_type, power_info->power_setting[index].seq_val);
			if (power_info->power_setting[index].seq_val == SENSOR_GPIO_CUSTOM2) {
				power_info->power_setting[index].config_val = 0;
				break;
				}
			}
	}
