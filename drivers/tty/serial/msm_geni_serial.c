...
static int msm_geni_serial_port_setup(struct uart_port *uport)
{
	...
	if (!uart_console(uport) || is_atCmd == 1) {
...
static int msm_geni_serial_startup(struct uart_port *uport)
{
	...
	if (uport->cons && !(uport->cons->flags & CON_ENABLED)) {
		pr_err("fake console startup\n");
		if (uport->cons->index == -1) {
			uport->cons->index = 0;
			pr_err("fake console setup\n");
			ret = uport->cons->setup(uport->cons, NULL);
			if (ret) {
				dev_err(uport->dev, "%s:Console setup fail, ret %d\n",
						__func__, ret);
				return ret;
			}
		}
