...
static int diag_socket_read(void *ctxt, unsigned char *buf, int buf_len)
{
	...
	if (total_recd > 0) {
		...
		if (g_wifi_diag_log == 1) {
			print_hex_dump(KERN_INFO, "Diag Packet Data before diag_socket_read : ", 16, 1, DUMP_PREFIX_ADDRESS, buf, total_recd, 1);
			DIAG_LOG(DIAG_DEBUG_PERIPHERALS, "%02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x\n",
...
static int diag_socket_write(void *ctxt, unsigned char *buf, int len)
{
	...
	if (g_wifi_diag_log == 1) {
		print_hex_dump(KERN_INFO, "Diag Packet Data before diag_socket_write : ", 16, 1, DUMP_PREFIX_ADDRESS, buf, len, 1);
		DIAG_LOG(DIAG_DEBUG_PERIPHERALS, "%02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x\n",
			*(buf), *(buf+1), *(buf+2), *(buf+3), *(buf+4), *(buf+5), *(buf+6), *(buf+7), *(buf+8), *(buf+9),
			*(buf+10), *(buf+11), *(buf+12), *(buf+13), *(buf+14), *(buf+15), *(buf+16), *(buf+17), *(buf+18), *(buf+19));
	}
