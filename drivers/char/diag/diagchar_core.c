...
extern unsigned char g_wifi_diag_log;
...
static int diag_process_userspace_remote(int proc, void *buf, int len)
{
	...
	if (g_wifi_diag_log == 1) {
		DIAG_LOG(DIAG_DEBUG_PERIPHERALS, "CMD:%02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x, len:%d\n",
			*(data_buf), *(data_buf+1), *(data_buf+2), *(data_buf+3), *(data_buf+4), *(data_buf+5), *(data_buf+6),
			*(data_buf+7), *(data_buf+8), *(data_buf+9), *(data_buf+10), *(data_buf+11), len);
	}
...
static int diag_user_process_userspace_data(const char __user *buf, int len)
{
	...
	u_data_buf = driver->user_space_data_buf;
	if (g_wifi_diag_log == 1) {
		DIAG_LOG(DIAG_DEBUG_PERIPHERALS, "CMD:%02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x, len:%d\n",
			*(u_data_buf), *(u_data_buf+1), *(u_data_buf+2), *(u_data_buf+3), *(u_data_buf+4), *(u_data_buf+5), *(u_data_buf+6),
			*(u_data_buf+7), *(u_data_buf+8), *(u_data_buf+9), *(u_data_buf+10), *(u_data_buf+11), *(u_data_buf+12),
			*(u_data_buf+13), *(u_data_buf+14), *(u_data_buf+15), len);
	}
	...
	if (!remote_proc) {
		...
		mutex_unlock(&driver->md_session_lock);
		if (g_wifi_diag_log == 1) {
			DIAG_LOG(DIAG_DEBUG_PERIPHERALS, "CMD:%02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x, hdlc_disabled:%02x\n",
				*(u_data_buf), *(u_data_buf+1), *(u_data_buf+2), *(u_data_buf+3), *(u_data_buf+4), *(u_data_buf+5), *(u_data_buf+6),
				*(u_data_buf+7), *(u_data_buf+8), *(u_data_buf+9), *(u_data_buf+10), *(u_data_buf+11), hdlc_disabled);
		}
		if (!hdlc_disabled)
