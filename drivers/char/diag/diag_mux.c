...
/* vivo guojunzheng add for engineermode begin */
unsigned char g_wifi_diag_log;
EXPORT_SYMBOL(g_wifi_diag_log);
static ssize_t write_wifi_diag_log(struct file *file, const char __user *buf,  size_t count, loff_t *ppos)
{
	char userbuf[4] = {0};
	if (count < 1) {
		printk("g_wifi_diag_log count error");
		return -EFAULT;
	}

	if (copy_from_user(userbuf, buf, 1)) {
		printk("g_wifi_diag_log copy_from_user error");
		return -EFAULT;
	}
	...
	printk("g_wifi_diag_log:%d", g_wifi_diag_log);
	return count;
}

static ssize_t read_wifi_diag_log(struct file *filp, char __user *buf, size_t count, loff_t *ppos)
{
	char tmpbuf[12];
	ssize_t length;

	length = scnprintf(tmpbuf, 12, "%d", g_wifi_diag_log);
	return simple_read_from_buffer(buf, count, ppos, tmpbuf, length);
}

static const struct file_operations wifi_diag_log_ops = {
	.read		= read_wifi_diag_log,
	.write		= write_wifi_diag_log,
...
int diag_mux_init(void)
{
    ...
    /* vivo guojunzheng add for engineermode begin */
	g_wifi_diag_log = 0;
	debugfs_create_file("wifi_diag_log", 0644, NULL, NULL, &wifi_diag_log_ops);
	/* vivo guojunzheng add for engineermode end */
	return 0;
}
