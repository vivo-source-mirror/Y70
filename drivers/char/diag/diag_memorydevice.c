...
int diag_md_write(int id, unsigned char *buf, int len, int ctx)
{
	...
	if (g_wifi_diag_log == 1) {
		DIAG_LOG(DIAG_DEBUG_PERIPHERALS, "diag:resp to user buf: %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x len: %d, cur_pid:%d, pid:%d, found:%d\n",
			*(char *)(buf), *(char *)(buf + 1), *(char *)(buf + 2),
			*(char *)(buf + 3), *(char *)(buf + 4), *(char *)(buf + 5),
			*(char *)(buf + 6), *(char *)(buf + 7), *(char *)(buf + 8),
			*(char *)(buf + 9), *(char *)(buf + 10), *(char *)(buf + 11),
			*(char *)(buf + 12), *(char *)(buf + 13), *(char *)(buf + 14),
			*(char *)(buf + 13), *(char *)(buf + 14), *(char *)(buf + 15),
			*(char *)(buf + 16), *(char *)(buf + 17), *(char *)(buf + 18),
			len, current->pid, pid, found);
	}
	...
	for (i = 0; i < NUM_DIAG_MD_DEV && !err; i++) {
		...
		for (j = 0; j < ch->num_tbl_entries && !err; j++) {
			...
			if (task_s) {
				...
				if (g_wifi_diag_log == 1) {
					pr_err("drain_again[%d], length[%d]\n", drain_again, entry->len);
					print_hex_dump(KERN_INFO, "@md write: ", DUMP_PREFIX_NONE,
