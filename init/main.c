...
unsigned int parallel_charger_type;
EXPORT_SYMBOL(parallel_charger_type);
...
static void __init is_at_boot(char *command_line)
{
	...
	if (strnstr(command_line, "sendNormal", strlen(command_line)) != NULL) {
		is_atCmd = 1;
	} else {
		is_atCmd = 0;
	}
	...
	//vivo hankecai add for normal_boot_mode init begin
	if ((power_off_charging_mode == 1) || (is_atboot == 1))
		...
		is_normal_mode = 0;
	//vivo hankecai add for normal_boot_mode init end
}
...
static void __init print_cmdline(void)
{
	...
	if (printed_len	< cmdline_len && printed_len < COMMAND_LINE_SIZE) {
