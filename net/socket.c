...
//vivo <jiangyu>,add for NETWORK_STATS_WITH_PACK_INFO begin{T1905131102}
	...
	if (pid) {
		...
		if (tgid_task) {
			snprintf(file->f_owner.tgid_task, TASK_COMM_LEN, tgid_task->comm);
...
out:
	...
	// vivo zouzhangzhi add for fd_leak_log begin.
	if (retval > FD_LEAK_COUNT)
	  filterByProcessName("system_server", retval, "socket");
	// vivo zouzhangzhi add for fd_leak_log end.
	return retval;
